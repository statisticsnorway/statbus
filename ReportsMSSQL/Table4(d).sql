...
Select Name, [Total], [Small], [Medium], [Big], [Farm], [Individual], [Sublegal Unit] 
	From (
		Select Coalesce(reg.Name, 'Кыргызская Республика') as Name, isnull(SizeGrp,'Total') SizeGrp, ActTotal 
		From (
			Select Region, SizeGrp, 
			Case SizeGrp
			When 'Small' Then 1
			When 'Medium' Then 2
			When 'Big' Then 3
			...
			Sum(Statuso) ActTotal
			From (
				Select * From ActualStatWithRegions a
				Cross Apply
				(Select ChildId as Region From RegionsFinal Where (Level = 2 and a.Parents Like '%.' + Convert(varchar(6), ChildId) + '.%') or (Level = 2 and LEN(Convert(varchar(6), a.Parents)) = 1)) t1
			) t group by Region, SizeGrp With Cube
		) x Left Join Regions reg on reg.Id = x.Region
	) tb
Pivot
(
sum(ActTotal) For SizeGrp in ([Total], [Small], [Medium], [Big], [Farm], [Individual], [Sublegal Unit])
) pvt
