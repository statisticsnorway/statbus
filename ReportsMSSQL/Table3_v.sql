...
---Table 3_v, PIVOT IS NEEDED

Select a.Name, SizeGrp, Region, Total
	From (
		Select Left(Sect,1) as Sect, SizeGrp, Coalesce(r.Name, 'Кыргызская Республика') Region,
		Case SizeGrp
		When 'Small' Then 1
		When 'Medium' Then 2
		When 'Big' Then 3
		...
		Right Join Regions r on r.Id = t.Region
		Group by Left(Sect,1), r.Name, SizeGrp with Cube
	) x
	Join ActivityCategories a on x.Sect = a.Code
