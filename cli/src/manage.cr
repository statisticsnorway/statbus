  ...
  class Manage
    ...
    #{derived.domain} {
            ...
            handle /api/* {
                ...
                reverse_proxy #{derived.postgrest_bind_address} {
                    header_up Host {host}
                    header_up X-Real-IP {remote}
                    header_up Authorization "Bearer {http.request.cookie.statbus-#{config.deployment_slot_code}}"
                }
            }
            
            handle / {
                reverse_proxy #{derived.app_bind_address} {
                    header_up Host {host}
                    header_up X-Real-IP {remote}
                }
            }
