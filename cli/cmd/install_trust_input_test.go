	...
	for _, source := range []string{"flag", "file"} {
		for _, fail := range []bool{false, true} {
			...
			t.Run(source+"/"+name, func(t *testing.T) {
				...
				if source == "file" {
					content += "TRUST_GITHUB_USER=fixture-signer\n"
				}
				...
				if _, exists := cfg.Get(installinput.TrustKey); exists {
					t.Fatal("input trust username copied into runtime configuration")
				}
