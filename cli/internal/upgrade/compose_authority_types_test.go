...
var allowedProcessLaunches = map[string]approvedLaunch{
	...
	"cmd/install_ports.go:occupiedPortOwner.func|os/exec|systemctl":                                    {Count: 1, Reason: "installer reads active systemd services without requiring root privileges"},
	"cmd/install_ports.go:occupiedPortOwner.func|os/exec|ss":                                            {Count: 2, Reason: "installer confirms a listener and optionally reads its process name before any service starts"},
	...
	"internal/config/config.go:computeDerivedInDir|os/exec|git":                                       {Count: 2, Reason: "computeDerivedInDir reads branch and revision from the caller's explicit checkout to populate generated deployment metadata"},
