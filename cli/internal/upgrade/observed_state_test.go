...
func TestIsConnError_CancellationStrings(t *testing.T) {
	cases := []struct {
		...
		{name: "context.DeadlineExceeded-is-terminal", err: context.DeadlineExceeded, want: false},
		{name: "wrapped-context.Canceled-is-terminal", err: fmt.Errorf("scan failed: %w", context.Canceled), want: false},
		{name: "conn-closed-prose-is-not-a-contract", err: errors.New("pgx: conn closed"), want: false},
		{name: "connection-reset-prose-is-not-a-contract", err: errors.New("write tcp ...: connection reset by peer"), want: false},
		{name: "pgx-context-already-done-prose-is-not-a-contract", err: errors.New("timeout: context already done: context canceled"), want: false},
...
func TestPreBackupStopFailureCleanupPreservesMarkerOnAnyUncertainty(t *testing.T) {
	...
	for _, required := range []string{
		"needsRecovery := false",
		`compose.Up(restartCtx, d.projDir, restartArgs...)`,
		`runPreparedCommandWithTimeout(restartCtx, restartCmd, 5*time.Minute, "docker compose up", restartArgs)`,
		`d.setMaintenance(false, "")`,
		"d.reconnect(ctx)",
