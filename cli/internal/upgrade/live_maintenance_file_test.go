	...
	t.Cleanup(func() {
		...
		for _, id := range ids {
			if _, err := d.queryConn.Exec(cctx, "DELETE FROM public.upgrade_state_log WHERE upgrade_id = $1", id); err != nil {
				t.Errorf("cleanup state log for id=%d: %v", id, err)
			}
			if _, err := d.queryConn.Exec(cctx, "DELETE FROM public.upgrade WHERE id = $1", id); err != nil {
				t.Errorf("cleanup row id=%d: %v", id, err)
			}
		}
		_ = os.Remove(maintenanceFlagHostPath())
	})

	version := "v2099.01.0-rc.01"
	cases := []struct {
		name               string
		sha                string
		commitVersion      *string
		wantHeadlineTarget string
		wantJSONVersion    string
	}{
		{name: "tagged", sha: "3470000000000000000000000000000000000041", commitVersion: &version, wantHeadlineTarget: version, wantJSONVersion: `"commit_version":"` + version + `"`},
		{name: "null commit_version", sha: "3470000000000000000000000000000000000042", commitVersion: nil, wantHeadlineTarget: "34700000", wantJSONVersion: `"commit_version":null`},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := d.queryConn.QueryRow(ctx, `
				...
				VALUES ($1, now() - interval '1 day', CASE WHEN $2::text IS NULL THEN '{}'::text[] ELSE ARRAY[$2::text] END, 'prerelease', 'live maintenance-file probe', 'scheduled', now(), $2, 'ready')
				RETURNING id`, tc.sha, tc.commitVersion).Scan(&id); err != nil {
			...
			if lines[0] != "upgrade "+itoa(id)+" to "+tc.wantHeadlineTarget {
				t.Errorf("headline = %q", lines[0])
			}
			for _, key := range []string{`"id":` + itoa(id), tc.wantJSONVersion, `"commit_sha":"` + tc.sha + `"`, `"started_at":"`} {
			...
			// The next case must be able to claim its row without colliding with the
			// single-in-progress invariant. Cleanup remains registered as a loud fallback.
			if _, err := d.queryConn.Exec(ctx, "DELETE FROM public.upgrade_state_log WHERE upgrade_id = $1", id); err != nil {
				t.Fatalf("delete state log before next case: %v", err)
			}
			if _, err := d.queryConn.Exec(ctx, "DELETE FROM public.upgrade WHERE id = $1", id); err != nil {
				t.Fatalf("delete row before next case: %v", err)
			}
		})
	}
}
