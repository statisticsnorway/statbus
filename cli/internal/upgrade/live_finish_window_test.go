...
func TestLiveRollbackFinishing_ExternalWritesReopen(t *testing.T) {
	...
	if err := d.queryConn.QueryRow(ctx, `
		...
		VALUES ($1, now() - interval '2 days', '{}', 'commit', 'live window probe', 'failed',
		        now() - interval '1 hour', now() - interval '59 minutes', $2, $3, '/nonexistent/live-window-probe-backup', 'live-window-probe.log')
		RETURNING id`, sha, ErrGitFetchRetryable, rollbackFinishPendingError("live window probe")).Scan(&id); err != nil {
