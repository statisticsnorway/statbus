...
RE-REVIEW (architect, 2026-06-18) of the engineer's folded must-fixes = COMMIT-READY (stage-1 CLI core + must-fixes). (1) apply-latest now RunRegister→RunSchedule — RACE-PROOF + deploy-safe: register upserts the row so schedule always finds it (silent-no-op + lying message gone); a normal deploy's already-ready row is NOT reset to 'building' by RunRegister's ON CONFLICT WHERE clause (readiness preserved); no new daemon conflict (one-shots atomic+lock-free). (2) AC#3/#9 tests GENUINE: schedule_require_register_test.go — classifyScheduleResult pure-fn ((0,false)→Unregistered-never-insert), TestErrNotRegistered_Actionable (names fix + echoes input), TestOnScheduledNotify_NoInsert (real source-inspection guard: INSERT absent/UPDATE present). Logic unit-proven; full DB wiring AC#8-VM-proven. (3) notify-all-clouds.yaml:49 = `check`. BONUS: RunSchedule state-guard `WHERE commit_sha=$1 AND state != 'in_progress'` + 0-row SELECT-state → errNotRegistered (no row) vs accurate in_progress refuse message (not misleading 'already scheduled'); completed/failed still re-run. Scope: fabricate still in tree (stage-2 deletes via ruling-C harness swap + AC#8 VM proof). Follow-ons (not blockers): --recreate double-NOTIFY (foreman filing); AC#7 doc sweep (DEPLOYMENT.md:337/687, DEVELOPMENT.md:621, comment service.go:1580).
