...
SSH-FAILURE ROOT CAUSE NAMED (foreman root-read, 2026-07-09 00:25): NOT drone-ssh (that theory refuted by its own test — the reverted single line failed identically), NOT the runner (all failing runs GitHub-hosted), NOT CrowdSec (zero active decisions). It is /etc/sshdoers:44 — statbus_test's CI key is allowlisted to exactly 'cd statbus && git fetch origin && git checkout -- . && git checkout <40-hex> && ./dev.sh continous-integration-test'; any other command is SILENTLY rejected (exit 1, no output, ~2s — sshd journal shows accepted→opened→closed for every GHA attempt). Both the multi-line block (b2a5cbe8e) and the ci-heal-inserted single line (b4df4bff2) failed the match. FALSE-EXONERATION LESSON: the operator's exec-channel tests ran under a different, unrestricted key — the discriminating variable was the KEY, not the channel; sshdo forces per-key. FIX (no server write): the self-heal invocation moves INSIDE continous-integration-test (dev.sh), and the workflow's SSH line restores byte-for-byte to the allowlisted template. The dev.sh relocation of the heal logic (b4df4bff2) stands on its own merits (architect: improvement on placement). Cost evidence recorded on STATBUS-128 (loud rejection would have ended this in one log read).
<!-- SECTION:NOTES:END -->
