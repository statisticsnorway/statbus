...
> BENEFIT: closes a 144-class hole on the exact path an NSO operator will walk: parked box → run install → still broken → walk away to get the fix — today that sequence leaves the daemon DOWN (no discovery, no scheduled backups, no NOTIFY listener, no future siren) until someone manually starts a systemd user unit they don't know exists.
...
WHY "the operator sees the non-zero exit" does not carry: the NSO-operator frame — the operator's action is install; requiring them to also diagnose an inactive systemd user unit is the class the operator-UX doctrine forbids, and the NEXT siren cannot fire from a dead daemon.
