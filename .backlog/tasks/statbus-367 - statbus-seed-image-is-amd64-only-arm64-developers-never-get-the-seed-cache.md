...
**In Progress:** `c57e12f04`, `2fe060ea5`: `postgres/Dockerfile`, `.github/workflows/images.yaml`, and `cli/cmd/seed.go` implement scratch payload and dual-platform index. The published one-digest GHCR manifest, arm64 single-restore run, and final adversarial acceptance are not evidenced. **Remaining:** inspect the published manifest and execute arm64 `./dev.sh build-sb` with one restore, not full replay.
