...
`internal/migrate` imports five symbols from `internal/release`: `MigrationInReleasedTag`, `MigrationExistsInTag`, `FileIsDirty`, `IntentionallyFixBrokenImmutableMigrationEnvVar` (all in `immutability.go`, verified to build standalone with stdlib only) and `CurrentImmutabilityBaselineTag` (in `predecessor.go`, also stdlib only; the rest of `predecessor.go` is tool-side). Both belong with migrate. Once they move, `internal/release` is imported by nothing box-side (verified: `go list -deps ./internal/upgrade ./internal/install` reaches `release` only through `migrate`), and the partition is real in the import graph.

## What to do

1. **Move `internal/release/immutability.go`** (and its test) and `CurrentImmutabilityBaselineTag` from `predecessor.go` to `internal/migrate/immutability.go`. Update `cmd/release.go` to import them from `migrate`. Assert with a test that `go list -deps` of the box-side commands does not contain `internal/release`.
