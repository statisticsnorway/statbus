...
- Mid-migration / between migrations / mid-transaction -> mid-tx (parent-kill in the commit↔record gap, the STATBUS-105 measurement): **[PENDING-145-REDERIVE]** — pre-145 PROVEN on run 28832014634 via the boot-migrate re-hit → STATBUS-017 defer → snapshot-restore PATH, which NO LONGER EXISTS under 145 (boot is floor-only). The 145 terminal is mechanism-independent — ANY parent death in the migration window leaves db.migration unadvanced → the Resuming-arm reads positively Behind → rolled_back — but that 145 path is UNEXERCISED, so the arc is GATED until slice-4's oracle proves it green (expected first try). between-migrations + mid-migration: **[PENDING-145-REDERIVE]** — the pre-145 "in-dispatch forward recovery → completed at attempts==1" proof (run 28837119781) is VOIDED by STATBUS-145: the delta no longer runs in the re-exec'd boot-migrate (now floor-only) but at the applyPostSwap step, so a mid-delta kill reads observed-state Behind → `rolled_back`. Both arcs GATED (loud SKIP marker) until slice-4's ORACLE run derives the true terminal on the inline `./sb install` dispatch.
