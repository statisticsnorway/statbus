...
- The DB dies TRANSIENTLY mid-recovery (killed and revived while a recovery pass runs) -> transient-db-backoff leg (to build) -> the backoff-retry dispatch (STATBUS-109, shipped 782ca2455) retries quietly in-process with the wall-clock connect probe, resolves when the DB returns, recovery completes — no unit exit, no restart-counter noise; the exhaust arm (DB never returns) routes to the classify-then-act disposition — **[UNPROVEN]** (obligation moved from STATBUS-109 at its clean-ship close, 2026-07-12; mechanism + design pins in 109's final summary)
- The target COMMIT is not yet fetchable mid-recovery (a fetch stall, not a dead remote) -> commit-not-fetched-backoff leg (to build) -> the stall-not-deadline git-fetch backoff (~60s no-progress, ~15min ceiling) resolves when the object arrives; the exhaust arm routes to classify-then-act — **[UNPROVEN]** (same 109 close; both legs prove the resolves-vs-exhausts branch pair the standard arcs never exercise)
