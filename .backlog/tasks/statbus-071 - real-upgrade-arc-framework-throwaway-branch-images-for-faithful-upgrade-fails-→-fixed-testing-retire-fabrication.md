...
- Eats all memory -> OS kills Postgres ONCE -> `postswap-migration-oom-arc` — **[PENDING-145-REDERIVE]** (was [PROVEN] forward→COMPLETED on run 28841893851 under PRE-145 code, where boot-migrate applied the delta; STATBUS-145 moves the delta to the applyPostSwap step, so a mid-delta OOM reads observed-state Behind → the ruled terminal is `rolled_back` on the FIRST kill — V-unrecorded + clean-slate fingerprint. The arc is GATED (loud SKIP marker) until slice-4's ORACLE run proves the flipped terminal; the mechanic owns the arc-file flip. The single/recurring-OOM split question is SUPERSEDED by STATBUS-145 — mid-delta OOM → Behind → rolled_back is structural).
...
- mid-tx (parent-kill in the commit↔record gap, the STATBUS-105 measurement): **[PENDING-145-REDERIVE]** — TWO static derivations have now failed, not one: pre-145 PROVEN on run 28832014634 via the boot-migrate re-hit → STATBUS-017 defer → snapshot-restore PATH, which NO LONGER EXISTS under 145 (boot is floor-only); the mechanic's STATBUS-145 re-derivation (any parent death in the migration window → the Resuming-arm reads positively Behind → rolled_back) was itself REFUTED by the wave-2 dispatch — AND that same run was DISQUALIFIED as evidence outright (the daemon unit was found running pre-dispatch, racing the manually-driven recovery; recovery_attempts=2 is the fingerprint of the two-actor race, now fixed at the construction level — the daemon is re-confirmed stopped immediately before each dispatch). No third armchair derivation — the terminal is undetermined by analysis; re-gated pending an instrumented single-actor run (progress-log + journal capture wired in, ready).
- mid-migration (parent-kill before the first migration even starts): **[PENDING-145-REDERIVE]** — TWO static derivations have now failed: the pre-145 "in-dispatch forward recovery → completed at attempts==1" proof (run 28837119781) was voided by STATBUS-145's floor-only boot-migrate; the mechanic's STATBUS-145 re-derivation (a mid-delta kill reads observed-state Behind at postSwapFailure → rolled_back) was itself REFUTED by the wave-2 dispatch — the row reached `completed` via forward recovery instead, for a reason neither derivation predicted. No third armchair derivation — re-gated pending an instrumented single-actor run (progress-log + journal capture wired in, ready).
- between-migrations (parent-kill between the two migrations): **[PROVEN]** run 28976918080 — the mechanic's STATBUS-145 re-derivation was CONFIRMED by the wave-2 dispatch (a green job means the arc ran, un-gated, and every assertion passed): subprocess kill lands after V1 records → the SAME dispatch's postSwapFailure read finds it positively Behind on the first read (V2 missing, DB reachable throughout — no extra restart pass needed) → one-shot rollback → os.Exit(75); ledger reverted to baseline, clean-slate fingerprint matched A. Its two siblings' chains (mid-tx, mid-migration — same reasoning shape) were REFUTED the same wave and are re-gated above; this is the one of the three the oracle confirmed.
