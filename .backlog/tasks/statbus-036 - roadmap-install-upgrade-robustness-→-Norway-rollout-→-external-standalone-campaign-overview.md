...
- THE OPERATOR CONFIG PATH IS SAFE. UPGRADE_CALLBACK survives every config regeneration (the park siren is armed on real boxes — observed firing through three real regenerations), and appending a key to .env.config can no longer corrupt settings (trailing-newline guarantee at the shared writer, committed 7054e7593). The African-operator contract — "re-run the installer" as the only action — holds through park, un-park, and repair.
