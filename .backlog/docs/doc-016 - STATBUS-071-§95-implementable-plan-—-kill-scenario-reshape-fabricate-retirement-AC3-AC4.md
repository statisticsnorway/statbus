...
- 3-postswap-archivebackup-resume — **CONTRACT PENDING FOREMAN CONFIRM** (see §5): the scenario's GREEN is 0-kills + COMPLETED (active-phase WatchdogSec ticker + FIX A: archiveBackup reordered after the completed-UPDATE → row completes WHILE the tar stalls), NOT the "TimeoutStartSec→delta=1→rollback-recomplete" originally briefed. (e)-gate INVERTS to "assert COMPLETED while stalled". Rides C15's mechanism + proves FIX A.
...
- **5c-hard** — the 2 harder CAT-B: resume-died-rollback (dropin-KILL 4th sub-variant, VM-prove) + archivebackup-resume (rides C15 + FIX-A contract, VM-prove; contract pending confirm).
...
- **(e) anti-false-pass (vacuous-guard family):** a bounded-restarts / no-kill assertion is trivially true if the stall never engaged → always assert the stall ACTUALLY held (row state proves it). For C15: still-in_progress after hold. For archivebackup-resume: INVERTED — completed while stalled (FIX A).
- **archivebackup-resume contract discrepancy (PENDING foreman confirm):** the scenario's GREEN is 0-kills + COMPLETED (active-phase WatchdogSec + FIX A), NOT delta=1 + rollback-recomplete. Likely conflated with archivebackup-WATCHDOG. Finalize on confirm.
...
- dropin STALL + WatchdogSec — PROVEN (5c/C15). Easy CAT-B + archivebackup-resume ride it.
