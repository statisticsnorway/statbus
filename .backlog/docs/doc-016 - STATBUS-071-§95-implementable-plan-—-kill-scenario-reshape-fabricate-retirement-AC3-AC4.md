...
- 3-postswap-archivebackup-watchdog, 3-postswap-migration-timeout (migrate.go:363) — ride C15's stall mechanism (structure+review). COMMITTED 9f3463f87.
- 4-rollback-restore-watchdog — RE-SCOPED (see §3 5c-hard): V_fail trigger → rollback → stall the restoreDatabase (stall-dropin) → active-phase ticker → rolled_back. (Originally slated to ride C15, but the self-heal finding re-scoped its rollback-TRIGGER off the resume-re-kill.)
- 3-postswap-resume-died-rollback — **DELETED (product-impossible; subsumed by C8)** — see the DELETE section. The two-run resume-re-kill is unreachable (the STATBUS-067 self-heal :5053 short-circuits the converged-deploy resume); C8 already proves the real resume-death + latch contract. (kill-via-dropin VM-prove 27820228835 RED disproved the unverified legacy claim — charter value.)
- 3-postswap-archivebackup-resume — **DELETED (subsumed by archivebackup-watchdog; the reshape self-heals)** — see the DELETE section. Its RUN1 container-restart kill → converged → RUN2 self-heals (:5053) → completed, the archiveBackup stall (exec.go:1050) NEVER reached. The fabricate-legacy "rollback-recomplete/NRestarts=1" contract was the FABRICATED-Resuming artifact, invalid for the reshape. archivebackup-watchdog (committed green) reaches the IDENTICAL stall site exec.go:1050 on the first upgrade → no distinct coverage.
...
- 3-postswap-resume-died-rollback — **DELETED: describes a PRODUCT-IMPOSSIBLE state.** Its two-run resume-re-kill assumed RUN2's resume re-runs applyPostSwap + re-hits the container-restart kill (:4805). The kill-via-dropin VM-prove (27820228835) RED-disproved it: the STATBUS-067 self-heal canary (resumePostSwap :5053 — `containersAtFlagTarget && !migrate.HasPending → mark completed WITHOUT re-running applyPostSwap`) short-circuits the converged-deploy resume, so :4805 is never reached on RUN2 (completed, NRestarts=0). The legacy header (line 59) admits "this scenario has not yet been run on real systemd" → its RUN2 claim was UNVERIFIED; this arc was the first real run and disproved it. C8 (3-postswap-container-restart-kill, GREEN) already proves the REAL resume-death + latch-no-loop contract (kill at the FIRST applyPostSwap — containers not-yet-at-target → no self-heal → :4805 reached → Resuming → recoveryRollback → rolled_back + UPGRADE_DIED_DURING_RESUME + bounded restarts; a regressed latch → re-resume → completed → C8 REDs on "expected rolled_back"). No coverage loss. King doctrine: a fake describing a state that CANNOT occur → DELETE.
- 3-postswap-archivebackup-resume — **DELETED: subsumed by archivebackup-watchdog; the reshape self-heals.** Same root cause as resume-died (RUN1 = killed-by-system-during-container-restart → converged → RUN2 resumePostSwap → self-heal :5053 → completed; the archiveBackup stall NEVER reached). CODE-GROUNDED subsumption: archivebackup-resume's RUN2 stall + archivebackup-watchdog use the IDENTICAL stall class `archive-backup-stall-active-phase-watchdog` → the SAME single inject site exec.go:1050 (inject.StallHere). archivebackup-watchdog (committed green) reaches it on the first upgrade's post-exit-42 applyPostSwap (active-phase, ticker→completed); archivebackup-resume's second-resume self-heals → never reaches exec.go:1050 → ZERO distinct coverage. The fabricate-legacy "rollback-recomplete/NRestarts=1" contract (runs 27107825797+27109134019) was the FABRICATED-Resuming artifact, invalid for the reshape. No coverage loss. (NOTE: all 3 resume-death-family scenarios — resume-died, archivebackup-resume, rollback-restore — shared the same self-heal-blocked container-restart-kill RUN1 trigger. Family fix = these 2 deletes + rollback-restore re-scoped to V_fail.)
...
## 2. Driver variants (the framework pieces) — 3 PROVEN
The reshaped scenarios live in upgrade-arc-harness.yaml (Q5-1; share install_statbus_at_sha + B-fixture + signing). THREE driver variants by inject mechanism (a 4th, dropin-KILL, was attempted for resume-died-rollback but is dead — see below):
...
4. ~~**daemon-RUN + dropin KILL**~~ **DEAD** — was for resume-died-rollback (now DELETED). The helper `arc_install_kill_dropin` (unit-level Environment=, which correctly survives the exit-42 re-exec) was sound, but its ONLY consumer was resume-died-rollback. rollback-restore (the other prospective consumer) re-scopes to a V_fail trigger (no kill-dropin — see §3 5c-hard). So arc_install_kill_dropin has zero live callers → DELETE it with resume-died-rollback.
...
- **5c-hard** — the resume-death family, RE-SCOPED by the STATBUS-067 self-heal finding (kill-via-dropin RED 27820228835):
  - resume-died-rollback → **DELETED** (product-impossible; subsumed by C8). arc_install_kill_dropin deleted with it.
  - rollback-restore-watchdog → **RETIRE the standalone scenario + FINISH the arc to ASSERT** (CORRECTION 2026-06-21, architect-ratified, file:line-verified — supersedes the earlier "RE-SCOPED to a V_fail trigger → BUILD" framing): the V_fail arc ALREADY EXISTS, observationally. `arcs/postswap-rollback-restore-watchdog-arc.sh` already drives V_fail (postSwap) → postSwapFailure → rollback() → REAL restoreDatabase → stall via `restore-db-stall-watchdog` (CONFIRMED: inject.StallHere at exec.go:761, inside restoreDatabase :738) → active-phase WatchdogSec ticker (runGatedWatchdogTicker) → rolled_back; wired upgrade-arc-harness.yaml:150-174 (reuses the failing V_fail fixture); already the RED/GREEN pair via base_sha (GREEN=master baseline+1-then-FLAT; RED=79375b9f9 baseline+1+SIGABRT-climb; the +1 = the deterministic exit-42 binary-swap handoff). So the WORK is: (1) RETIRE the standalone `scenarios/4-rollback-restore-watchdog.sh` — its Resuming-latch trigger is self-heal-blocked AND the install-recovery harness re-tags release images via stage-head.sh (no V_fail image build), so a standalone V_fail isn't cleanly reachable; subsumed by the arc. (2) FINISH the arc observational→asserting, but ONLY AFTER a VM observe GROUNDS that GREEN reaches rolled_back (header flags it "never cleanly seen" — do NOT assert an unobserved terminal); RED re-cut off CURRENT master (delta trigger-agnostic: master-minus-rollback()-ticker bites V_fail too). exec.go:761 is a UNIQUE site (C15=:4633 reconnect, archivebackup=:1050, rollback-restore=:761) → not subsumed. The whole STATBUS-031 empirical proof (AC#2/#4/#5) now lives in this asserting arc (+ the AC#3 source-order guard already landed).
  - archivebackup-resume → **DELETED** (subsumed by archivebackup-watchdog; the reshape self-heals — see the DELETE section). Self-heal check CONFIRMED: RUN1 container-restart kill → converged → RUN2 self-heal → archiveBackup never reached → its rollback-recomplete contract is product-impossible.
  Easy-2 (migration-timeout + archivebackup-watchdog) COMMITTED 9f3463f87.
...
- ~~dropin KILL during the post-exit-42 resume~~ DEAD — resume-died-rollback DELETED (product-impossible via the STATBUS-067 self-heal); arc_install_kill_dropin removed.
