...
- **5c-hard** — the resume-death family, RE-SCOPED by the STATBUS-067 self-heal finding (kill-via-dropin RED 27820228835):
  ...
  - rollback-restore-watchdog → **RE-SCOPED to a V_fail trigger** (self-heal-defeating): a failing migration (postSwap) → postSwapFailure → rollback() → REAL restoreDatabase → stall the restore (the stall-dropin, C15's mechanism) → active-phase WatchdogSec ticker keeps the unit alive across the stalled restore → rolled_back. V_fail is proven by the GREEN failing arc + is postswap (real restoreDatabase, unlike a PreSwap no-op). Trigger options considered: (i) after-commit kill (:844/:845 → HasPending → rollback) — more complex; (ii) V_fail → CHOSEN (simplest, proven); (iii) pre-step-11 kill — doesn't cleanly reach rollback. Engineer confirms the restore-stall inject class exists. VM-prove.
  - archivebackup-resume → **VERIFY vs the self-heal BEFORE building** — its rollback-recomplete/NRestarts=1 contract was measured on the FABRICATE-based legacy (not a reliable oracle for the reshape); the real reshape may self-heal instead. Check its real recovery path (does RUN1's kill leave a self-healing or non-self-healing state?) before finalizing.
