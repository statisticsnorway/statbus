...
- **5c-hard** — the resume-death family, RE-SCOPED by the STATBUS-067 self-heal finding (kill-via-dropin RED 27820228835):
  ...
  - rollback-restore-watchdog → **RE-SCOPED to a V_fail trigger** (self-heal-defeating) — the ONLY family member with distinct, reachable coverage. A failing migration (postSwap) → postSwapFailure → rollback() → REAL restoreDatabase → stall the restore via `restore-db-stall-watchdog` (CONFIRMED EXISTS: inject.StallHere at exec.go:761, inside restoreDatabase :738 — a real product site, no King-flag) → active-phase WatchdogSec ticker (runGatedWatchdogTicker) keeps the unit alive → rolled_back, NRestarts at baseline, data intact. V_fail is proven by the GREEN failing arc + is postswap (real restoreDatabase, unlike a PreSwap no-op). exec.go:761 is a UNIQUE site (C15=:4633 reconnect, archivebackup=:1050, rollback-restore=:761) → not subsumed → BUILD. (NOTE: the legacy 4-rollback-restore-watchdog's RUN1 trigger was ALSO killed-by-system-during-container-restart → self-heal-blocked on the reshape; V_fail is NECESSARY, not optional.) VM-prove.
