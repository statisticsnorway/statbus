...
- **PostgREST's connection POOL works** under read-only: the baseline vs repro logs show the schema cache still loads (reads are allowed). So the failure is **not** all REST connections.
- **PostgREST's notification LISTENER fails at connect.** The error `connection … failed: session is read-only` is emitted by **libpq only** for its `target_session_attrs=read-write` (primary) check: libpq runs `SHOW transaction_read_only`, and on `on` it rejects the connection. Since our URI doesn't set it, **PostgREST v12 appends `target_session_attrs=read-write` to its listener connection** (it wants the LISTEN channel on the writable primary). `default_transaction_read_only = on` makes the primary report read-only → the listener is rejected → infinite retry → `/ready` 503.
...
- **PostgREST-config-side (rejected).** Disabling the notification channel (no live schema reloads) or forcing `target_session_attrs=any` isn't cleanly exposed in v12 and would break auto schema-cache reloads; version-fragile.
