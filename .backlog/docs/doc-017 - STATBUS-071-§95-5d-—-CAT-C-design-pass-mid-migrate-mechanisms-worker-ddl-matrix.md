...
## 7. Phasing within 5d
5d-a: the one-shot-marker helper enhancement + :388/:911 (one VM-prove). 5d-b: :202 mid-tx (wire wait_for_midtx_stall_ready + kill_pid_in_vm; VM-prove). 5d-c: :844/:845 after-commit + the non-idempotent-V (VM-prove). 5d-d: the shared-fixture refactor + matrix mode. 5d-e: worker-ddl GREEN-asserting reshape (R1 quiesce ALREADY implemented — compose.go:126 wired UPGRADE service.go:4663 + INSTALL install.go:676; STATBUS-100 CLOSED) — build as a normal GREEN scenario + VM-prove as positive regression-proof of the R1 fix on the autonomous-upgrade path; NOT a King-flag (see §4 correction). Then 5e (matrix full-suite → delete fabricate).
