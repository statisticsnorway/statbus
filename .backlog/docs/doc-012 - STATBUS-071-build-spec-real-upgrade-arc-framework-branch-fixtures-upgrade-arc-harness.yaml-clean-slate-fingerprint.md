...
## 8. (f) The kill scenarios — fabricate STAYS in 086, retired in 071 (086 is the ENABLER)
**ARCHITECTURAL THROUGH-LINE (the key insight — fabricate's retirement is GATED on 086, not deferred arbitrarily):** 086's `RunSchedule` is a **lock-free CLI one-shot that SETS a 'scheduled' row WITHOUT running it** → it leaves a persistent **daemon-DOWN 'scheduled' row**. That is *exactly* what the kill scenarios consume (`./sb install` inline-dispatch + `STATBUS_INJECT_AT`), and *exactly* what `fabricate_scheduled_upgrade_row` fakes today with a synthetic INSERT. v2026.05.2 has **no such verb** (its schedule-write lives in the daemon and self-runs — see below). So **086 IS THE ENABLER**: moving the schedule-write from the daemon to a CLI one-shot is the capability that lets 071 reshape the kill scenarios onto post-086 baselines and retire fabricate THERE, with **no synthetic INSERT**. The retirement is gated on the very capability 086 introduces — not an arbitrary deferral.

**THE KILL MECHANISM:** the process-death micro-window scenarios (lost-stamp, resume-death, rollback-kill, mid-migration/mid-tx/between kills) STAY INJECT — branch arcs can't reproduce a kill at an exact instruction. Each needs a daemon-DOWN `public.upgrade` 'scheduled' row that `./sb install` inline-dispatches WITH `STATBUS_INJECT_AT` set (verified: `2-preswap-binary-swap-kill.sh:123-136` = `fabricate_scheduled_upgrade_row` + `./sb install`). The DAEMON must NOT run it — `./sb install` does, so the kill fires at the precise window.

**WHY fabricate can't be retired on the v2026.05.2 baseline** (STATBUS-086 stage-2 — option C REJECTED, verified at the tag): v2026.05.2's CLI `apply` only UPDATEs a discovered row (0 rows for an undiscovered HEAD); the insert-if-missing lives in the SERVICE's `scheduleImmediate`, and the v2026.05.2 daemon runs `handleNotification`→`executeScheduled` **SYNCHRONOUSLY** in one notifyCh iteration (service.go @ tag ~1574) — so daemon-up `apply` self-runs the upgrade immediately (no daemon-down window; the daemon, not `./sb install`, is the dispatcher → no injection), and daemon-down `apply` creates nothing (NOTIFY lost). So **`fabricate_scheduled_upgrade_row` STAYS for the ~18 v2026.05.2-baseline kill scenarios in 086** — AC#6 does NOT fully apply in 086 (its real-path proof rides the AC#8 happy-path + the §5 arcs).

**HOW 071 retires fabricate:** on a POST-086 baseline, `git fetch` → `./sb upgrade register` → `./sb upgrade schedule` (RunSchedule, daemon down) leaves the persistent daemon-down 'scheduled' row → `./sb install` inline-dispatches with injection. 071 reshapes the kill scenarios onto post-086 baselines, then DELETES fabricate.

Inject sites unchanged (migrate.go): `:388` during-migration, `:436-438` mid-tx, `:911` between-migrations, 60-min ceiling `:420`. The kill arcs (Shape catalogue, build-order step 5 — e.g. recovery-of-recovery: kill during B's rollback) layer the inject env onto the install-dispatched scheduled row.
...
- **Albania fidelity is the whole point:** drive the ARCs via the `public.upgrade` row + the systemd service, NEVER `./sb install` or a deploy-branch move (those are operator/cloud paths; they'd test the wrong thing). (The kill scenarios are the exception — they intentionally use `./sb install` inline-dispatch for precise injection; see §8.)
