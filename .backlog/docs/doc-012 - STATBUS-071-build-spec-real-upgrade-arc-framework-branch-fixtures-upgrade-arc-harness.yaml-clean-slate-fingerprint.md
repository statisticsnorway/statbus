...
**SOLUTION (verified, perms-minimal, NO PAT):** JOB-1 EXPLICITLY dispatches images.yaml after pushing each branch: `gh workflow run images.yaml --ref <branch>`. images.yaml is **UNCHANGED** — it already has `workflow_dispatch` (images.yaml:24, no path/tag filter), and its `describe` job checks out the dispatched ref + computes `commit_short = git rev-parse --short=8 HEAD` (images.yaml:49) → the build matrix tags all images `statbus-{app,worker,db,proxy,sb}:<commit_short>` + seed for THAT ref's commit (images.yaml:104 / manifest :133 / seed :173). VERIFIED in code. **RETRY the dispatch** (~5×, 5s backoff): GitHub's ref propagation is eventually-consistent, so `gh workflow run --ref <just-pushed-branch>` can transiently fail "No ref found" if it fires within seconds of the push — a single attempt is a flaky red.

**PERMISSIONS:** JOB-1 needs `contents: write` (push test/*) + `actions: write` (the `gh workflow run` / workflow-dispatch API requires actions:write). GITHUB_TOKEN suffices — **NO PAT** (a PAT is a long-lived, broad, rotation-burdened credential; the explicit dispatch is perms-minimal). The `test/**` PUSH trigger is NOT added.
...
NEW `.github/workflows/upgrade-arc-harness.yaml`, sibling to `install-recovery-harness.yaml`, REUSING its image-wait + cleanup patterns. TRIGGER: `workflow_dispatch` (inputs: `base_sha` [default caller SHA], `scenario`). The tester fires it (single serializer; concurrency cancel-in-progress:false to avoid orphan-on-cancel).

- **JOB 1 — construct** (ubuntu; permissions `contents: write` + `actions: write`): create B + C as REAL commits off A; push `test/<scenario>-migration[-run_id]` then `test/<scenario>-fixed-migration[-run_id]` via GITHUB_TOKEN; **then for EACH pushed branch `gh workflow run images.yaml --ref <branch>` (with the §2 retry)**. Emit `B_short`, `C_short`. The "fix" step in building C = edit V in place (§7 Option 1).
- **JOB 2 — image-wait** (REUSE STATBUS-056): poll ghcr until the per-commit images exist. SKELETON gates the 4 SERVICE images (app/worker/db/proxy) at BOTH `B_short` AND `C_short`. **REAL ARC (c) onward: ALSO wait for `statbus-sb:<short>`** at B_short + C_short — the upgrade's binary procurement (buildBinaryOnDisk → ProcureShort) pulls it; without it the swap falls back to a slow in-container build. (A=base_sha's images, incl. statbus-sb:base_short, are assumed present as a built master commit; if base_sha could be unbuilt, also confirm base_short — see JOB-3 step 1.) Bounded budget (≈40m, cold build); fail loud with the "images built only by images.yaml" diagnostic.
- **JOB 3 — run-arc** (Hetzner VM; mirror harness VM bootstrap + EXIT-trap teardown). Drive via the Albania mechanism (register+schedule, §1):
  1. **Install A at base_sha via the NEW `install_statbus_at_sha <vm> <base_sha>`** — an edge-install PINNED to base_sha: VM `git fetch` + `git checkout <base_sha>` → toolchain-free procure `statbus-sb:<base_short>` (docker pull/create/cp — the proven no-host-Go path) → cp env-config/users → `./sb install`. DETERMINISTIC (no master drift) + post-086 (so the box HAS register/schedule to drive the arc), DISJOINT from the shared `install_statbus_in_vm`. WHY a new helper: `install_statbus_in_vm` CANNOT install an arbitrary post-086 SHA — its empty mode installs master-HEAD (drifts between fire and install → A≠base_sha) and its release-tag mode is all pre-086 (no register/schedule). Precondition: A's images (4 services + statbus-sb) present at base_short (true for a built base_sha). Health-check.
...
- **JOB 4 — teardown** (STATBUS-057; `if: always()`; `contents: write`): delete the two `test/*` branches (RECOMPUTE the names from scenario+run_id so it works even if construct partially failed; ls-remote-guarded, best-effort, exit 0). image-delete DEFERRED to the weekly image-GC (Q2) — commit_short uniqueness guarantees orphan test images can't touch master/release images. VM EXIT-trap reaps the Hetzner instance.
...
**RESOLVED** (foreman per STATBUS-091 autonomy, 2026-06-18; King may revisit): **Option 1 — C EDITS V's `.up.sql` in place** (same version V, corrected bytes → content-hash H_C ≠ H_B). JOB-1's "fix" step edits the migration FILE, not adds a new one. Rationale: it is the King-ratified STATBUS-072 mechanism (the amendments.tsv conveyance, shipped), and Option 2 cannot rescue the FEW-who-failed without collapsing to Option 1 or building a new mechanism.
- **Option 1 — EDIT V IN PLACE (chosen).** FEW (failed at V): V unrecorded → C's V(fixed) applies fresh. MANY (succeeded at V): recorded H_B ≠ on-disk H_C → re-stamp re-records without re-running (declared in migrations/amendments.tsv → eagerContentHashCheck re-stamps; STATBUS-072 shipped). Depth-independent, outcome-preserving.
...
**HOW 071 retires fabricate:** on a POST-086 baseline (install_statbus_at_sha, §5), `git fetch` → `register` → `schedule` (RunSchedule, daemon down) leaves the persistent daemon-down 'scheduled' row → `./sb install` injects. 071 reshapes the kill scenarios onto post-086 baselines, then DELETES fabricate. Inject sites unchanged (migrate.go): `:388` during-migration, `:436-438` mid-tx, `:911` between-migrations, `:420` 60-min ceiling.

## 9. (e) Build order + dependencies
1. **(a) skeleton** [DONE/in-fire]: construct (push + dispatch w/ retry) → image-wait → no-op arc → teardown. Prove the branch/image lifecycle leaves NO orphans BEFORE real arcs.
2. **(c) FIRST scenario:** working→working-fixed (amend/Albania, re-stamp) end-to-end — needs install_statbus_at_sha (§5 step 1) + statbus-sb in image-wait (§5 JOB-2). Exercises STATBUS-072 (shipped).
3. **(d) fingerprint + SECOND scenario:** hanging→hanging-fixed (fail→rollback→fix) — the clean-slate centerpiece.
4. **Shape catalogue:** too-long/OOM at both data sizes, recovery-of-recovery (kill during B's rollback, §8), canary-through-framework; reshape the legacy kill scenarios onto post-086 baselines (register+schedule) + delete fabricate.
5. **(future, separate design)** silent-wrong-data.

DEPENDENCIES: STATBUS-056 (image-wait); STATBUS-057 (teardown branch-delete now; image-GC weekly, Q2); STATBUS-072 (re-stamp — SHIPPED via amendments.tsv); STATBUS-067 (canary THROUGH this framework — drive recovery via the systemd upgrade-service restart, not `./sb install` crashed-recovery which rolls back before the canary).

## 10. Risks / decisions for the engineer
- **Token (Q1, no PAT):** JOB-1 = `contents: write` + `actions: write`; JOB-4 = `contents: write`. image-delete → weekly GC. Dispatch needs the §2 retry (ref-propagation race).
- **install-A (§5 step 1):** the NEW `install_statbus_at_sha` (edge-install pinned to base_sha) — DETERMINISTIC + post-086 + disjoint from `install_statbus_in_vm`. Requires A's images (4 services + statbus-sb) at base_short.
- **C-fix is load-bearing (Option 1):** the construct job must EDIT the migration FILE + add the amendments.tsv row, not add a new migration.
- **Teardown `if: always()` + idempotent:** recompute names from scenario+run_id; a failed arc must still delete its branches.
- **Albania fidelity:** drive the upgrades via the `public.upgrade` row + the systemd service, NEVER `./sb install` or a deploy-branch move. (The kill scenarios are the exception — `./sb install` inline-dispatch for precise injection; §8.)
