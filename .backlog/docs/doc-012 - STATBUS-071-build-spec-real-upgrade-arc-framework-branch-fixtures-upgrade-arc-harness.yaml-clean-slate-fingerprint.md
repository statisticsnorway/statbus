...
## 8. (f) The kill scenarios — fabricate STAYS until the post-086 reshape (NOT in 086)
The process-death micro-window scenarios (lost-stamp, resume-death, rollback-kill, mid-migration/mid-tx/between kills) STAY INJECT — branch arcs can't reproduce a kill at an exact instruction. Their mechanism: a **daemon-DOWN** `public.upgrade` 'scheduled' row that **`./sb install`** inline-dispatches WITH `STATBUS_INJECT_AT` set (verified: `2-preswap-binary-swap-kill.sh:123-136` = `fabricate_scheduled_upgrade_row` + `./sb install`). The DAEMON must NOT run it — `./sb install` does, so the kill fires at the precise window.

WHY fabricate cannot be retired on the v2026.05.2 baseline (STATBUS-086 stage-2 ruling, 2026-06-18 — option C considered and REJECTED, verified at the tag): v2026.05.2 has NO verb that produces a daemon-down 'scheduled' row. Its CLI `apply` only UPDATEs a discovered row (0 rows for an undiscovered HEAD); the insert-if-missing lives in the SERVICE's `scheduleImmediate`, and the v2026.05.2 daemon runs `handleNotification`→`executeScheduled` **SYNCHRONOUSLY** in one notifyCh iteration (service.go @ tag ~1574) — so daemon-up `apply` self-runs the upgrade immediately (no daemon-down window, and the daemon, not `./sb install`, is the dispatcher → no injection), and daemon-down `apply` creates nothing (NOTIFY lost). So **`fabricate_scheduled_upgrade_row` STAYS for the ~18 v2026.05.2-baseline kill scenarios in 086** — AC#6 does NOT fully apply in 086 (its real-path proof rides the AC#8 happy-path + the §5 arcs).

HOW 071 retires fabricate: register+schedule require a post-086 binary, where `./sb upgrade schedule` (RunSchedule) is a **lock-free CLI one-shot that SETS 'scheduled' WITHOUT running it** → daemon-down → the row persists 'scheduled' → `./sb install` inline-dispatches with injection. So 071 reshapes the kill scenarios onto **POST-086 baselines** (`git fetch` → `register` → `schedule` produces exactly the daemon-down row fabricate makes today), then DELETES fabricate. (This is a side-effect of 086's design: moving the schedule-write from the daemon to a CLI one-shot is what makes daemon-down scheduling possible without fabricate.)
