...
DONE (2026-06-17): committed + pushed as 285b787c3 (dev.sh-only, +37). Both parts in: (1) fail-fast `*)` catch-all in both stamp-guard caller switches (fast-test + db-docs); (2) Test 6 write-site source-assert (3 sites gated: fast-test<-FAST_STAMP_WITHHELD, db-docs<-DOCDB_STAMP_WITHHELD, types<-stampGuardRunNoStamp). Verified: foreman byte-review of tmp/sb079.diff + tester live self-test 9/0 + architect independent Test-6 replication (3 PASS + negative-control FAIL = discriminating) + bash -n + go build/vet. Non-gating for the rc.04 re-run (catch-all fires only on an impossible rc; Test 6 is a self-test) — doesn't touch the 78e770ac-pinned image. Closes the STATBUS-078 gate-hardening follow-up; the whole from_commit_sha arc (077 removal + 078 gate + 079 hardening) landed with zero FORCE=1/--no-verify.
<!-- SECTION:NOTES:END -->
