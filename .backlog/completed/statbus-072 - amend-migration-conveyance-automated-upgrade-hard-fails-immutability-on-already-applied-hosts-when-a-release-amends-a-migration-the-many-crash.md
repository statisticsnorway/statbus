...
doc-014 §7 DECIDED (foreman, 2026-06-18): KEEP STATBUS_CIRCUMVENT_IMMUTABLE_MIGRATION as a DOCUMENTED local-dev override, union'd with the committed amendments file — a dev affordance (iterate on an amendment pre-commit), NOT a back-compat shim. Production leaves it unset → file-only conveyance. Matches the doc-014 recommendation. doc-014 is FINAL (incl. §2a: version-is-only-load-bearing-field + append-only/never-prune ledger). Engineer is implementing per the live doc; architect to review the impl diff hands-on when it lands.

DONE 2026-06-18 — committed 24907e2f8 (6 files). Auto-conveyance of in-place migration amendments via a committed migrations/amendments.tsv declaration (version load-bearing; amending_release+reason = audit; append-only never-prune ledger; crash-fix-only convention; no checker — deliberateness preserved). release.CircumventVersions = ParseAmendmentsFile(committed) UNION ParseCircumventVersions(env). ALL THREE immutability-gate readers now route through it = single source of truth (foreman-verified: `rg ParseCircumventVersions\(os.Getenv cli/` returns exactly ONE hit, inside CircumventVersions): migrate.eagerContentHashCheck (runtime per-host), release.checkMigrationImmutability (pre-create RC-cut), release_verify.compareMigrationsForTag (pre-push validation — the third reader the architect caught; was env-only). env var kept as documented local-dev override (§7 foreman decision). content_hash stays PLAIN (no generated column / no schema change — artifacts_ready precedent honored). 8 unit tests. Re-stamp MECHANISM was already present; only the conveyance was built. END-TO-END amend-upgrade proof rides STATBUS-071's working→working-fixed arc (lands here with unit tests; VM-proven in 071). Design = doc-014 (architect). Architect-reviewed + foreman-verified; go build/vet + release/migrate/cmd suites green.
<!-- SECTION:NOTES:END -->
