...
---
WRITER CONVICTED — code-first, no new run needed (architect, 2026-07-11). The invisible writer is markCurrentVersionCompleted (service.go:2922, UPDATE at :2946): `WHERE commit_sha = $1 AND completed_at IS NULL` — no state guard, no parked guard. After the swap the running binary IS the target commit, so the parked row matches; the gate (verifyUpgradeObservedState) checks binary-vs-HEAD + migrations only — exactly what the health-park arc leaves intact — so it passes while the app cannot serve. It was in the wave-7 journal all along: tmp/wave7-healthpark-job.log:5237 'Marked current version (e255b562) as completed' at 02:15:18 (pid 35719, the FIRST parked-skip boot, 57s after the 02:14:21 park). The grep missed it because this site logs a plain Printf, not a logUpgradeRow marker. Full timeline: 02:14:21 park lands (in_progress+parked) → 02:14:52 parked-skip boot → 02:15:18 completed-write (parked_at untouched) → later boots still read PARKED off parked_at → 02:16:25 un-park clears parked_at (state stays completed) → 02:17:32 re-park guard state='in_progress' matches 0 rows → 'not parkable' → 02:17:35 red; the assert line itself printed 'post-unpark row: completed|1|f|'. The latent finding is not a sibling — completion-over-parked leaving completed+parked IS this event. Root statement: one boot reconciler silently overrode both its sibling's explicit deferral (completeInProgressUpgrade logged 'NOT self-healing; deferring' 26s earlier) and the park's judgment.
---
