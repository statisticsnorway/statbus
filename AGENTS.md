...
- **Dev** — the automatic canary (STATBUS-247): every candidate reaches it without a human. *Transitional note:* until STATBUS-247's tag-driven deploy lands, `master-to-dev` and its deploy branch (`ops/cloud/deploy/dev`) are the one deliberate exception still in place — STATBUS-244b removes that button once 247 supplies its replacement. See `deploy-to-dev.yaml`. If a candidate wrecks dev beyond what rollback/park/un-park repairs, `ops/reset-statbus-slot.sh <slot> <wrecking-version>` (STATBUS-250) wipes and reinstalls it at the previous stable release and dismisses the wrecking candidate so it is never re-offered.
