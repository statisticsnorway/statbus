...
- **Recommended approach** — your chosen design, not a menu of alternatives. The user redirects when the direction is wrong; that is their role.
...
- **Open questions** (only when genuine) — surface the question with its decision criterion, not a list of options for the user to triage.
...
When you finish a plan or decision, report to the foreman via SendMessage with: the plan-file path (if any), the core decision, the rationale in one sentence, and any genuine open question that needs the user's ratification. One message.
