...
export const NavigationManager = () => {
  ...
  useGuardedEffect(
    () => {
      ...
      const debug = isInspectorVisible;
      ...
      if (
        ...
        setTimeout(() => {
          if (debug) {
            console.log("[performSideEffects] CALLING router.push NOW", {
              targetPath,
            });
          }
          try {
            ...
            if (debug) {
              console.log(
                "[performSideEffects] router.push returned (no error thrown)"
              );
            }
          } catch (err) {
        ...
        console.warn(
          "[performSideEffects] HARD NAVIGATE fallback after router.push timeout",
          { targetPath, pathname }
        );
        ...
        if (pathname !== "/login") {
          if (debug) {
            console.log("[performSideEffects] EXECUTING savePath", {
              fullPath,
            });
          }
        ...
        console.log(
          "[performSideEffects] sideEffect NOT executed as navigate",
          {
            action,
            targetPath,
            pathname,
            reason: !action
              ? "no action"
              : action !== "navigateAndSaveJournal"
                ? "not navigateAndSaveJournal action"
                : !targetPath
                  ? "no targetPath"
                  : targetPath === pathname
                    ? "targetPath === pathname (already there)"
                    : "unknown",
          }
        );
      }
  ...
  useGuardedEffect(
    ...
    [pathname, sideEffectStartTime, sideEffectStartPathname],
    "NavigationManager:syncPollingState"
  );
  ...
  useGuardedEffect(
    () => {
      ...
      const checkNavigation = () => {
        ...
        if (startPathname && currentPathname !== startPathname) {
          ...
          send({
            type: "CLEAR_SIDE_EFFECT",
            reason: "polling_detected_completion",
          });
          return; // Stop polling
        }
        ...
        if (startTime && Date.now() - startTime > TIMEOUT_MS) {
          console.error(
            "[Polling] Navigation TIMEOUT (too slow) - clearing sideEffect",
            {
    ...
    [sideEffect, sideEffectStartPathname, pathname, send, isInspectorVisible],
    "NavigationManager:pollingDetection"
  );
