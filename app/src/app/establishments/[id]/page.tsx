...
export default async function EstablishmentGeneralInfoPage({params: {id}}: { readonly params: { id: string } }) {
    ...
    async function setPrimary(id: number) {
        ...
        const {error} = await client.rpc('set_primary_establishment_for_legal_unit', {establishment_id: id})
    ...
    return (
        <DetailsPage title="General Info" subtitle="General information such as name, sector">
            <p className="p-12 text-sm text-center">
...

            {
                establishment.primary_for_legal_unit && (
                    <InfoBox>
                        <p>
                            This is the primary establishment. Changes you make to this establishment will affect the
                            legal unit.
                        </p>
                    </InfoBox>
                )
            ...
            {
                !establishment.primary_for_legal_unit && (
                    <form action={setPrimary.bind(null, establishment.id)} className="p-2 bg-gray-100">
                        <Button type="submit" variant="outline">Set as primary establishment</Button>
                    </form>
                )
