...
export function getDbHostPort() {
  ...
  const parsePort = (portStr: string | undefined, varName: string): number => {
    ...
    if (isNaN(portNum) || portNum <= 0 || portNum > 65535) {
      throw new Error(
        `Invalid port number configured for ${varName}: ${portStr}`
      );
    }
    return portNum;
  };
  ...
  if (localPortStr) {
    ...
    if (!/^\d+$/.test(localPortStr)) {
      throw new Error(
        `Invalid format for environment variable CADDY_DB_PORT: ${localPortStr}. Must be a number.`
      );
    }
    dbHost = "localhost";
...
const channelCallbacks: ChannelSubscribers =
  globalThis.__dbListenerCallbacks || new Map();
...
let reconnectTimeout: NodeJS.Timeout | null =
  globalThis.__dbListenerReconnectTimeout || null;
...
let reconnectDelay: number =
  globalThis.__dbListenerReconnectDelay || INITIAL_RECONNECT_DELAY_MS;

// State for debouncing notifications
const debounceTimers: { [key: string]: NodeJS.Timeout } =
  globalThis.__dbListenerDebounceTimers || {};
if (typeof globalThis !== "undefined") {
...
function handleImportJobProgressNotification(
...
function handleWorkerTaskChangedNotification(rawPayload: string) {
  try {
    ...
    debounceTimers[debounceKey] = setTimeout(() => {
      ...
      channelCallbacks
        .get("worker_task_changed")
        ?.forEach((callback) => callback(data));
      delete debounceTimers[debounceKey];
    }, WORKER_TASK_DEBOUNCE_MS);
  } catch (e) {
    console.error(
      "DB Listener: Error parsing worker_task_changed notification:",
      e
    );
  }
}
...
async function connectAndListen() {
  ...
  try {
      ...
      if (msg.channel === "worker_status") {
        try {
          ...
          if (payload.type === "upgrade_changed") {
            console.log(
              `DB Listener: upgrade_changed notification received, broadcasting to ${channelCallbacks.get("worker_status")?.size ?? 0} SSE client(s)`
            );
          }
          if (process.env.NODE_ENV === "development") {
            console.log(
              `DB Listener: Received worker_status notification:`,
              payload
            );
          }
        } catch (e) {
          console.error(
            { error: e, payload: msg.payload },
            "DB Listener: Error parsing worker_status notification"
          );
        }
        ...
        try {
          ...
          console.error(
            { error: e, payload: msg.payload },
            "DB Listener: Error parsing import_job_progress notification"
          );
        }
    ...
    newClient.on("error", (err) => {
      console.error(
        { error: err, dbHost, dbPort },
        "DB Listener: Connection error"
      );
    ...
    console.log(
      `DB Listener: Connected to ${dbHost}:${dbPort}/${dbName} as ${dbUser} and listening on "worker_status", "import_job", "import_job_progress", and "worker_task_changed" channels`
    );
    ...
    console.error(
      { error, dbHost, dbPort },
      "DB Listener: Failed to connect or listen"
    );
...
function scheduleReconnect() {
  ...
  if (!reconnectTimeout) {
    ...
    console.log(
      `DB Listener: Reconnection scheduled in ${reconnectDelay / 1000} seconds`
    );
...
async function handleImportJobNotification(payload: string) {
  try {
    const minimalPayload = JSON.parse(
      payload
    ) as MinimalImportJobNotificationPayload;
    ...
    if (verb === "DELETE") {
      ...
      if (!pgClient) {
        console.error(
          "DB Listener: pgClient is not available for enriching import_job notification."
        );
        return;
      }
      ...
      if (process.env.NODE_ENV === "development") {
        console.log(
          `DB Listener: Enriching notification for job ${id} using raw pgClient.`
        );
      }
      ...
      if (rows.length === 0) {
        console.warn(
          `DB Listener: No import_job found for id ${id} during enrichment. It may have been deleted.`
        );
        return;
      }
    ...
    if (process.env.NODE_ENV === "development") {
      console.log(
        `DB Listener: Enriched and sent import_job notification for job ${id} (verb: ${verb})`
      );
    }
  } catch (e) {
    console.error(
      { error: e, payload: payload },
      "DB Listener: Error processing or enriching import_job notification"
    );
  }
}
  ...
  if (pgClient) {
    pgClient
      ...
      .catch((err: Error) =>
        console.error(
          { error: err },
          "DB Listener: Error during connection end"
        )
      ); // Add type annotation
 ...
 */
export function addClientCallback(
  channel: string,
 ...
 */
export function removeClientCallback(
  channel: string,
  callback: NotificationCallback
) {
...
export async function initializeDbListener() {
  ...
  const activeCallbacks = Array.from(channelCallbacks.values()).reduce(
