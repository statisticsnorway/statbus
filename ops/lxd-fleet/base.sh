...
remote "lxc exec '$name' -- sudo -i -u statbus bash -lc 'cd ~/statbus && test \"\$(git describe --tags --exact-match)\" = $tag && env STATBUS_INSTALL_VERSION=$tag STATBUS_MIN_DISK_GB=5 bash ./install.sh --non-interactive --trust-github-user jhf'"
