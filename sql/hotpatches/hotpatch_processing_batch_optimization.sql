-- HOT-PATCH: Optimize processing phase batch selection query
-- This changes ORDER BY row_id to ORDER BY state, action, row_id for better index usage
-- Expected improvement: Better index utilization for processing batch selection
-- Date: 2026-01-23
-- Issue: Processing batch selection doesn't efficiently use existing indexes

BEGIN;

-- Drop and recreate the processing phase function with optimized ORDER BY
...
BEGIN
    ...
    -- HOT-PATCH OPTIMIZATION: Changed ORDER BY to better utilize indexes
    EXECUTE format(
        $$
        WITH batch_rows AS (
            SELECT row_id
...
-- Create optimized index for processing phase if it doesn't exist
CREATE INDEX IF NOT EXISTS import_hovedenhet_2025_data_processing_idx 
ON import_hovedenhet_2025_data (state, action, row_id) 
WHERE state = 'processing' AND action = 'use';

SELECT 'HOT-PATCH APPLIED: Processing batch selection optimization' as status;