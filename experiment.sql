...
DROP VIEW IF EXISTS public.timeline_legal_unit_optimised;
CREATE OR REPLACE VIEW public.timeline_legal_unit_optimised
( unit_type
, unit_id
, valid_after
, valid_from
...
AS
  WITH relevant_timesegments AS (
    SELECT * FROM public.timesegments WHERE unit_type = 'legal_unit'
  ), basis AS (
  SELECT t.unit_type
       , t.unit_id
       , t.valid_after
  ...
  --
  FROM relevant_timesegments AS t
  ...
  LEFT JOIN LATERAL (
    ...
    SELECT tes.legal_unit_id
         , basis.valid_after
         , basis.valid_to
         , ARRAY[]::INTEGER[] AS data_source_ids
         , ARRAY[]::TEXT[] AS data_source_codes
...
--\o tmp/timeline_legal_unit-analyze.txt
--EXPLAIN ANALYZE SELECT * FROM public.timeline_legal_unit;
--\o

\o tmp/timeline_legal_unit_optimised-analyze.txt
EXPLAIN ANALYZE SELECT * FROM public.timeline_legal_unit_optimised;
\o
