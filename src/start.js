/* eslint-disable no-console, global-require, import/newline-after-import */
const fs = require('fs')
...
tasks.set(
  ...
  () => process.argv.includes('--packages')
    ? Promise.resolve().then(() => del(['node_modules/**']))
    : Promise.resolve()
...
// Build and publish web application to remote
tasks.set(
  'publish',
  () => {
    global.DEBUG = process.argv.includes('--debug') || false
...
// Execute the specified task or default one. E.g.: node start build
