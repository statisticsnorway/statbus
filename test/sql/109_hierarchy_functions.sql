...
SELECT row_id, errors, warnings, merge_status FROM public.import_34_lu_era_data WHERE errors IS NOT NULL OR warnings IS NOT NULL ORDER BY row_id;

\echo "Checking for data processing issues in import_34_esflu_era_data"
SELECT row_id, errors, warnings, merge_status FROM public.import_34_esflu_era_data WHERE errors IS NOT NULL OR warnings IS NOT NULL ORDER BY row_id;

\echo "Checking for data processing issues in import_34_eswlu_era_data"
SELECT row_id, errors, warnings, merge_status FROM public.import_34_eswlu_era_data WHERE errors IS NOT NULL OR warnings IS NOT NULL ORDER BY row_id;
