...
DECLARE
    ...
    v_helper_patterns JSONB := '[
        ...
        {"pattern": "%_active", "description": "Views that filter for only the `active` records in a classification table."},
    ...
    SELECT * FROM (VALUES
        ...
        (3, 2, 5, 'General Code/Classification Tables', 'These tables typically store codes, names, and flags for `custom` and `active` status.', jsonb_build_array(
        ...
        (6, 1, 1, 'Worker System', 'Handles background processing. A long-running worker process calls `worker.process_tasks()` to process tasks synchronously.', jsonb_build_array(
            ...
            '{"schema": "worker", "name": "last_processed"}'::jsonb
        )),
            ...
            FROM (
                SELECT
                    ...
                    CASE
                        ...
                        WHEN column_name = 'active' THEN 5
                        ELSE 6
                    END as priority
