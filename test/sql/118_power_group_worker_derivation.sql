...
INSERT INTO public.legal_rel_type (code, name, description, enabled, custom)
SELECT 'ownership', 'Ownership', 'Direct share ownership percentage', true, false
WHERE NOT EXISTS (SELECT 1 FROM public.legal_rel_type WHERE code = 'ownership');

INSERT INTO public.legal_rel_type (code, name, description, enabled, custom)
SELECT 'control', 'Control', 'Controlling interest (may differ from ownership via voting rights or agreements)', true, false
...
\echo "Create non-controlling ownership: MinorInvestor owns 30% of Independent"
INSERT INTO public.legal_relationship (
    valid_from, 
    influencing_id, 
    influenced_id, 
...
SELECT 
    ...
    (SELECT id FROM public.legal_rel_type WHERE code = 'ownership'),
    30.00,  -- Less than 50%, so not controlling
    (SELECT id FROM auth.user LIMIT 1),
    'MinorInvestor owns 30% of Independent (non-controlling)';

\echo "Re-run derive_power_groups"
SELECT worker.derive_power_groups();

\echo "Non-controlling relationship should have NULL power_group_id"
SELECT 
    influencer.name AS influencing_name,
    influenced.name AS influenced_name,
