...
BEGIN
    ...
    -- Perform login
    SELECT to_json(source.*) INTO login_result FROM public.login('test.regular@example.com', 'regular123') AS source;
    ...
    ASSERT auth_status_result->>'email' = 'test.regular@example.com', 'Auth status should have correct email';
...
BEGIN
    ...
    SELECT to_json(source.*) INTO login_result FROM public.login('test.regular@example.com', 'wrongpassword') AS source;
    
    -- Debug the login result
    RAISE DEBUG 'Login result (should be null): %', login_result;
    
    -- Verify login failed (result should be null)
    ASSERT login_result ->> 'user_id' IS NULL, 'Login with wrong password should return null';
...
BEGIN
    ...
    SELECT to_json(source.*) INTO login_result FROM public.login('test.unconfirmed@example.com', 'unconfirmed123') AS source;
    
    -- Debug the login result
    RAISE DEBUG 'Login result (should be null): %', login_result;
    
    -- Verify login failed (result should be null)
    ASSERT login_result ->> 'user_id' IS NULL, 'Login with unconfirmed email should return null';
...
BEGIN
    -- Reset all context from previous tests
    ...
    -- First login
    SELECT to_json(source.*) INTO login_result FROM public.login('test.admin@example.com', 'admin123') AS source;
    RAISE DEBUG 'Login result: %', login_result;
    ...
    BEGIN
        ...
        ASSERT refresh_jwt_payload->>'role' = 'test.admin@example.com', 'Role in refresh token should match email';
        ASSERT refresh_jwt_payload->>'email' = 'test.admin@example.com', 'Email in refresh token should be correct';
    ...
    ASSERT auth_status_before->>'email' = 'test.admin@example.com', 'Auth status should have correct email';
        
    -- Clear any existing cookies and headers before setting new ones
    PERFORM set_config('request.cookies', '{}', true);
    PERFORM set_config('request.headers', '{}', true);
    PERFORM set_config('response.headers', '[]', true);
    ...
    ASSERT auth_status_after->>'is_authenticated' = 'true', 'Auth status should show authenticated';
    ASSERT auth_status_after->'uid' IS NOT NULL, 'Auth status should include user info';
    ASSERT auth_status_after->>'email' = 'test.admin@example.com', 'Auth status should have correct email';
...
BEGIN
    ...
    SELECT to_json(source.*) INTO login_result FROM public.login('test.restricted@example.com', 'restricted123') AS source;
    RAISE DEBUG 'Login result: %', login_result;
    ...
    ASSERT auth_status_before->>'is_authenticated' = 'true', 'Auth status should show authenticated';
    ASSERT auth_status_before->'uid' IS NOT NULL, 'Auth status should include user info';
    ASSERT auth_status_before->>'email' = 'test.restricted@example.com', 'Auth status should have correct email';
...
BEGIN
    ...
    SELECT to_json(source.*) INTO login_result_admin FROM public.login(admin_email, 'admin123') AS source;
    ...
    SELECT to_json(source.*) INTO login_result_regular FROM public.login(regular_email, 'regular123') AS source;
