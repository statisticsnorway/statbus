            ...
            using (var context = CreateDbContext())
            {
                ...
                switch (type)
                {
                    case StatUnitTypes.LegalUnit:
                        ...
                        Assert.IsType<LegalUnitHistory>(
                            context.LegalUnitHistory.Single(x => x.Name == unitName && !x.IsDeleted && x.ParentId == unitId));
                        break;
                    case StatUnitTypes.LocalUnit:
                        ...
                        Assert.IsType<LocalUnitHistory>(
                            context.LocalUnitHistory.Single(x => x.Name == unitName && !x.IsDeleted && x.ParentId == unitId));
                        break;
                    case StatUnitTypes.EnterpriseUnit:
