...
# arc_install_kill_dropin <inject_class> [restart_sec] — the KILL-via-dropin variant
# (4th sub-variant: daemon-RUN + a dropin KILL, no stall file). Installs a systemd
# USER dropin arming a KILL (Environment=STATBUS_INJECT_AT=<kill-class>) + a
# RestartSec + a short TimeoutStopSec, then STARTS the unit (--no-block — the unit
# resumes, the kill fires [137], systemd restarts it; it is in flux, so do NOT
# wait-active). The dropin env survives the upgrade's exit-42 syscall.Exec re-exec
# (it is the unit's Environment, inherited across exec) → the kill lands at the
# post-exit-42 resume. Used by resume-died-rollback (death-during-resume → the
# Resuming one-shot latch must roll back, NOT loop) + rollback-restore-watchdog's
# RUN2. Same inject.conf filename as arc_install_stall_dropin → a later swap to the
# stall dropin overwrites cleanly (rollback-restore-watchdog RUN2→RUN3).
arc_install_kill_dropin() {
    local inject_class="$1" restart_sec="${2:-10}"
    echo ""
    echo "── install KILL dropin (STATBUS_INJECT_AT=${inject_class}, RestartSec=${restart_sec}s) + start unit ──"
    VM_EXEC systemctl --user stop "$ARC_UPGRADE_UNIT" 2>/dev/null || true
    local script
    script=$(mktemp /tmp/arc-kill-dropin-XXXXXX.sh)
    cat > "$script" << SCRIPT_EOF
#!/bin/bash
set -euo pipefail
DROPIN_DIR="\$HOME/.config/systemd/user/${ARC_UPGRADE_UNIT}.d"
...
SCRIPT_EOF
    chmod 644 "$script"
    upload_install_script_to_vm "$VM_NAME" "$script" /tmp/arc-kill-dropin.sh
    rm -f "$script"
    VM_EXEC bash /tmp/arc-kill-dropin.sh
    # --no-block: the unit resumes the PostSwap wedge, kills itself (137), systemd
    # restarts it into recoverFromFlag Resuming → rollback. It is NOT cleanly active.
    VM_EXEC bash -c "systemctl --user --no-block start $ARC_UPGRADE_UNIT"
    echo "  ✓ KILL dropin installed + unit started (--no-block; the kill fires in the resumed upgrade)"
}

# arc_remove_dropin — remove the inject dropin + daemon-reload + restart the unit
# (the NO-LOOP cleanup: confirm the unit settles once the inject is gone).
arc_remove_dropin() {
    VM_EXEC bash -c "rm -f ~/.config/systemd/user/${ARC_UPGRADE_UNIT}.d/inject.conf; systemctl --user daemon-reload; systemctl --user --no-block restart $ARC_UPGRADE_UNIT 2>/dev/null || true" 2>/dev/null || true
}
