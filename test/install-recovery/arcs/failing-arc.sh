...
_dump_failing_failure_diagnostics() {
    echo "" >&2
    echo "══════════ failure diagnostics (daemon journal + flag + B/C's progress log + row state) ══════════" >&2
    ...
    VM_EXEC bash -c "cd ~/statbus && echo \"SELECT id, state, recovery_attempts, recovery_parked_at IS NOT NULL AS parked, COALESCE(recovery_parked_reason,''), error FROM public.upgrade WHERE commit_sha IN ('${B_FULL:-}', '${C_FULL:-}') ORDER BY id;\" | ./sb psql" >&2 || echo "  (could not query row state — VM/DB unreachable)" >&2
