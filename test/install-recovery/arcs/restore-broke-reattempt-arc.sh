...
_dump_restore_broke_reattempt_failure_diagnostics() {
    echo "" >&2
    echo "══════════ failure diagnostics (daemon journal + flag + latest progress log + row state) ══════════" >&2
    ...
    echo "── row state at exit (B=${B_FULL:-?}, C=${C_FULL:-?}) ──" >&2
    VM_EXEC bash -c "cd ~/statbus && echo \"SELECT id, state, recovery_attempts, backup_path IS NOT NULL AS has_backup_path, error FROM public.upgrade WHERE commit_sha IN ('${B_FULL:-}', '${C_FULL:-}') ORDER BY id;\" | ./sb psql" >&2 || echo "  (could not query row state — VM/DB unreachable)" >&2
...
#      StartDatabaseRouteServingMayRun (existing containers, install_upgrade.go) →
