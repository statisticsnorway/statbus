...
# _dump_between_migrations_failure_diagnostics — STATBUS-155 rider (mirrors
...
# row state to STDERR BEFORE the kill marker is removed and cleanup_vm reaps
# the VM, so a red run is self-sufficient without needing a kept VM.
# Best-effort throughout (|| true) — a diagnostics failure must never mask
# the real assertion error that triggered this trap.
_dump_between_migrations_failure_diagnostics() {
    echo "" >&2
    echo "══════════ failure diagnostics (daemon journal + flag + B's progress log + row state) ══════════" >&2
    # STATBUS-296: SSH-only captures first — db-dependent captures (log_rel
    # query, row state) last and individually guarded, so a down/unreachable
    # db reports itself as a datum instead of aborting the rest under this
    # file's set -e.
...
trap 'rc=$?; if [ "$rc" -ne 0 ]; then _dump_between_migrations_failure_diagnostics; fi; VM_EXEC bash -c "rm -f $KILL_MARKER 2>/dev/null" 2>/dev/null || true; cleanup_vm "$VM_NAME"; exit $rc' EXIT
