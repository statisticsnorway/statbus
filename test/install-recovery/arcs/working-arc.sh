...
_dump_working_failure_diagnostics() {
    ...
    # STATBUS-296: SSH-only captures first (journal, flag) — db-dependent
    # captures (log_rel query, row state) last and individually guarded, so a
    # down/unreachable db reports itself as a datum instead of aborting the
