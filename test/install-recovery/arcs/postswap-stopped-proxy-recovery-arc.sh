...
# stopped proxy, and StartDatabaseRouteServingMayRun's asymmetric-safe start now covers
...
_dump_stopped_proxy_failure_diagnostics() {
    echo "" >&2
    echo "══════════ failure diagnostics (daemon journal + flag + container ps + B's progress log + row state) ══════════" >&2
    ...
    # file's set -e. `docker compose ps -a` needs only docker/SSH, not the db,
    # so it stays in the SSH-only-first group alongside the journal.
    ...
    VM_EXEC bash -c "cd ~/statbus && docker compose ps -a" >&2 || echo "  (could not read container state)" >&2
    local log_rel
...
[ "$PROXY_AFTER" = "running" ] || { echo "✗ expected proxy running after recovery, got '$PROXY_AFTER' — StartDatabaseRouteServingMayRun must resume the whole route, not just the db" >&2; exit 1; }
