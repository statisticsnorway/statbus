...
# _dump_preswap_backup_kill_failure_diagnostics — STATBUS-155 rider (mirrors
...
# triggered this trap.
_dump_preswap_backup_kill_failure_diagnostics() {
    echo "" >&2
...
trap 'rc=$?; if [ "$rc" -ne 0 ]; then _dump_preswap_backup_kill_failure_diagnostics; fi; cleanup_vm "$VM_NAME"; exit $rc' EXIT
