...
| `4-rollback-abort-churn-then-alive-idle` *(interim net, STATBUS-071 comment #17)* | Reuses the (former) ABORT construction but WITHOUT the cleanup step, so a second, floor-bound deterministically-failing migration survives the terminal write: the next FLAGLESS boot's boot-migrate hits it and fails deterministically (exit 20) — but instead of pre-144's restart-to-`StartLimit` death churn, the daemon logs "BOOT MIGRATE FAILED DETERMINISTICALLY" once and continues alive-idle (no further exit/restart, row stays `failed`, other services keep serving). Stands alone, uncoupled from the ABORT scenario's retirement (it tested a disjoint property the arc never touches). Interim net only: its state has a LIVE producer (STATBUS-144 was found from a real occurrence, not invented for this test), so it too owes a real-path successor, queued as its own future map row; deletes when that successor goes green. | STATBUS-144 AC#3 (flagless post-terminal boot-migrate churn → alive-idle fix, shipped 46f979a3a); STATBUS-071 comments #16/#17 (dead-producer doctrine, interim-net framing) — **[UNPROVEN — no dedicated arc-harness run tracked here yet; existing scenario run is its only proof]** |
