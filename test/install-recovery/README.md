...
| §4a FIX A — archiveBackup after the terminal UPDATE + removeUpgradeFlag | 27 (without it, a start-phase kill during the resume's tar leaves the row `in_progress` → loop) + the local Go guard `TestArchiveBackupAfterTerminalUpdate` |
| §4a FIX B1 — READY=1 before recoverFromFlag (resume runs active-phase) | 27 (without it, the resume's archiveBackup runs in the start phase under TimeoutStartSec) + the local Go guard `TestReadyEmittedBeforeRecoverFromFlag`; #18 still validates the static pre-READY start-budget contract for the cheap init |
