...
| Scenario | What it proves | Grounding |
|---|---|---|
| `3-postswap-migrate-killed-after-commit` *(RETIRED — superseded by the real register+schedule producers, STATBUS-071 P2)* | **RETIRED (file deleted; set-difference recorded on STATBUS-071).** The fabricated committed-but-unrecorded after-commit cell (STATBUS-017 rune-wedge fix regression guard) is now driven for real by TWO arcs covering its two distinct kill sites: `postswap-after-commit-kill-arc` (the daemon PARENT killed mid-stall — the boot-time `recoverFromFlag`/Resuming-latch recovery path) and `after-commit-before-recorded-kill-arc` (the migrate SUBPROCESS killed, parent survives — Layer 0 in-process `postSwapFailure` recovery, no boot required). Set-difference: `assert_upgrade_row_state`/`rolled_back`, `assert_flag_file_absent`, `assert_systemd_restart_counter_bounded`, `assert_health_passes`, and the orphan/ledger halves of `_assert_faithful_restore` all carry over (with bounds recalibrated to each arc's own real construction). The `UPGRADE_DIED_DURING_RESUME` error-match — specific to the boot-time Resuming-latch path the SUBPROCESS arc never takes — is carried instead by the sibling `postswap-container-restart-kill-arc` (asserts the same `ErrResumeDied` constant via its own parent-kill-then-restart construction). One sub-check named ILLUSORY and dropped: "synthetic migration file removed from disk" existed only because the fabrication itself committed a foreign git file mid-scenario; the real arcs' `V_VERSION` is a standing, already-committed working-lineage migration, so no analogous file-survival risk exists to check. | StallHere primitives; STATBUS-017 (cell c) |
...
| `3-postswap-rune-wedge` *(RETIRED — HISTORICAL, dead-producer class extinct; STATBUS-071 P5)* | **HISTORICAL (file deleted).** The rune shape's producer — the Apr-24 SDNOTIFY-collision-during-active-upgrade bug — is now DEAD: fixed by `e76505eec` (skips `--now` in the fixup during an active upgrade), `61e79e265` (lands the terminal UPDATE before the fixup), and `cb7344dd6` (adds the self-heal net). Under the King's dead-producer doctrine (comment #12) an extinct state is no longer a real operation to fabricate on cue; the LIVE code this scenario exercised (`resumeNewSb`'s containers-at-target self-heal) is now covered instead by the `postswap-converged-selfheal` arc (a REAL working-lineage upgrade converged, crashed at the converged-but-unlanded instant, self-healed to `completed` with `[completed-self-heal]` logged — first run in diagnosis; this retirement does not wait on it, per the architect's explicit sequencing). `fabricate_resume_state` reaches ZERO callers with this retirement and is DELETED — no sanctioned fabrication exceptions remain. | STATBUS-039 takeover + forward-when-at-target (historical); STATBUS-052 flock-confirmed quiesce (historical); STATBUS-044 AC#1 case 0 (historical); STATBUS-047 (the one-shot live rune recovery this scenario stood as the regression net for). Retired per STATBUS-071 comment #12 (dead-producer carve-out) and P5 (root-cause fixes cited above). |
...
> STATBUS-134's 2-consecutive-rollback-deaths restore-broke terminal is exercised by the **upgrade-arc-harness** arc `rollback-pair-terminal` (deterministic PreSwap-wedge entry — the same C5→C9 lineage as the `rollback-kill-arc` that reshaped the retired `4-rollback-kill` scenario — with a SECOND, re-armed C9 kill added so the on-disk `(rollback, rollback)` step pair forms and `rollbackResumeIsTerminal` fires `state=failed` before any third restore attempt), **not** an install-recovery scenario: it needs the real register+schedule lineage `rollback-kill-arc.sh` already proved deterministic. Deliberately a SEPARATE arc from the (now-retired) `4-rollback-abort-write-lands`'s ABORT construction above — STATBUS-136's DB-start-before-write fix lives exclusively in the git-corrupt ABORT branch, structurally disjoint from this pair-terminal's (non-abort) kill site; one fabrication cannot exercise both (verified against shipped code, architect-ruled 2026-07-06).

> STATBUS-111's own arc-proof obligation (AC#1/#5, moved to STATBUS-071's coverage map at STATBUS-111's close) is exercised by the **upgrade-arc-harness** arc `restore-broke-reattempt`: the operator's `./sb install` re-attempt of a restore-broke row (`state='failed'` with a retained `backup_path`), driven on BOTH row shapes the STATBUS-111 probe can match — (i) the `rollback-pair-terminal` construction above, re-attempted to a real, byte-identical `rolled_back`; (ii) a REAL git-corrupt ABORT row (a real dispatch killed post-swap before its migration runs, then the pinned `pre-upgrade` branch is deleted for real on the VM — genuine machinery-state manipulation, not fabrication), whose re-attempt must REFUSE actionably (`ErrRollbackGitCorrupt`) before any destructive step. Also folds the former `4-rollback-abort-write-lands`'s own oracle (the ABORT terminal write lands in one pass) as a live proof rather than a fabrication — **[PROVEN] run 29344519124**. That scenario's ABORT half retired on this green run (the r19 pattern); its OTHER half (the flagless self-heal) was NOT provable by this arc and survives, narrowed + renamed, as `4-flagless-selfheal-at-target` (STATBUS-071 comments #16/#17 — the map's original single-deletion note was incomplete, corrected there).
