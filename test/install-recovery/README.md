...
./dev.sh test-install-recovery 09                 # by number prefix
./dev.sh test-install-recovery bool-text          # by slug fragment
./dev.sh test-install-recovery --list             # show available
./dev.sh test-install-recovery --keep-vm 09       # leave VM alive on failure (€0.17/day if forgotten)
```
...
| §4a FIX A — archiveBackup after the terminal `state='completed'` UPDATE + removeUpgradeFlag | 27 (without it, a start-phase kill during the resume's tar cancels the DB context before the terminal UPDATE persists → row stuck `in_progress` → loop) + the local Go guard `TestArchiveBackupAfterTerminalUpdate` |
| #4 unit-reconcile — `checkServiceDone` byte-compares the on-disk unit to the repo template; drift ⇒ rewrite + daemon-reload + restart | 28 (without it, a drifted unit on a healthy box is never rewritten — rune's stale 90/infinity persists) + the local Go guards `TestUnitFileMatchesRepo_*` (drift detection) + `TestRunInstallService_RestartsOnDriftToArmTimers` (re-arm) |
