...
| `3-postswap-resume-died-parked` | Killed twice at the SAME step (migrate-up) across two crash-resumes → the death budget PARKS the upgrade (alive-idle, NRestarts bounded+frozen, siren fires exactly once, never `rolled_back`) instead of looping loud forever or rolling back; a deliberate `./sb install` un-parks it for exactly one fresh attempt, which completes. | STATBUS-046/D3 crash-resume death budget (`resumeEscalation`, same-step-twice); STATBUS-044 park-scenario spec. Renamed/rebuilt from the deleted `3-postswap-resume-died-rollback` (STATBUS-099) — that shape predates the death budget and asserted rollback, which the new mechanism no longer produces at-target. |
