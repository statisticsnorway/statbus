...
for YEAR in $YEARS; do
    ...
    $WORKSPACE/sb psql -c "\copy public.import_hovedenhet_${YEAR}_small_history_upload FROM 'samples/norway/small-history/${YEAR}-enheter.csv' WITH CSV HEADER;"

    # Load underenhet (establishments) data
    echo "Loading underenhet data for $YEAR"
    $WORKSPACE/sb psql -c "\copy public.import_underenhet_${YEAR}_small_history_upload FROM 'samples/norway/small-history/${YEAR}-underenheter.csv' WITH CSV HEADER;"
done

#echo "Running worker processing to process import jobs"
#$WORKSPACE/sb psql -c "SELECT worker.process_tasks();"

echo "Checking import job states"
$WORKSPACE/sb psql -c "SELECT slug, state FROM public.import_job WHERE slug LIKE '%small_history' ORDER BY slug;"
