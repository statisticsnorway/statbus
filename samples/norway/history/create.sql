...
-- \copy (SELECT * FROM brreg.enhet WHERE "organisasjonsnummer" IN (SELECT enhet_orgnr FROM gh.selection ORDER BY random LIMIT 5000)) TO 'samples/norway/legal_unit/enheter-selection.csv' WITH (HEADER true, FORMAT csv, DELIMITER ',', QUOTE '"', FORCE_QUOTE *);
-- \copy (SELECT * FROM brreg.underenhet WHERE "overordnetEnhet" IN (SELECT enhet_orgnr FROM gh.selection ORDER BY random LIMIT 5000)) TO 'samples/norway/establishment/underenheter-selection.csv' WITH (HEADER true, FORMAT csv, DELIMITER ',', QUOTE '"', FORCE_QUOTE *);
