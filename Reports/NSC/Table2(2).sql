...
Select coalesce(Name, 'Total'), isnull([Gov],0),isnull([Mun],0),isnull([Private],0),isnull([Else],0)
	From (
		Select Left(Sect,1) as Sect, PropGrp, count(RegId) qty From ActualStatFinal Where Statuso = 1 Group by Left(Sect,1) , PropGrp with Cube
	) x
	Join ActivityCategories a on x.Sect = a.Code
	Pivot
	(Sum(qty) For PropGrp in ([Gov],[Mun],[Private],[Else])) pvt
