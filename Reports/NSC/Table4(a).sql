...
Select Coalesce(Name, 'Кыргызская Республика'), SizeGrp, [101], [1], [2], [3], [4], [5], [6], [0] 
	From (
		Select reg.Name as Name, SizeGrp, Coalesce(Status, 101) Status, 
		count(RegId) qty,
		Case SizeGrp
		When 'Small' Then 1
		When 'Medium' Then 2
		...
		group by reg.Name, SizeGrp, Status With Cube
	) x 
	Pivot(Sum(qty) For Status in ([101], [1], [2], [3], [4], [5], [6], [0] ))pvt Order by Name, r
