...
----EGRSE U.L(3) PIVOT IS NEEDED	
	
	
Select Coalesce(reg.Name, 'Кыргызская Республика') Region, t.Status stat, count(RegId) qty 
From (
	Select * From ActualStatWithRegions a
	Cross Apply
	(Select ChildId as Region From RegionsFinal Where (Level = 2 and a.Parents Like '%.' + Convert(varchar(6), ChildId) + '.%') or (Level = 2 and LEN(Convert(varchar(6), a.Parents)) = 1)) t1 WHERE LForm != 'Individual' and  LForm != 'SublegalUnit'
) t 
Join Regions reg on reg.Id = t.Region
Join ActivityCategories a on t.Sect = a.Code
Group by reg.Name, t.Status with cube		