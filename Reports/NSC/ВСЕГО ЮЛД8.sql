...
----Total Entities 8, PIVOT IS NEEDED	
	
Select Coalesce(reg.Name, 'Кыргызская Республика') Region, coalesce(a.Name, 'Total'), count(RegId) qty 
From (
	...
	(Select ChildId as Region From RegionsFinal Where (Level = 2 and a.Parents Like '%.' + Convert(varchar(6), ChildId) + '.%') or (Level = 2 and LEN(Convert(varchar(6), a.Parents)) = 1)) t1 
		WHERE Statuso = 1 and LForm != 'Individual' and Legal = 1
) t 
Join Regions reg on reg.Id = t.Region
Join ActivityCategories a on t.Sect = a.Code
Group by reg.Name, a.Name with cube
