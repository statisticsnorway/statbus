...
---Table 1
Select Coalesce(Name, 'Кыргызская Республика'), isnull(SizeGrp,'Total'), Total, ActTotal, Legal, ActLegal, prev as '2016', curr as '2017' 
	From(
		Select rg.Name as Name, SizeGrp, 
		Case SizeGrp
		When 'Small' Then 1
		When 'Medium' Then 2
		When 'Big' Then 3
		...
		count(RegId) Total, Sum(Statuso) ActTotal, Sum(Legal) Legal, Sum(ALegal) ActLegal, (count(RegId) - Sum(y)) prev, count(RegId) curr
		From (
		...
		Join Regions rg on t.Region = rg.Id
		group by rg.Name, SizeGrp With Cube
	) x 
	Order by Name, r
