      ...
      WITH aggregation AS (
        SELECT ten.enterprise_id, ten.valid_from, ten.valid_until
          ...
          FROM (
            ...
            LEFT JOIN LATERAL (SELECT * FROM public.timeline_legal_unit enplu_1 WHERE enplu_1.enterprise_id = ten.enterprise_id AND enplu_1.primary_for_enterprise = true AND from_until_overlaps(ten.valid_from, ten.valid_until, enplu_1.valid_from, enplu_1.valid_until) ORDER BY enplu_1.valid_from DESC, enplu_1.legal_unit_id DESC LIMIT 1) enplu ON true
            LEFT JOIN LATERAL (SELECT * FROM public.timeline_establishment enpes_1 WHERE enpes_1.enterprise_id = ten.enterprise_id AND enpes_1.primary_for_enterprise = true AND from_until_overlaps(ten.valid_from, ten.valid_until, enpes_1.valid_from, enpes_1.valid_until) ORDER BY enpes_1.valid_from DESC, enpes_1.establishment_id DESC LIMIT 1) enpes ON true
...
CREATE INDEX IF NOT EXISTS idx_timeline_enterprise_unit_daterange ON public.timeline_enterprise
    USING gist (daterange(valid_from, valid_until, '[)'), unit_id);
...
DECLARE
    ...
    v_batch_size INT := 32768; -- Reduced batch size for enterprises
