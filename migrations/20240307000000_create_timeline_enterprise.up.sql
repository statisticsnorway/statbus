...
CREATE OR REPLACE VIEW public.timeline_enterprise_def
    ...
    AS
      WITH enterprise_with_primary_legal_unit AS (
      ...
      FROM (
        ...
        LEFT JOIN LATERAL (
          ...
          FROM (VALUES (ten.edit_comment, ten.edit_by_user_id, ten.edit_at), (tlu.last_edit_comment, tlu.last_edit_by_user_id, tlu.last_edit_at)) AS all_edits(edit_comment, edit_by_user_id, edit_at)
          WHERE edit_at IS NOT NULL ORDER BY edit_at DESC LIMIT 1
      ...
      FROM (
        ...
        LEFT JOIN LATERAL (
          SELECT edit_comment, edit_by_user_id, edit_at
          FROM (VALUES (ten.edit_comment, ten.edit_by_user_id, ten.edit_at), (tes.last_edit_comment, tes.last_edit_by_user_id, tes.last_edit_at)) AS all_edits(edit_comment, edit_by_user_id, edit_at)
          WHERE edit_at IS NOT NULL ORDER BY edit_at DESC LIMIT 1
        ) AS last_edit ON TRUE
      ...
      LEFT JOIN LATERAL (
          ...
          WHERE enplu_1.enterprise_id = ten.enterprise_id AND after_to_overlaps(ten.valid_after, ten.valid_to, enplu_1.valid_after, enplu_1.valid_to)
      ...
      LEFT JOIN LATERAL (
          ...
          WHERE enpes_1.enterprise_id = ten.enterprise_id AND after_to_overlaps(ten.valid_after, ten.valid_to, enpes_1.valid_after, enpes_1.valid_to)
          ORDER BY enpes_1.valid_after DESC, enpes_1.primary_establishment_id DESC LIMIT 1
      ) enpes ON true
      LEFT JOIN LATERAL (
        SELECT edit_comment, edit_by_user_id, edit_at
        FROM (VALUES (ten.edit_comment, ten.edit_by_user_id, ten.edit_at), (enplu.last_edit_comment, enplu.last_edit_by_user_id, enplu.last_edit_at), (enpes.last_edit_comment, enpes.last_edit_by_user_id, enpes.last_edit_at)) AS all_edits(edit_comment, edit_by_user_id, edit_at)
        WHERE edit_at IS NOT NULL ORDER BY edit_at DESC LIMIT 1
      ) AS last_edit ON TRUE
      ), aggregation AS (
        SELECT ten.enterprise_id, ten.valid_after, ten.valid_to
          ...
          LEFT JOIN LATERAL (
              SELECT enterprise_id, ten.valid_after, ten.valid_to
              ...
              WHERE enterprise_id = ten.enterprise_id AND after_to_overlaps(ten.valid_after, ten.valid_to, valid_after, valid_to) GROUP BY enterprise_id, ten.valid_after, ten.valid_to
          ) AS tlu ON true
          LEFT JOIN LATERAL (
              SELECT enterprise_id, ten.valid_after, ten.valid_to
          ...
          SELECT basis.unit_type, basis.unit_id, basis.valid_after, basis.valid_to, basis.name, basis.birth_date, basis.death_date, basis.primary_activity_category_id, basis.primary_activity_category_path, basis.primary_activity_category_code, basis.secondary_activity_category_id, basis.secondary_activity_category_path, basis.secondary_activity_category_code, basis.sector_id, basis.sector_path, basis.sector_code, basis.sector_name,
               (SELECT array_agg(DISTINCT id) FROM (SELECT unnest(basis.data_source_ids) AS id UNION SELECT unnest(aggregation.data_source_ids) AS id) AS ids) AS data_source_ids,
               (SELECT array_agg(DISTINCT code) FROM (SELECT unnest(basis.data_source_codes) AS code UNION ALL SELECT unnest(aggregation.data_source_codes) AS code) AS codes) AS data_source_codes,
               basis.legal_form_id, basis.legal_form_code, basis.legal_form_name, basis.physical_address_part1, basis.physical_address_part2, basis.physical_address_part3, basis.physical_postcode, basis.physical_postplace, basis.physical_region_id, basis.physical_region_path, basis.physical_region_code, basis.physical_country_id, basis.physical_country_iso_2, basis.physical_latitude, basis.physical_longitude, basis.physical_altitude, basis.postal_address_part1, basis.postal_address_part2, basis.postal_address_part3, basis.postal_postcode, basis.postal_postplace, basis.postal_region_id, basis.postal_region_path, basis.postal_region_code, basis.postal_country_id, basis.postal_country_iso_2, basis.postal_latitude, basis.postal_longitude, basis.postal_altitude, basis.web_address, basis.email_address, basis.phone_number, basis.landline, basis.mobile_number, basis.fax_number, basis.unit_size_id, basis.unit_size_code, basis.status_id, basis.status_code, basis.include_unit_in_reports, basis.last_edit_comment, basis.last_edit_by_user_id, basis.last_edit_at, basis.invalid_codes, basis.has_legal_unit,
               COALESCE(aggregation.related_establishment_ids, ARRAY[]::INT[]) AS related_establishment_ids, COALESCE(aggregation.excluded_establishment_ids, ARRAY[]::INT[]) AS excluded_establishment_ids, COALESCE(aggregation.included_establishment_ids, ARRAY[]::INT[]) AS included_establishment_ids, COALESCE(aggregation.related_legal_unit_ids, ARRAY[]::INT[]) AS related_legal_unit_ids, COALESCE(aggregation.excluded_legal_unit_ids, ARRAY[]::INT[]) AS excluded_legal_unit_ids, COALESCE(aggregation.included_legal_unit_ids, ARRAY[]::INT[]) AS included_legal_unit_ids,
               basis.enterprise_id, basis.primary_establishment_id, basis.primary_legal_unit_id, aggregation.stats_summary
          FROM enterprise_with_primary AS basis
          LEFT OUTER JOIN aggregation ON basis.enterprise_id = aggregation.enterprise_id AND basis.valid_after = aggregation.valid_after AND basis.valid_to = aggregation.valid_to
        ), enterprise_with_primary_and_aggregation_and_derived AS (
        SELECT unit_type, unit_id, valid_after, (valid_after + '1 day'::INTERVAL)::DATE AS valid_from, valid_to, name, birth_date, death_date, to_tsvector('simple', name) AS search
