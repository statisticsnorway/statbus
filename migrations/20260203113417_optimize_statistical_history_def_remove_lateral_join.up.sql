-- Migration 20260203113417: optimize_statistical_history_def_remove_lateral_join
--
-- PERF: Replace O(n²) LATERAL JOIN with O(n) pre-aggregated CTE for stats_summary.
--
-- Same optimization as statistical_history_facet_def - the LATERAL JOIN was scanning
-- latest_versions_curr for each row in demographics (one per unit_type = 3 rows).
-- While this is less dramatic than the facet version (3 vs 17,960 demographic groups),
-- it still eliminates unnecessary repeated scans.
BEGIN;

CREATE OR REPLACE FUNCTION public.statistical_history_def(p_resolution public.history_resolution, p_year integer, p_month integer)
