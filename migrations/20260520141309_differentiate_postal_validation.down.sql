-- Reverse migration 20260520141309: restore import.analyse_location to its prior shape,
-- restore the RLS policy on public.location, and revoke admin_user grants on region_access.

BEGIN;

-- ============================================================================
-- 1. Restore import.analyse_location (exact \sf dump from before the migration)
-- ============================================================================
...
-- ============================================================================
-- 2. Restore RLS policy (silent lockout on NULL location.region_id — the bug we removed)
-- ============================================================================
...
-- ============================================================================
-- 3. Revoke admin_user grants on region_access
-- ============================================================================

REVOKE SELECT, INSERT, UPDATE, DELETE ON public.region_access FROM admin_user;

-- ============================================================================
-- 4. Revoke restricted_user write grants on location
-- ============================================================================

REVOKE SELECT, INSERT, UPDATE, DELETE ON public.location FROM restricted_user;

-- ============================================================================
-- 5. Revoke restricted_user write grants on activity
-- ============================================================================

REVOKE SELECT, INSERT, UPDATE, DELETE ON public.activity FROM restricted_user;

-- ============================================================================
-- 6. Revoke admin_user grants on activity_category_access
-- ============================================================================

REVOKE SELECT, INSERT, UPDATE, DELETE ON public.activity_category_access FROM admin_user;

-- ============================================================================
-- 7. Do NOT REVOKE notify_reader on import_job — it's granted by an earlier
--    migration (20250814110000). Section 7 of the up was a defensive re-grant,
--    not a fresh add; reversing it would break the listener pipeline.
-- ============================================================================

-- ============================================================================
-- 8. Clear role comments
-- ============================================================================

COMMENT ON ROLE admin_user      IS NULL;
COMMENT ON ROLE regular_user    IS NULL;
COMMENT ON ROLE restricted_user IS NULL;
COMMENT ON ROLE authenticated   IS NULL;
COMMENT ON ROLE external_user   IS NULL;
COMMENT ON ROLE anon            IS NULL;
