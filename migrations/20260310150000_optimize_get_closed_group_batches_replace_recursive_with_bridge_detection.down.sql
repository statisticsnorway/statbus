-- Down Migration 20260309195139: optimize_get_closed_group_batches_replace_recursive_with_bridge_detection
-- Restores the recursive transitive closure approach and integer[] parameter signature.
BEGIN;

-- Drop the new function (multirange signature)
...
-- Restore the original function with integer[] parameters and recursive CTE
...
-- Restore derive_statistical_unit to call get_closed_group_batches with integer[] arrays
...
BEGIN
    ...
    ELSE
        ...
        -- BATCHING: EST/LU/EN use closed-group batches (with integer[] arrays)
