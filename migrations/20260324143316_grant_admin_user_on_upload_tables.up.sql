-- Migration 20260324143316: grant_admin_user_on_upload_tables
-- Fix: admin users could not upload to import jobs created by other users.
-- The upload table only had GRANT to the job creator's email role.
BEGIN;
...
-- Retroactively grant admin_user on all existing upload tables
DO $$
DECLARE
    rec RECORD;
BEGIN
    FOR rec IN SELECT id, upload_table_name FROM public.import_job WHERE upload_table_name IS NOT NULL LOOP
        IF to_regclass(format('public.%I', rec.upload_table_name)) IS NOT NULL THEN
            EXECUTE format($grant$GRANT ALL ON TABLE public.%I TO admin_user$grant$, rec.upload_table_name);
            RAISE NOTICE 'Granted admin_user ALL on %', rec.upload_table_name;
        END IF;
    END LOOP;
END$$;
