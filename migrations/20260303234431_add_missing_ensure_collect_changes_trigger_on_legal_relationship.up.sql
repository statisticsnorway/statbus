...
-- Fix 1: The 20260226 migration added log_base_change triggers on legal_relationship
-- (a_legal_relationship_log_insert/update/delete) but forgot to add the
-- ensure_collect_changes trigger that enqueues a collect_changes worker task.
-- Without this trigger, changes to legal_relationship are logged to
-- base_change_log but never picked up by the worker.
...
-- Fix 2: The 20260226 migration linked power_group_link to existing legal_relationship
-- definitions, but sample scripts that create definitions AFTER migration time
-- don't include it. Ensure all legal_relationship definitions have the step.

INSERT INTO public.import_definition_step (definition_id, step_id)
SELECT idef.id, ist.id
FROM public.import_definition AS idef
CROSS JOIN public.import_step AS ist
WHERE idef.mode = 'legal_relationship'
  AND ist.code = 'power_group_link'
