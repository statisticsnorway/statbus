-- Migration 20260202210959: add_advisory_locks_for_unit_type_refresh
BEGIN;

-- Add advisory locks to prevent B-tree page contention during concurrent batch refresh.
--
-- Problem: Even though batches have disjoint enterprise IDs, their establishments
-- and legal_units share B-tree index pages, causing deadlocks during concurrent
-- DELETE/INSERT operations.
--
-- Solution: Use advisory locks to serialize access per unit_type within each
-- refresh procedure. Batches can still run in parallel, but they'll serialize
-- when modifying the same table+unit_type combination.
--
-- Advisory lock IDs:
--   hashtext('timepoints_establishment')   - timepoints establishment operations
--   hashtext('timepoints_legal_unit')      - timepoints legal_unit operations  
--   hashtext('timepoints_enterprise')      - timepoints enterprise operations
--   hashtext('timesegments_establishment') - timesegments establishment operations
--   hashtext('timesegments_legal_unit')    - timesegments legal_unit operations
--   hashtext('timesegments_enterprise')    - timesegments enterprise operations
--   hashtext('statistical_unit_establishment') - statistical_unit establishment operations
--   hashtext('statistical_unit_legal_unit')    - statistical_unit legal_unit operations
--   hashtext('statistical_unit_enterprise')    - statistical_unit enterprise operations

-- Update timepoints_refresh to use advisory locks for partial refresh
...
-- Update timesegments_refresh to use advisory locks for partial refresh
...
-- Update statistical_unit_refresh to use advisory locks for partial refresh
