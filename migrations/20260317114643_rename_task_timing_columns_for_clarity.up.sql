...
-- =============================================================================
-- Rename task timing columns for clarity.
--
-- Before:
--   processed_at  — ambiguous (when processing started? when done?)
--   completed_at  — when fully done including children
--   duration_ms   — ambiguous (processing time? total time?)
--
-- After:
--   process_start_at       — when handler began executing
--   process_stop_at        — when handler returned (children may still run)
--   completed_at           — when task + all children fully done (unchanged)
--   process_duration_ms    — process_stop_at - process_start_at
--   completion_duration_ms — completed_at - process_start_at
--
-- For leaf tasks: process_stop_at ≈ completed_at, both durations equal.
-- For parent tasks: completion_duration_ms > process_duration_ms (gap = children).
-- =============================================================================

-- Step 1: Rename existing columns
ALTER TABLE worker.tasks RENAME COLUMN processed_at TO process_start_at;
ALTER TABLE worker.tasks RENAME COLUMN duration_ms TO process_duration_ms;

-- Step 2: Add new columns
ALTER TABLE worker.tasks ADD COLUMN process_stop_at TIMESTAMPTZ;
ALTER TABLE worker.tasks ADD COLUMN completion_duration_ms NUMERIC;

-- Step 3: Backfill from existing data
-- Old processed_at was set when the handler returned (= process_stop_at semantics)
-- So process_start_at should be computed backward from duration.
-- Actually, the old code set:
--   processed_at = clock_timestamp()  -- after handler returned
--   duration_ms  = elapsed from start_time
-- So: old processed_at = handler finish time = our new process_stop_at
--     process_start_at = processed_at - duration_ms
UPDATE worker.tasks
SET process_stop_at = process_start_at,  -- old processed_at = handler finish time
    process_start_at = process_start_at - make_interval(secs => process_duration_ms / 1000.0),
    completion_duration_ms = CASE
        WHEN completed_at IS NOT NULL
        THEN EXTRACT(EPOCH FROM (completed_at - (process_start_at - make_interval(secs => process_duration_ms / 1000.0)))) * 1000
        ELSE process_duration_ms
    END
WHERE process_start_at IS NOT NULL;

-- Step 4: Recreate worker_task view with new column names
...
-- Step 5: Update process_tasks to use new column names and record process_start_at
-- before handler execution, process_stop_at after.
...
-- Step 5b: Update derive_statistical_unit handler to use process_start_at
-- (This handler uses ORDER BY processed_at to find its own task_id)
...
-- Step 6: Update complete_parent_if_ready to compute completion_duration_ms
