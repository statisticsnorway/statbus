...
BEGIN
    ...
    v_sql := format($SQL$
        ...
        UPDATE public.%1$I dt SET
            ...
            END,
            errors = CASE
                WHEN l.missing_influencing THEN dt.errors || jsonb_build_object('missing_influencing_tax_ident', 'influencing_tax_ident is required')
                WHEN l.unknown_influencing THEN dt.errors || jsonb_build_object('unknown_influencing_tax_ident', 'No legal unit found for influencing_tax_ident')
                WHEN l.missing_influenced THEN dt.errors || jsonb_build_object('missing_influenced_tax_ident', 'influenced_tax_ident is required')
                WHEN l.unknown_influenced THEN dt.errors || jsonb_build_object('unknown_influenced_tax_ident', 'No legal unit found for influenced_tax_ident')
                WHEN l.missing_rel_type THEN dt.errors || jsonb_build_object('missing_rel_type_code', 'rel_type_code is required')
                WHEN l.unknown_rel_type THEN dt.errors || jsonb_build_object('unknown_rel_type_code', 'Unknown rel_type_code')
                WHEN l.invalid_percentage THEN dt.errors || jsonb_build_object('invalid_percentage', 'percentage must be a number 0-100')
