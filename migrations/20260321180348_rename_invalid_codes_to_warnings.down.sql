...
-- Step 1: Restore import_data_column spec
UPDATE public.import_data_column
SET column_name = 'invalid_codes'
WHERE column_name = 'warnings';

-- Step 2: Rename column back on all existing data tables
DO $$
DECLARE r RECORD;
BEGIN
  ...
  LOOP
    IF EXISTS (
      ...
      EXECUTE format('ALTER TABLE public.%I RENAME COLUMN warnings TO invalid_codes', r.data_table_name);
    END IF;
  END LOOP;
END $$;

-- Step 3: Restore definition_snapshot JSONB
UPDATE public.import_job
SET definition_snapshot = jsonb_set(
  definition_snapshot,
  '{import_data_column_list}',
  (SELECT jsonb_agg(
    CASE WHEN elem->>'column_name' = 'warnings'
         THEN jsonb_set(elem, '{column_name}', '"invalid_codes"')
         ELSE elem
    END
  ) FROM jsonb_array_elements(definition_snapshot->'import_data_column_list') AS elem)
)
WHERE definition_snapshot->'import_data_column_list' @> '[{"column_name": "warnings"}]';

-- Step 4: Restore original functions (from dumps)

-- 4a: admin.import_job_process(integer, jsonb)
