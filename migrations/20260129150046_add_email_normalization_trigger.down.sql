-- Down Migration: Remove email normalization trigger
-- Note: This does NOT revert emails to their original case - that information is lost

-- Drop the normalization trigger
DROP TRIGGER IF EXISTS "00_normalize_email_trigger" ON auth.user;

-- Drop the normalization function
DROP FUNCTION IF EXISTS auth.normalize_email();

-- Restore the original sync_user_credentials_and_roles function without lower()
-- This is the version from the original migration
