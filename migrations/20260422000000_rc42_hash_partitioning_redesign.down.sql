-- rc.42 DOWN: revert hash-partitioning redesign to pre-rc.42 state
--
-- This migration reverses the up.sql in the exact inverse block order.
-- Each reverse block restores the verbatim pre-rc.42 bodies from
-- tmp/paralegal-rc42-dumps.md (the authoritative source for what the
-- functions, procedures, types, and columns looked like before rc.42).
--
-- Reverting means:
--   - Hash functions back to `report_partition_seq`, body returns `% 256`.
--   - All renamed columns / indexes / triggers / constraints / tables
--     back to their old names (partition_seq, report_partition_modulus,
--     statistical_unit_facet_dirty_partitions, etc.).
--   - statistical_history TYPE rebuilt with `partition_seq integer` at
--     attr 24 (replacing the rc.42 `hash_partition int4range`).
--   - All consumer functions (statistical_history_def etc.) restored with
--     their pre-rc.42 signatures (`p_partition_seq_from`, `p_partition_seq_to`
--     integer args).
--   - The three spawners restored with the pre-rc.42 broken
--     `FOR v_range_start IN 0..(modulus-1) BY v_range_size LOOP`.
--     This returns #49 (the silent-slot-drop bug) but matches original
--     behaviour — the down.sql is an inverse, not a further fix.
--   - Stored L1 slot values re-hashed back to `% 256` space (reverse of
--     Block G's % 16384 rehash).
--
-- Reverse block order. Note: this is NOT a strict mirror of the forward
-- sequence — R-C is moved before R-F because `statistical_history_def`
-- RETURNS SETOF `statistical_history_type`, so the function can only be
-- (re)created after the pre-rc.42 type has been restored. In forward, C
-- ran before F for the same dependency; the dependency does not invert.
--   R-H2 — Revert 4 non-#67 routines (history_reduce, flush_staging,
--          relevant_statistical_units, get_statistical_unit_data_partial).
--   R-H  — Revert 4 paralegal-#67 routines (public.reset,
--          derive_statistical_unit, 2 reduces).
--   R-G  — Rehash stored L1 slots back to % 256 space (on _staging and
--          the main statistical_unit column; R-A later restores the
--          hash_slot() body to the pre-rc.42 `% 256` formula).
--   R-C  — Drop rc.42 statistical_history TYPE + typed table; recreate
--          pre-rc.42 TYPE (attr 24 = partition_seq integer) and typed
--          table with original indexes, RLS, grants. Runs before R-F
--          because the consumer fn R-F restores returns SETOF this type.
--   R-F  — Restore pre-rc.42 consumer fns + period handlers
--          (statistical_history_def, _facet_def, 3 period handlers).
--   R-E  — Restore pre-rc.42 spawners (3 procedures; keeps original
--          FOR LOOP BY v_range_size — the broken version).
--   R-D  — Rename settings column back (`partition_count_target` →
--          `report_partition_modulus`); restore `get_report_partition_modulus`
--          function and `adjust_report_partition_modulus` procedure.
--   R-B  — Reverse all column / index / trigger / constraint / table
--          renames (hash_slot → partition_seq / report_partition_seq /
--          dirty_hash_slot; _dirty_hash_slots → _dirty_partitions).
--   R-A  — Restore hash function bodies to `% 256`; rename hash_slot
--          functions back to report_partition_seq.
--
-- Source dumps: tmp/paralegal-rc42-dumps.md §3 (functions) + §4 (procedures)
-- + §1 (type attribute lists).

BEGIN;

-- ============================================================
-- R-H2 — Revert 4 non-#67 routines
-- ============================================================
-- Verbatim pre-rc.42 bodies; restores references to partition_seq,
-- report_partition_seq(), and admin.adjust_report_partition_modulus().
-- These routines must revert BEFORE R-B reverses the column renames,
-- otherwise the stale rc.42 bodies (hash_partition / hash_slot /
-- partition_count_target) would fail at parse time against post-R-B
-- columns.
...
-- ============================================================
-- R-H — Revert 4 paralegal-#67 routines
-- ============================================================
-- Verbatim pre-rc.42 bodies: public.reset, worker.derive_statistical_unit,
-- worker.statistical_history_facet_reduce, worker.statistical_unit_facet_reduce.
-- Restores references to:
--   - statistical_unit_facet_dirty_partitions table (old name)
--   - partition_seq column (old name)
--   - 'dirty_partitions' JSON key in p_info (old name)
--   - public.report_partition_seq() function (old name)
-- Order matches up.sql Block H for symmetry; none depends on another within
-- this block.
...
-- R-H part 2: restore worker.statistical_history_facet_reduce and
-- worker.statistical_unit_facet_reduce to their pre-rc.42 bodies
-- (referencing public.statistical_unit_facet_dirty_partitions, partition_seq
-- column, and the 'dirty_partitions' p_info key).
...
-- ============================================================
-- R-G — Rehash stored L1 slots back to % 256 space
-- ============================================================
--
-- Mirror of up.sql Block G (which rehashed % 256 → % 16384). We cannot call
-- public.hash_slot() here because its body still returns % 16384 (R-A restores
-- the % 256 body later in the ordering). Compute inline with the same hashtext
-- formula the pre-rc.42 public.report_partition_seq() used. The column is
-- still named `hash_slot` at this point (R-B renames it back to
-- report_partition_seq afterwards); values land in 0..255 regardless of the
-- column's eventual name. Triggers are BEFORE INSERT only, so UPDATE does not
-- re-fire them; we assign the new value directly.

UPDATE public.statistical_unit
   SET hash_slot = abs(hashtext(unit_type::text || ':' || unit_id::text)) % 256;

UPDATE public.statistical_unit_staging
   SET hash_slot = abs(hashtext(unit_type::text || ':' || unit_id::text)) % 256;

-- ============================================================
-- R-C — Drop rc.42 statistical_history TYPE; recreate pre-rc.42 TYPE
-- ============================================================
--
-- Inverse of forward Block C. DROP CASCADE removes the rc.42 type's
-- dependents: the statistical_history table (OF type) and the rc.42
-- statistical_history_def(..., int4range) function (RETURNS SETOF type).
-- We then rebuild the pre-rc.42 type (attr 24 = partition_seq integer),
-- the typed table, 9 indexes (predicates on partition_seq), 3 RLS
-- policies, and the original grants. The consumer function
-- statistical_history_def is recreated in R-F (next block), since its
-- RETURNS type now exists. Derived data rebuilds on the next derive
-- trigger tick after down.sql boots.

DROP TABLE public.statistical_history CASCADE;
DROP TYPE  public.statistical_history_type CASCADE;

-- Recreate type with pre-rc.42 shape (attr 24 reverts from hash_partition
-- int4range back to partition_seq integer).
...
-- 9 indexes matching doc/db/table/public_statistical_history.md exactly.
...
-- 3 RLS policies (same gate semantics, column reverted).
...
-- ============================================================
--
-- Inverse of forward Block F. Restores:
--   1. public.statistical_history_def (5-arg old; returns SETOF the
--      restored pre-rc.42 statistical_history_type from R-C).
--   2. public.statistical_history_facet_def (4-arg old; param renamed
--      p_hash_slot → p_partition_seq, body uses su.report_partition_seq).
--   3. worker.derive_statistical_history_period
--   4. worker.derive_statistical_history_facet_period
--   5. worker.derive_statistical_unit_facet_partition
--
-- The 3 period handlers read payload keys partition_seq_from/_to (with
-- legacy single-partition_seq fallback) and call
-- statistical_history_def / statistical_history_facet_def with integer
-- args. They reference `partition_seq` columns on
-- statistical_history / statistical_history_facet_partitions /
-- statistical_unit_facet_staging — these columns are still named
-- hash_slot at this point; R-B renames them back. plpgsql body parsing
-- is deferred, so the CREATE OR REPLACE succeeds. The bodies are fully
-- executable once the rest of down.sql completes.

-- -- public.statistical_history_def (5-arg pre-rc.42 form) -----------------
CREATE FUNCTION public.statistical_history_def(p_resolution history_resolution, p_year integer, p_month integer, p_partition_seq_from integer DEFAULT NULL::integer, p_partition_seq_to integer DEFAULT NULL::integer)
...
-- -- public.statistical_history_facet_def (4-arg pre-rc.42 form) -----------
-- Param-name reverse (p_hash_slot → p_partition_seq), same signature. The
-- rc.42 facet_def is still live here — facet_type was NOT dropped by R-C —
-- so CREATE OR REPLACE would fail with "cannot change name of input
-- parameter". Drop the rc.42 function first, mirror of the forward DROP.
...
-- ============================================================
-- R-E — Restore pre-rc.42 spawners (3 procedures)
-- ============================================================
-- Verbatim pre-rc.42 bodies sourced from tmp/paralegal-rc42-dumps.md §4
-- (dumps taken 2026-04-22 from the same DB state as doc/db/function/).
-- These use the original range-batch loop:
--   FOR v_range_start IN 0..(v_modulus-1) BY v_range_size LOOP
-- with `partition_seq_from` / `partition_seq_to` payload keys. The R-F
-- period handlers are already verbatim pre-rc.42 and accept these keys
-- with COALESCE fallback — restoring the full pre-rc.42 pipeline.
-- Both spawners (history_facet, unit_facet) also TRUNCATE + INSERT into
-- their pre_dirty_dims tables so the R-H reduce procedures can delete
-- stale rows in scoped MERGE paths.
...
-- ============================================================
-- R-D — Settings + auto-tune rename/restore
-- ============================================================
-- Rename settings column back: partition_count_target → report_partition_modulus.
-- DEFAULT stays 256 (original pre-rc.42 value was also 256, per migration
-- 20260325011559_virtual_physical_partition_mapping_with_auto_tune), so no
-- explicit SET DEFAULT is needed after the rename.
ALTER TABLE public.settings
    RENAME COLUMN partition_count_target TO report_partition_modulus;

-- Rename function + procedure back to their pre-rc.42 names. CREATE OR REPLACE
-- bodies below restore the pre-rc.42 logic (different bucket thresholds).
ALTER FUNCTION public.get_partition_count_target()
    RENAME TO get_report_partition_modulus;
ALTER PROCEDURE admin.adjust_partition_count_target()
    RENAME TO adjust_report_partition_modulus;
...
-- ============================================================
-- R-B — Reverse column / index / trigger / constraint / table renames
-- ============================================================
-- Reverse of Block B — ALTER RENAME sweep for columns, tables, indexes,
-- triggers, constraints. Statements are ordered as the reverse of the forward
-- block (section 7→1). Grants carry through ALTER RENAME so no re-GRANT needed.
-- Block A hash-function renames are handled separately in R-A.

-- 7' Constraint renames (reverse forward section 7).
--    Constraint names are not auto-renamed by column/table renames, so explicit
--    reverse is required. Order before the dirty-table rename since the constraint
--    is attached to the table under its current name.
ALTER TABLE public.statistical_history_facet_partitions
    RENAME CONSTRAINT statistical_history_facet_partitions_hash_slot_not_null
                   TO statistical_history_facet_partitions_partition_seq_not_null;
ALTER TABLE public.statistical_unit_facet_dirty_hash_slots
    RENAME CONSTRAINT statistical_unit_facet_dirty_hash_slots_dirty_hash_slot_not_null
                   TO statistical_unit_facet_dirty_partitions_partition_seq_not_null;
ALTER TABLE public.statistical_unit_facet_staging
    RENAME CONSTRAINT statistical_unit_facet_staging_hash_slot_not_null
                   TO statistical_unit_facet_staging_partition_seq_not_null;

-- 6' Dirty-queue table + column + pkey-index rename-back.
--    Forward order was: table, column, pkey. Reverse: pkey, column, table.
ALTER INDEX public.statistical_unit_facet_dirty_hash_slots_pkey
    RENAME TO statistical_unit_facet_dirty_partitions_pkey;
ALTER TABLE public.statistical_unit_facet_dirty_hash_slots
    RENAME COLUMN dirty_hash_slot TO partition_seq;
ALTER TABLE public.statistical_unit_facet_dirty_hash_slots
    RENAME TO statistical_unit_facet_dirty_partitions;

-- 5' Facet index renames (reverse forward section 5).
ALTER INDEX public.statistical_history_facet_par_hash_slot_resolution_year_key
    RENAME TO statistical_history_facet_par_partition_seq_resolution_year_key;
ALTER INDEX public.idx_shf_partitions_hash_slot
    RENAME TO idx_shf_partitions_seq;
ALTER INDEX public.statistical_unit_facet_stagin_hash_slot_valid_from_vali_key
    RENAME TO statistical_unit_facet_stagin_partition_seq_valid_from_vali_key;
ALTER INDEX public.idx_statistical_unit_facet_staging_hash_slot
    RENAME TO idx_statistical_unit_facet_staging_partition_seq;
ALTER INDEX public.idx_statistical_unit_facet_hash_slot
    RENAME TO idx_statistical_unit_facet_partition_seq;

-- 4' Facet pipeline column renames: hash_slot → partition_seq.
ALTER TABLE public.statistical_history_facet_partitions
    RENAME COLUMN hash_slot TO partition_seq;
ALTER TABLE public.statistical_unit_facet_staging
    RENAME COLUMN hash_slot TO partition_seq;
ALTER TABLE public.statistical_unit_facet
    RENAME COLUMN hash_slot TO partition_seq;

-- 3' Trigger renames on statistical_unit + _staging.
ALTER TRIGGER trg_set_hash_slot ON public.statistical_unit_staging
    RENAME TO trg_set_report_partition_seq;
ALTER TRIGGER trg_set_hash_slot ON public.statistical_unit
    RENAME TO trg_set_report_partition_seq;

-- 2' Statistical_unit index rename. (statistical_unit_staging has no L1-slot
--    index to reverse — same asymmetry noted in up.sql Block B.)
ALTER INDEX public.idx_statistical_unit_hash_slot
    RENAME TO idx_statistical_unit_report_partition_seq;

-- 1' Statistical_unit + _staging column renames: hash_slot → report_partition_seq.
ALTER TABLE public.statistical_unit_staging
    RENAME COLUMN hash_slot TO report_partition_seq;
ALTER TABLE public.statistical_unit
    RENAME COLUMN hash_slot TO report_partition_seq;

-- ============================================================
-- R-A — Hash function rename-back + body restore to % 256
-- ============================================================
-- Reverse of Block A — rename hash functions + trigger wrapper back, then
-- CREATE OR REPLACE bodies to the pre-rc.42 `% 256` hash space.
ALTER FUNCTION public.set_hash_slot()
    RENAME TO set_report_partition_seq;
ALTER FUNCTION public.hash_slot(text, integer)
    RENAME TO report_partition_seq;
ALTER FUNCTION public.hash_slot(public.statistical_unit_type, integer)
    RENAME TO report_partition_seq;
