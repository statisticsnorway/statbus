-- Down Migration 20260324143316: grant_admin_user_on_upload_tables
BEGIN;

-- Restore original function without admin_user GRANT on upload tables
...
-- Revoke admin_user from existing upload tables
DO $$
DECLARE
    rec RECORD;
BEGIN
    FOR rec IN SELECT id, upload_table_name FROM public.import_job WHERE upload_table_name IS NOT NULL LOOP
        IF to_regclass(format('public.%I', rec.upload_table_name)) IS NOT NULL THEN
            EXECUTE format($revoke$REVOKE ALL ON TABLE public.%I FROM admin_user$revoke$, rec.upload_table_name);
            RAISE NOTICE 'Revoked admin_user from %', rec.upload_table_name;
        END IF;
    END LOOP;
END$$;
