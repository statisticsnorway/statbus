-- Down Migration: Restore LATERAL-based timeline_legal_unit_def view
-- and the pre-materialized temp table refresh procedure
BEGIN;

-- ============================================================================
-- Restore the original LATERAL-based view definition
-- (from migration 20240306000000_create_timeline_legal_unit)
-- ============================================================================
...
SELECT basis.unit_type,
  ...
  FROM basis
    LEFT JOIN LATERAL ( SELECT tes.legal_unit_id,
         ...
         WHERE tes.legal_unit_id = basis.legal_unit_id AND public.from_until_overlaps(basis.valid_from, basis.valid_until, tes.valid_from, tes.valid_until)
         GROUP BY tes.legal_unit_id) AS esa ON true
  ORDER BY basis.unit_type, basis.unit_id, basis.valid_from;

-- ============================================================================
-- Restore the pre-materialized temp table refresh procedure
-- (from migration 20260205122909_fix_timeline_legal_unit_on2_behavior)
-- ============================================================================
...
BEGIN
    ...
    ELSE
        -- Partial refresh: Pre-materialize filtered timeline_establishment to avoid O(n^2) scan
        ...
        -- NOTE: This uses inline SQL with the pre-filtered table instead of the view.
        -- The full inline SQL is omitted here for brevity — the up migration contains
        -- the improved CTE-based approach that makes this workaround unnecessary.
        -- If a full rollback is needed, restore from migration 20260205122909.
        CALL public.timeline_refresh('timeline_legal_unit', 'legal_unit', p_unit_id_ranges);
