...
CREATE INDEX ix_establishment_legal_unit_id_valid_range ON public.establishment USING gist (legal_unit_id, valid_range);
CREATE INDEX ix_establishment_enterprise_id_valid_range ON public.establishment USING gist (enterprise_id, valid_range);
CREATE INDEX ix_establishment_valid_range ON public.establishment USING gist (valid_range);
...
SELECT sql_saga.add_era('public.establishment', 'valid_range', 'valid', ephemeral_columns => ARRAY['edit_comment', 'edit_by_user_id', 'edit_at']);
-- This creates a PRIMARY KEY with WITHOUT OVERLAPS to ensure that
-- there are no overlapping time periods for the same establishment ID.
