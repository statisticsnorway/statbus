-- Migration: create_set_based_temporal_update_functions
--
-- This migration introduces the set-based functions for handling temporal
-- data "insert or replace" operations. These functions
...
source_initial AS (
    SELECT
        ...
        t.valid_after,
        t.valid_to,
        jsonb_strip_nulls(%2$s) AS data_payload -- For UPDATE, NULLs in source are ignored
    FROM %3$s.%4$s t
...
source_rows AS (
    ...
    WHERE CASE %10$L::import.set_operation_mode
        ...
        WHEN 'upsert_replace' THEN true -- Should not be passed to _update func, but handle gracefully
        ...
        WHEN 'replace_only' THEN si.entity_id IN (SELECT tr.entity_id FROM target_rows tr) -- Should not be passed here
