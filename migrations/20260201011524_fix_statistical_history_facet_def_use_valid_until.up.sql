-- Migration: Fix statistical_history_facet_def to use valid_until instead of valid_to
-- 
-- CRITICAL BUG FIX: The function was using valid_to (a derived column for UI convenience)
-- instead of valid_until (the canonical column) in daterange comparisons.
-- 
-- This prevented the GIST exclusion index from being used:
--   - Index is on: daterange(valid_from, valid_until, '[)')
--   - Function was using: daterange(valid_from, valid_to, '[)')
-- 
-- The fix changes two lines:
--   1. Line 34: daterange(su.valid_from, su.valid_to, '[)') -> daterange(su.valid_from, su.valid_until, '[)')
--   2. Line 41: uip.valid_to >= v_curr_start -> uip.valid_until > v_curr_start
--
-- Expected performance improvement: From sequential scan to index scan on statistical_unit table.
