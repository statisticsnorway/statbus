-- Migration: Convert timeline_legal_unit_def LATERAL establishment join to CTE
--
-- Problem: The LATERAL join on timeline_establishment forces a Nested Loop:
--   FOR each basis row (timesegment):
--     Scan timeline_establishment WHERE legal_unit_id = X AND temporal overlap
--     GROUP BY legal_unit_id
-- This is O(N × scan) per batch, with the scan growing as timeline_establishment fills.
--
-- Solution: Pre-aggregate the establishment data in a CTE, then exact-match join:
--   CTE establishment_aggs:
--     JOIN timesegments × timeline_establishment (PostgreSQL chooses Hash/Merge Join)
--     GROUP BY (unit_id, valid_from)
--   Then: basis LEFT JOIN establishment_aggs ON (unit_id, valid_from) — exact equality
--
-- This mirrors the existing legal_unit_stats CTE pattern.
-- Additionally, the refresh procedure no longer needs the pre-materialized temp table
-- workaround since the view itself is now efficient.
BEGIN;

-- ============================================================================
-- Replace the view with CTE-based establishment aggregation
-- ============================================================================
...
WITH legal_unit_stats AS (
    ...
    -- This replaces the LATERAL join that forced nested loops.
    -- PostgreSQL can now choose Hash Join or Merge Join for the temporal overlap.
    SELECT t.unit_id,
       t.valid_from,
...
-- ============================================================================
-- Refresh procedure: full refresh uses the view (CTE scans everything once).
-- Partial refresh uses pre-filtered UNLOGGED table + the view's CTE structure
-- with explicit unit_id filtering to avoid scanning all timesegments per batch.
-- ============================================================================
...
BEGIN
    IF p_unit_id_ranges IS NULL THEN
        -- Full refresh: ANALYZE first, then use the CTE-based view via timeline_refresh.
        -- The view's CTEs scan everything once — O(T + E), efficient for full refresh.
    ...
    ELSE
        -- Partial refresh: PostgreSQL can't push the outer unit_id filter into the view's
        -- establishment_aggs CTE, causing it to scan ALL timesegments per batch.
        -- Pre-filter timeline_establishment into an UNLOGGED table, then aggregate via CTE
        -- with explicit unit_id filtering.
        ...
        DROP TABLE IF EXISTS pg_temp.timeline_establishment_filtered;
        PERFORM set_config('client_min_messages', 'notice', true);

        CREATE TEMP TABLE timeline_establishment_filtered ON COMMIT DROP AS
        SELECT tes.*
        ...
        -- Insert using the same CTE structure as the view, but with:
        -- 1. unit_id filtering in all CTEs
        -- 2. Pre-filtered establishment table instead of full timeline_establishment
        ...
        WITH legal_unit_stats AS (
            ...
            SELECT t.unit_id,
               ...
               public.jsonb_stats_summary_merge_agg(tes.stats_summary) FILTER (WHERE tes.used_for_counting) AS stats_summary
              FROM public.timesegments AS t
                JOIN pg_temp.timeline_establishment_filtered AS tes
                  ON tes.legal_unit_id = t.unit_id
                 AND public.from_until_overlaps(t.valid_from, t.valid_until, tes.valid_from, tes.valid_until)
             WHERE t.unit_type = 'legal_unit'::public.statistical_unit_type
               AND t.unit_id = ANY(v_unit_ids)
            ...
            SELECT t.unit_type AS src_unit_type,
              ...
              FROM public.timesegments AS t
                JOIN LATERAL ( SELECT lu_1.id,
                       lu_1.valid_range, lu_1.valid_from, lu_1.valid_to, lu_1.valid_until,
                       lu_1.short_name, lu_1.name, lu_1.birth_date, lu_1.death_date,
                       lu_1.free_econ_zone, lu_1.sector_id, lu_1.status_id, lu_1.legal_form_id,
                       lu_1.edit_comment, lu_1.edit_by_user_id, lu_1.edit_at,
                       lu_1.unit_size_id, lu_1.foreign_participation_id, lu_1.data_source_id,
                       lu_1.enterprise_id, lu_1.primary_for_enterprise, lu_1.invalid_codes, lu_1.image_id
                      FROM public.legal_unit AS lu_1
                     WHERE lu_1.id = t.unit_id AND public.from_until_overlaps(t.valid_from, t.valid_until, lu_1.valid_from, lu_1.valid_until)
                     ORDER BY lu_1.id DESC, lu_1.valid_from DESC
                    LIMIT 1) AS lu ON true
                LEFT JOIN legal_unit_stats AS lu_stats ON lu_stats.unit_id = t.unit_id AND lu_stats.valid_from = t.valid_from
                LEFT JOIN LATERAL ( SELECT a.id, a.valid_range, a.valid_from, a.valid_to, a.valid_until,
                       a.type, a.category_id, a.data_source_id, a.edit_comment, a.edit_by_user_id, a.edit_at,
                       a.establishment_id, a.legal_unit_id
                      FROM public.activity AS a
                     WHERE a.legal_unit_id = lu.id AND a.type = 'primary'::public.activity_type AND public.from_until_overlaps(t.valid_from, t.valid_until, a.valid_from, a.valid_until)
                     ORDER BY a.id DESC
                    LIMIT 1) AS pa ON true
                ...
                LEFT JOIN public.sector AS s ON lu.sector_id = s.id
                LEFT JOIN public.legal_form AS lf ON lu.legal_form_id = lf.id
                LEFT JOIN LATERAL ( SELECT l.id, l.valid_range, l.valid_from, l.valid_to, l.valid_until,
                       l.type, l.address_part1, l.address_part2, l.address_part3,
                       l.postcode, l.postplace, l.region_id, l.country_id,
                       l.latitude, l.longitude, l.altitude,
                       l.establishment_id, l.legal_unit_id, l.data_source_id,
                       l.edit_comment, l.edit_by_user_id, l.edit_at
                      FROM public.location AS l
                     WHERE l.legal_unit_id = lu.id AND l.type = 'physical'::public.location_type AND public.from_until_overlaps(t.valid_from, t.valid_until, l.valid_from, l.valid_until)
                     ORDER BY l.id DESC
                    LIMIT 1) AS phl ON true
                ...
                LEFT JOIN public.region AS por ON pol.region_id = por.id
                LEFT JOIN public.country AS poc ON pol.country_id = poc.id
                LEFT JOIN LATERAL ( SELECT c_1.id, c_1.valid_range, c_1.valid_from, c_1.valid_to, c_1.valid_until,
                       c_1.web_address, c_1.email_address, c_1.phone_number, c_1.landline,
                       c_1.mobile_number, c_1.fax_number,
                       c_1.establishment_id, c_1.legal_unit_id, c_1.data_source_id,
                       c_1.edit_comment, c_1.edit_by_user_id, c_1.edit_at
                      FROM public.contact AS c_1
                     WHERE c_1.legal_unit_id = lu.id AND public.from_until_overlaps(t.valid_from, t.valid_until, c_1.valid_from, c_1.valid_until)
                     ORDER BY c_1.id DESC
                    LIMIT 1) AS c ON true
                ...
                LEFT JOIN LATERAL ( SELECT array_agg(DISTINCT sfu.data_source_id) FILTER (WHERE sfu.data_source_id IS NOT NULL) AS data_source_ids
                      FROM public.stat_for_unit AS sfu
                     WHERE sfu.legal_unit_id = lu.id AND public.from_until_overlaps(t.valid_from, t.valid_until, sfu.valid_from, sfu.valid_until)) AS sfu_ds ON true
                LEFT JOIN LATERAL ( SELECT sfu.edit_comment, sfu.edit_by_user_id, sfu.edit_at
                      FROM public.stat_for_unit AS sfu
                     WHERE sfu.legal_unit_id = lu.id AND public.from_until_overlaps(t.valid_from, t.valid_until, sfu.valid_from, sfu.valid_until)
                     ORDER BY sfu.edit_at DESC
                    LIMIT 1) AS sfu_le ON true
                LEFT JOIN LATERAL ( SELECT array_agg(ds_1.id) AS ids, array_agg(ds_1.code) AS codes
                      FROM public.data_source AS ds_1
                     WHERE COALESCE(ds_1.id = lu.data_source_id, false) OR COALESCE(ds_1.id = pa.data_source_id, false) OR COALESCE(ds_1.id = sa.data_source_id, false) OR COALESCE(ds_1.id = phl.data_source_id, false) OR COALESCE(ds_1.id = pol.data_source_id, false) OR COALESCE(ds_1.id = ANY (sfu_ds.data_source_ids), false)) AS ds ON true
                LEFT JOIN LATERAL ( SELECT all_edits.edit_comment, all_edits.edit_by_user_id, all_edits.edit_at
                      FROM ( VALUES (lu.edit_comment,lu.edit_by_user_id,lu.edit_at), (pa.edit_comment,pa.edit_by_user_id,pa.edit_at), (sa.edit_comment,sa.edit_by_user_id,sa.edit_at), (phl.edit_comment,phl.edit_by_user_id,phl.edit_at), (pol.edit_comment,pol.edit_by_user_id,pol.edit_at), (c.edit_comment,c.edit_by_user_id,c.edit_at), (sfu_le.edit_comment,sfu_le.edit_by_user_id,sfu_le.edit_at)) AS all_edits(edit_comment, edit_by_user_id, edit_at)
                     WHERE all_edits.edit_at IS NOT NULL
                     ORDER BY all_edits.edit_at DESC
                    LIMIT 1) AS last_edit ON true,
        ...
        SELECT basis.src_unit_type,
           ...
           basis.name, basis.birth_date, basis.death_date, basis.search,
           basis.primary_activity_category_id, basis.primary_activity_category_path, basis.primary_activity_category_code,
           basis.secondary_activity_category_id, basis.secondary_activity_category_path, basis.secondary_activity_category_code,
           basis.activity_category_paths,
           basis.sector_id, basis.sector_path, basis.sector_code, basis.sector_name,
           ( SELECT array_agg(DISTINCT ids.id) FROM ( SELECT unnest(basis.data_source_ids) AS id UNION ALL SELECT unnest(esa.data_source_ids) AS id) AS ids) AS data_source_ids,
           ( SELECT array_agg(DISTINCT codes.code) FROM ( SELECT unnest(basis.data_source_codes) AS code UNION ALL SELECT unnest(esa.data_source_codes) AS code) AS codes) AS data_source_codes,
           basis.legal_form_id, basis.legal_form_code, basis.legal_form_name,
           basis.physical_address_part1, basis.physical_address_part2, basis.physical_address_part3,
           basis.physical_postcode, basis.physical_postplace,
           basis.physical_region_id, basis.physical_region_path, basis.physical_region_code,
           basis.physical_country_id, basis.physical_country_iso_2,
           basis.physical_latitude, basis.physical_longitude, basis.physical_altitude,
           basis.domestic,
           basis.postal_address_part1, basis.postal_address_part2, basis.postal_address_part3,
           basis.postal_postcode, basis.postal_postplace,
           basis.postal_region_id, basis.postal_region_path, basis.postal_region_code,
           basis.postal_country_id, basis.postal_country_iso_2,
           basis.postal_latitude, basis.postal_longitude, basis.postal_altitude,
           basis.web_address, basis.email_address, basis.phone_number,
           basis.landline, basis.mobile_number, basis.fax_number,
           basis.unit_size_id, basis.unit_size_code,
           basis.status_id, basis.status_code, basis.used_for_counting,
           basis.last_edit_comment, basis.last_edit_by_user_id, basis.last_edit_at,
           basis.invalid_codes, basis.has_legal_unit,
           COALESCE(esa.related_establishment_ids, ARRAY[]::integer[]) AS related_establishment_ids,
           COALESCE(esa.excluded_establishment_ids, ARRAY[]::integer[]) AS excluded_establishment_ids,
           COALESCE(esa.included_establishment_ids, ARRAY[]::integer[]) AS included_establishment_ids,
           ARRAY[basis.src_unit_id] AS related_legal_unit_ids,
           ARRAY[]::integer[] AS excluded_legal_unit_ids,
           CASE WHEN basis.used_for_counting THEN ARRAY[basis.src_unit_id] ELSE '{}'::integer[] END AS included_legal_unit_ids,
           CASE WHEN basis.enterprise_id IS NOT NULL THEN ARRAY[basis.enterprise_id] ELSE ARRAY[]::integer[] END AS related_enterprise_ids,
           ARRAY[]::integer[] AS excluded_enterprise_ids,
           ARRAY[]::integer[] AS included_enterprise_ids,
           basis.legal_unit_id, basis.enterprise_id, basis.primary_for_enterprise,
           basis.stats,
           CASE WHEN basis.used_for_counting THEN COALESCE(public.jsonb_stats_summary_merge(esa.stats_summary, basis.stats_summary), basis.stats_summary, esa.stats_summary, '{}'::jsonb) ELSE '{}'::jsonb END AS stats_summary
          FROM basis
            LEFT JOIN establishment_aggs AS esa ON esa.unit_id = basis.src_unit_id AND esa.valid_from = basis.src_valid_from
