-- Migration 20260320115108: fix_target_pg_type_default_and_create_function
--
-- The previous migration (20260320105936) added target_pg_type NOT NULL but only
-- updated synchronize_definition_step_mappings(). This fixes the other INSERT path
-- (create_source_and_mappings_for_definition) and adds a DEFAULT so that any INSERT
-- omitting the column gets 'TEXT' — the correct default for user-supplied source data.
BEGIN;

-- 1. Add DEFAULT so existing direct INSERTs (tests, samples, custom definitions)
--    don't violate NOT NULL.
ALTER TABLE public.import_source_column ALTER COLUMN target_pg_type SET DEFAULT 'TEXT';

-- 2. Update create_source_and_mappings_for_definition to populate target_pg_type
--    using the same internal-sibling lookup pattern as synchronize_definition_step_mappings.
...
BEGIN
    ...
    FOREACH v_col_name IN ARRAY p_source_columns LOOP
        ...
        -- NOTE: SELECT INTO sets variable to NULL when 0 rows returned,
        -- so we must use COALESCE to preserve the 'TEXT' default.
        SELECT COALESCE(idc_int.column_type, 'TEXT') INTO v_target_pg_type
        FROM public.import_definition_step ds
        JOIN public.import_data_column idc_int ON ds.step_id = idc_int.step_id
        WHERE ds.definition_id = p_definition_id
          AND idc_int.purpose = 'internal'
          AND idc_int.column_name = v_col_name;
        IF NOT FOUND THEN
            v_target_pg_type := 'TEXT';
        END IF;

        INSERT INTO public.import_source_column (definition_id, column_name, priority, target_pg_type)
        VALUES (p_definition_id, v_col_name, v_priority, v_target_pg_type)
    ...
    IF v_has_stat_step THEN
        ...
        -- target_pg_type uses the column DEFAULT ('TEXT'); the sync procedure
        -- will later update it to the correct type from the internal column.
...
-- 3. Fix the same SELECT INTO bug in synchronize_definition_step_mappings
--    (added in 20260320105936 but with the same PL/pgSQL SELECT INTO pitfall).
...
BEGIN
    ...
    LOOP
        -- Look up sibling internal column's type (best-effort)
        -- NOTE: SELECT INTO sets variable to NULL when 0 rows returned,
        -- so we must use COALESCE and check FOUND to preserve the default.
        SELECT COALESCE(idc_int.column_type, 'TEXT') INTO v_target_pg_type
