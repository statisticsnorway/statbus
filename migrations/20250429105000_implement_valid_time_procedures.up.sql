...
CREATE OR REPLACE PROCEDURE admin.analyse_valid_time_from_context(p_job_id INT, p_batch_ctids TID[], p_step_code TEXT)
...
CREATE OR REPLACE PROCEDURE admin.analyse_valid_time_from_source(p_job_id INT, p_batch_ctids TID[], p_step_code TEXT)
...
BEGIN
    ...
    v_sql := format($$
        ...
        SELECT
            ctid, -- Select ctid from the main data table
            jsonb_strip_nulls(
        ...
        FROM public.%I
        WHERE ctid = ANY(%L) -- Filter by ctid from the main data table
     $$, v_data_table_name, p_batch_ctids);
    ...
    SELECT array_agg(data_ctid) INTO v_error_ctids FROM temp_batch_errors WHERE error_jsonb != '{}'::jsonb; -- Corrected to data_ctid
