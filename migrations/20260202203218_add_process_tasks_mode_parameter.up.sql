-- Migration 20260202203218: add_process_tasks_mode_parameter
BEGIN;

-- Create enum for process_tasks mode parameter (type-safe)
CREATE TYPE worker.process_mode AS ENUM ('top', 'child');

-- Drop the old procedure signature (different parameter list)
DROP PROCEDURE IF EXISTS worker.process_tasks(integer, integer, text, bigint);

-- Add p_mode parameter to worker.process_tasks()
-- Modes:
--   'top'  : Only process top-level tasks. Return immediately if a waiting parent exists.
--   'child': Only process children of waiting parents. Return immediately if no waiting parent.
--   NULL   : Current behavior (backward compatible for tests)
--
-- This enables structured concurrency where:
--   - 1 "top" fiber picks top-level tasks and enters 'waiting' when children spawn
--   - N "child" fibers process children in parallel
--   - When children complete, parent can continue
...
CREATE OR REPLACE PROCEDURE worker.process_tasks(
    ...
    IN p_mode worker.process_mode DEFAULT NULL  -- NEW: 'top', 'child', or NULL
)
 LANGUAGE plpgsql
AS $process_tasks$
DECLARE
...
Parameters:
  p_batch_size: Maximum number of tasks to process in one call
  p_max_runtime_ms: Maximum runtime in milliseconds
  p_queue: Queue name to process (NULL = all queues)
  p_max_priority: Maximum priority level to process
  p_mode: Processing mode for structured concurrency (worker.process_mode enum):
    - ''top'': Only process top-level tasks. Returns if a waiting parent exists.
    - ''child'': Only process children of waiting parents. Returns if no waiting parent.
    - NULL: Original behavior (backward compatible)

Structured Concurrency Model:
  - 1 "top" fiber picks top-level tasks and enters ''waiting'' when children spawn
  - N "child" fibers process children in parallel
  - When all children complete, parent transitions to ''completed''
  - Top fiber then continues with next top-level task or continuation';
