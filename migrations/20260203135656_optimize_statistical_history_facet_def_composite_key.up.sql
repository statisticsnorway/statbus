-- Migration 20260203135656: optimize_statistical_history_facet_def_composite_key
--
-- PERF: Use composite key for hash join instead of IS NOT DISTINCT FROM
--
-- Problem: The join between demographics and stats_by_facet used IS NOT DISTINCT FROM
-- on 8 nullable columns. PostgreSQL could only hash on unit_type, then had to filter
-- 138M rows using the other 8 conditions as a Join Filter.
--
-- Solution: Create a composite text key from all 9 facet dimensions (unit_type + 8 nullable fields),
-- using COALESCE to convert NULLs to empty strings. Then join on a single string equality.
--
-- Result: 15.2s -> 1.0s (15x speedup)
-- - Original: Hash Join with 138M rows removed by Join Filter
-- - Optimized: Hash Join with pure equality on facet_key, no filter needed
...
BEGIN
    ...
    WITH
    units_in_period AS (
        -- PERF: Select only needed columns to reduce memory usage
        SELECT 
    ...
    -- This avoids expensive ROW field extraction in comparisons
    changed_units AS (
        SELECT
    ...
    -- Demographics with composite facet key for hash join
    demographics AS (
        SELECT
    ...
    -- PERF: Final join using composite key for pure hash equality
    -- This replaces the 8-column IS NOT DISTINCT FROM with a single string equality
    SELECT
