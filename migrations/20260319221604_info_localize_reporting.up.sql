-- Migration 20260319221604: info_localize_reporting
--
-- Info Principle: report what THIS invocation did, no extra calculations,
-- keep info close to where it originates. Mostly comes for free.
--
-- Changes:
-- 1. import_job_process: add current_step (free from job row),
--    move rows_processed to finished transition (was per-batch delta)
-- 2. derive_reports_phase: report valid_from/valid_until/years (free from payload)
-- 3. statistical_unit_flush_staging: report rows_flushed (cheap COUNT on small table)
BEGIN;

-- 1. import_job_process: localize info reporting
...
-- 2. derive_reports_phase: report date range and years (free from payload)
...
-- 3. statistical_unit_flush_staging: report rows_flushed (cheap COUNT on small staging table)
