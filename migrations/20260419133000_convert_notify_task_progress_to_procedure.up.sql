-- Migration 20260419133000: Convert notify_task_progress from function to procedure
--
-- The before_procedure hook in worker.command_registry dispatches via
-- EXECUTE format('CALL %s()', ...) which requires a PROCEDURE. But
-- notify_task_progress was a FUNCTION (RETURNS void). This caused:
--   "Error in before_procedure worker.notify_task_progress:
--    worker.notify_task_progress() is not a procedure"
--
-- Fix: convert to PROCEDURE and change the direct PERFORM call in
-- process_tasks to CALL.

BEGIN;

-- 1. Drop the function (can't CREATE OR REPLACE across function→procedure)
DROP FUNCTION IF EXISTS worker.notify_task_progress();

-- 2. Recreate as procedure (same body, no RETURNS void)
...
COMMENT ON PROCEDURE worker.notify_task_progress IS
    'Sends pipeline progress notifications via pg_notify. Called as a before_procedure '
    'hook on derive_units_phase and derive_reports_phase, and directly after each '
    'analytics task completion. Converted from function to procedure (2026-04-19) '
    'because before_procedure dispatch uses CALL, which requires a procedure.';

-- 3. Update process_tasks: change PERFORM to CALL for the direct invocation.
--    The only change from the current body is line 335:
--      PERFORM worker.notify_task_progress()  →  CALL worker.notify_task_progress()
