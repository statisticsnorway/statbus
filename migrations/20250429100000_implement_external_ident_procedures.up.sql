...
BEGIN
    ...
    v_sql := format($$
        ...
        WHERE dt.row_id = ru.data_row_id
          AND dt.row_id = ANY(%L) AND dt.row_id != ALL(%L); -- Update only non-error rows from the original batch that had a resolved unit
    ...
    v_where_clause_step5 := format('dt.row_id = ANY(%L) AND dt.row_id != ALL(%L)', p_batch_row_ids, COALESCE(v_error_row_ids, ARRAY[]::BIGINT[]));
