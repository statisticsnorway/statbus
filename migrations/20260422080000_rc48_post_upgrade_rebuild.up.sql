...
-- Migration 20260422080000: rc.48 post-upgrade rebuild + NULL=everything receiver
--
-- Closes three gaps exposed on dev + demo after rc.42 shipped:
--
--   1. rc.42 dropped public.statistical_history (CASCADE) and truncated
--      statistical_history_facet on the assumption that "the next derive
--      trigger tick" would repopulate them. It does not — triggers fire on
--      base-table DML, not on schema DDL — so the tables stayed empty on
--      every upgraded install that had pre-existing data.
--
--   2. worker.derive_statistical_unit invoked public.*_used_derive() before
--      worker.statistical_unit_flush_staging published activity/region/etc.
--      paths to public.statistical_unit. On a first-import cycle the
--      *_used_def views returned 0 rows, MERGE inserted 0, and *_used stayed
--      empty. Subsequent imports masked this; a single-import dev did not.
--
--   3. Since 2026-03-25, worker.reset_abandoned_processing_tasks has spawned
--      collect_changes with a payload the receiver ignored entirely — so
--      crash recovery silently did nothing.
--
-- The fix is coordinated in six blocks:
--
--   A + D. worker.command_collect_changes grows a direct-mode branch keyed on
--          payload-key presence. Callers that supply any *_id_ranges key
--          (even with NULL value) bypass the base_change_log drain and
--          synthesise ids from base tables when NULL. A new serial child
--          spawn for derive_used_tables is appended to derive_units_phase.
--
--   B.    worker.derive_statistical_unit (function) loses its six inline
--          *_used_derive() calls. It becomes a pure spawner — side effects
--          on *_used tables move out.
--
--   C.    worker.derive_used_tables is a new handler that runs the six
--          *_used_derive() calls AFTER statistical_unit_flush_staging has
--          published paths. Registered in worker.command_registry.
--
--   E.    worker.reset_abandoned_processing_tasks emits a payload of the
--          new shape (all-NULL *_id_ranges keys). Crash recovery now
--          routes through the direct-mode branch and actually triggers a
--          full rebuild.
--
--   F.    A one-shot DO block at the end of the migration spawns the rebuild
--          on any install that has pre-existing data (EXISTS guard skips
--          fresh installs).
--
-- Discrimination by JSONB key PRESENCE, not value:
--   * trigger-emitted payload        = '{"command":"collect_changes"}'
--     → no *_id_ranges keys → drain-log mode (unchanged).
--   * migration / crash-recovery     = jsonb_build_object('establishment_id_ranges', NULL, ...)
--     → keys present, NULL values → direct-mode full rebuild.
--   * scoped direct (future users)   = '{"establishment_id_ranges": "{[1,10)}", ...}'
--     → keys present, non-NULL values → direct-mode scoped rebuild.

BEGIN;

-- =====================================================================
-- BLOCK A + D. worker.command_collect_changes:
--   A — direct-mode branch on JSONB key presence.
--   D — third serial child (derive_used_tables) under derive_units_phase.
-- Base body dumped from 20260319124229:421-573 with the changes noted.
-- =====================================================================
...
-- =====================================================================
-- BLOCK B. worker.derive_statistical_unit (function):
--   drop inline *_used_derive() calls. Side effects move to derive_used_tables.
-- Base body dumped from 20260422000000:1506-1770, minus lines 1754-1759.
-- =====================================================================
...
-- =====================================================================
-- BLOCK C. worker.derive_used_tables: new handler.
-- Runs after statistical_unit_flush_staging, so *_used_def views see
-- populated *_path columns. MERGE semantics unchanged per call site.
-- =====================================================================
...
VALUES (
    ...
    'Refresh *_used tables from populated public.statistical_unit paths (runs after flush_staging)'
)
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_tasks_derive_used_tables_dedup
    ON worker.tasks (command)
    WHERE command = 'derive_used_tables' AND state = 'pending'::worker.task_state;


-- =====================================================================
-- BLOCK E. worker.reset_abandoned_processing_tasks: payload shape fix.
-- Base body dumped from 20260325114130:19-93; only the spawn payload
-- changes. Crash recovery now routes through the direct-mode branch.
-- =====================================================================
...
-- =====================================================================
-- BLOCK F. One-shot post-upgrade rebuild.
-- Fresh install has no base data → the EXISTS guard is false → no-op.
-- Every subsequent import fires triggers normally. On any upgraded
-- install with pre-existing data (dev, demo, rune), this fires once
-- and rebuilds the full derived-state tree via the direct-mode branch.
-- =====================================================================

DO $rc48_post_upgrade_rebuild$
BEGIN
    IF EXISTS (
        SELECT 1 FROM public.establishment
        UNION ALL
        SELECT 1 FROM public.legal_unit
        UNION ALL
        SELECT 1 FROM public.enterprise
        UNION ALL
        SELECT 1 FROM public.power_group
        LIMIT 1
    ) THEN
        PERFORM worker.spawn(
            p_command => 'collect_changes',
