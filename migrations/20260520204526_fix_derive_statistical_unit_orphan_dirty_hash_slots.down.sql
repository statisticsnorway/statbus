-- Down Migration 20260520204526: fix derive_statistical_unit orphan dirty hash slots
--
-- Restores worker.derive_statistical_unit to its pre-fix body, captured
-- byte-for-byte from \sf at the time the up migration was authored
-- (tmp/derive_statistical_unit.sf.sql). The four INSERT INTO
-- statistical_unit_facet_dirty_hash_slots additions inside the orphan-
-- cleanup IF blocks are removed; everything else is identical.
