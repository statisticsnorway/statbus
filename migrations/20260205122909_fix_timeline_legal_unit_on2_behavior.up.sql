-- Migration: Fix O(n²) behavior in timeline_legal_unit refresh
--
-- Problem: The timeline_legal_unit_def view's LATERAL join against timeline_establishment
-- scans the ENTIRE timeline_establishment table for each row, causing O(n²) behavior.
-- As timeline_establishment grows during import, each batch gets progressively slower:
--   - Early batch (12s): 120,130 rows × 4,924 loops = 591M evaluations
--   - Late batch (40s):  408,442 rows × 4,924 loops = 2B evaluations
--
-- Root cause: The temporal overlap condition `from_until_overlaps()` uses GIST index
-- which scans all rows matching the temporal range, regardless of the legal_unit_id filter.
-- PostgreSQL can't effectively combine the btree (legal_unit_id) and GIST (temporal) scans.
--
-- Solution: For partial refresh, pre-materialize the relevant timeline_establishment rows
-- into a temp table BEFORE the main join. This changes:
--   O(n × T) where T = total timeline_establishment rows
-- to:
--   O(n × k) where k = avg establishments per legal unit (~constant)
BEGIN;

-- ============================================================================
-- Modify timeline_legal_unit_refresh to use pre-materialized temp table
-- ============================================================================
