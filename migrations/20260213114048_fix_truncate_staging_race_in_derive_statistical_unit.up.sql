-- Migration 20260213114048: fix_truncate_staging_race_in_derive_statistical_unit
--
-- Fix latent race condition: TRUNCATE staging at the start of derive_statistical_unit
-- could destroy staged data from a previous cycle's batch children if a new
-- collect_changes → derive_statistical_unit started before flush_staging completed.
--
-- The TRUNCATE is unnecessary because:
-- 1. Batch children already do DELETE FROM staging WHERE unit_type/unit_id before inserting
...
--
-- Also removes v_uncle_priority which was dead code — allocated via nextval()
-- but never passed to any function (enqueue functions use DEFAULT sequence values).
BEGIN;
