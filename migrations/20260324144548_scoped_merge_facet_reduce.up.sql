...
-- Adds scoped MERGE to both facet reduce procedures so that incremental
-- refreshes only touch the dimension combos affected by dirty partitions,
-- while full refreshes keep the existing TRUNCATE + INSERT path.
BEGIN;
...
BEGIN
    ...
    ELSE
        ---------------------------------------------------------------
        -- Scoped refresh: re-aggregate only affected dim combos, MERGE
        ---------------------------------------------------------------

        -- 1. Re-aggregate dim combos that appear in dirty partitions
        ...
        -- 2. MERGE into final table (COALESCE sentinels for hash-joinable ON)
        ...
        -- 3. Delete dim combos that were in dirty partitions but disappeared
           ...
           AND NOT EXISTS (
                SELECT 1 FROM _scoped_agg AS a
                 WHERE a.valid_from = f.valid_from
                   ...
                   AND COALESCE(a.physical_region_path, '__null__'::ltree) = COALESCE(f.physical_region_path, '__null__'::ltree)
                   AND COALESCE(a.primary_activity_category_path, '__null__'::ltree) = COALESCE(f.primary_activity_category_path, '__null__'::ltree)
                   AND COALESCE(a.sector_path, '__null__'::ltree) = COALESCE(f.sector_path, '__null__'::ltree)
...
BEGIN
    ...
    ELSE
        ...
        SELECT s.resolution, s.year, s.month, s.unit_type,
         ...
         WHERE (s.resolution, s.year, COALESCE(s.month, -1), s.unit_type,
                COALESCE(s.primary_activity_category_path::text, ''),
                ...
                COALESCE(s.status_id, -1))
            IN (
                SELECT d.resolution, d.year, COALESCE(d.month, -1), d.unit_type,
                       COALESCE(d.primary_activity_category_path::text, ''),
        ...
        DELETE FROM public.statistical_history_facet AS f
         WHERE (f.resolution, f.year, COALESCE(f.month, -1), f.unit_type,
                COALESCE(f.primary_activity_category_path::text, ''),
           ...
           AND NOT EXISTS (
                SELECT 1 FROM _scoped_history_agg AS a
                 WHERE a.resolution = f.resolution
                   ...
                   AND a.unit_type = f.unit_type
                   AND COALESCE(a.primary_activity_category_path, '__null__'::ltree) = COALESCE(f.primary_activity_category_path, '__null__'::ltree)
                   AND COALESCE(a.secondary_activity_category_path, '__null__'::ltree) = COALESCE(f.secondary_activity_category_path, '__null__'::ltree)
                   AND COALESCE(a.sector_path, '__null__'::ltree) = COALESCE(f.sector_path, '__null__'::ltree)
                   AND COALESCE(a.legal_form_id, -1) = COALESCE(f.legal_form_id, -1)
                   AND COALESCE(a.physical_region_path, '__null__'::ltree) = COALESCE(f.physical_region_path, '__null__'::ltree)
                   AND COALESCE(a.physical_country_id, -1) = COALESCE(f.physical_country_id, -1)
