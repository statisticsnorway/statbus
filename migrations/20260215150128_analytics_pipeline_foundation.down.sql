-- Down Migration 20260215150128: analytics_pipeline_foundation
BEGIN;

-- 3. Restore derive_statistical_unit to previous version (dirty tracking on explicit IDs only)
...
-- 2. Drop pipeline progress table
DROP TABLE IF EXISTS worker.pipeline_progress;

-- 1. Remove report_partition_count from settings
ALTER TABLE public.settings DROP COLUMN IF EXISTS report_partition_count;
