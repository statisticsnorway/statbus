...
BEGIN
    ...
    CASE WHEN scope IN ('all') THEN
        ...
        INSERT INTO public.stat_definition(code, type, frequency, name, description, priority, archived)
        VALUES 
            ('employees', 'int', 'yearly', 'Employees', 'The number of people receiving an official salary with government reporting.', 1, false),
            ('turnover', 'float', 'yearly', 'Turnover', 'The amount (Local Currency)', 2, false);
    ELSE END CASE;
    ...
    CASE WHEN scope IN ('all') THEN
        ...
        VALUES 
            ('tax_ident', 'Tax Identifier', 1, 'Stable and country unique identifier used for tax reporting.', false),
            ('stat_ident', 'Statistical Identifier', 2, 'Stable identifier generated by Statbus', false);
    ELSE END CASE;
