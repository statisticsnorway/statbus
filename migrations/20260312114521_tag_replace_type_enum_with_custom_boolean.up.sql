...
-- Replace tag.type enum with tag.custom boolean to match all other classification tables.
-- Other classifications (sector, legal_form, country, etc.) use custom+enabled booleans.

-- Step 1: Add custom column and populate from type
ALTER TABLE public.tag ADD COLUMN custom boolean;
UPDATE public.tag SET custom = (type = 'custom');
ALTER TABLE public.tag ALTER COLUMN custom SET NOT NULL;
ALTER TABLE public.tag ALTER COLUMN custom SET DEFAULT false;

-- Step 2: Drop type column, index, and enum
DROP INDEX IF EXISTS ix_tag_type;
ALTER TABLE public.tag DROP COLUMN type;
DROP TYPE public.tag_type;
CREATE INDEX ix_tag_custom ON public.tag USING btree (custom);

-- Step 3: Update reset function to use WHERE custom instead of WHERE type = 'custom'
-- Also add the standard re-enable pattern for system tags (same as other classifications).
...
BEGIN
    ...
    CASE WHEN scope IN ('units', 'data', 'getting-started', 'all') THEN
        -- Count all tables before truncation.
        -- TRUNCATE checks FK constraints at DDL level, so all FK-related tables
        -- must be truncated in one statement. We collect counts first.
        ...
        -- Single TRUNCATE for all base_data, link, and asset tables.
        -- Required because TRUNCATE checks FK constraints at DDL level:
        -- e.g. person_for_unit FK→person, legal_relationship FK→legal_unit,
        ...
        TRUNCATE
            -- base_data (temporal FKs to establishment/legal_unit)
            public.activity,
            public.location,
            public.contact,
            public.stat_for_unit,
            public.external_ident,
            -- link (FKs to establishment/legal_unit/person/enterprise/tag)
            public.person_for_unit,
            public.person,
            public.tag_for_unit,
            public.unit_notes,
            -- base_data (relationship/hierarchy, FK to legal_unit/power_group)
            public.legal_relationship,
            public.power_root,
            -- base_data (core units, FK chain: establishment→legal_unit→enterprise)
            public.establishment,
            public.legal_unit,
            public.enterprise,
            public.power_group,
            -- asset (FK from legal_unit/establishment)
            public.image,
            -- derived (computed outputs, no delete-side triggers)
    ...
    CASE WHEN scope IN ('all') THEN
        -- Classification: uses custom boolean, same pattern as other classifications.
        WITH deleted_tag AS (
