...
-- For partial refresh, materializes power_group_membership into a temp table
-- to avoid re-evaluating the view (UNION DISTINCT + NOT EXISTS on legal_relationship)
-- in each LATERAL iteration. Without this: 42 min for 201 PGs. With this: <100ms.
...
-- enqueue_derive_power_groups removed: derive_power_groups no longer exists.
-- Power group analytics now handled directly by derive_statistical_unit.
...
BEGIN
    ...
    -- Incremental year insert: only add years from this batch's timesegments (<1ms).
    -- Replaces timesegments_years_refresh_concurrent() which scans all 3.1M timesegments (~4s).
    -- Years are never deleted during batch processing; full refresh handles deletion.
    ...
    WHERE (  (v_establishment_id_ranges IS NOT NULL AND t.unit_type = 'establishment' AND t.unit_id <@ v_establishment_id_ranges)
          OR (v_legal_unit_id_ranges IS NOT NULL AND t.unit_type = 'legal_unit' AND t.unit_id <@ v_legal_unit_id_ranges)
          OR (v_enterprise_id_ranges IS NOT NULL AND t.unit_type = 'enterprise' AND t.unit_id <@ v_enterprise_id_ranges)
          OR (v_power_group_id_ranges IS NOT NULL AND t.unit_type = 'power_group' AND t.unit_id <@ v_power_group_id_ranges)
          )
...
-- SECTION 7: derive_power_groups REMOVED
-- Power group creation/linking now happens in process_power_group_link (import holistic step).
-- Power root population also happens in process_power_group_link.
-- derive_power_groups worker command no longer exists.
...
-- SECTION 16: REMOVED (derive_power_groups no longer exists to unlink from pipeline)
