...
CREATE INDEX ix_contact_legal_unit_id_valid_range ON public.contact USING gist (legal_unit_id, valid_range);
CREATE INDEX ix_contact_establishment_id_valid_range ON public.contact USING gist (establishment_id, valid_range);
CREATE INDEX ix_contact_valid_range ON public.contact USING gist (valid_range);
...
CREATE INDEX ON contact (establishment_id, legal_unit_id);
CREATE INDEX ON contact (establishment_id) WHERE legal_unit_id IS NULL;
CREATE INDEX ON contact (legal_unit_id) WHERE establishment_id IS NULL;
...
SELECT sql_saga.add_era('public.contact', 'valid_range', 'valid', ephemeral_columns => ARRAY['edit_comment', 'edit_by_user_id', 'edit_at']);
