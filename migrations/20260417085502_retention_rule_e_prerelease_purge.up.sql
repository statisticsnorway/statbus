-- Migration 20260417085502: Add Rule E to upgrade_retention_plan
--
-- Rule E: just-installed prerelease -> purge same-family prereleases in
-- non-evidence states. Symmetric to Rule A (which fires on release install)
-- but targets the prerelease-only deployment pattern where no release is
-- ever installed. Without this rule, superseded prereleases accumulate
-- indefinitely on prerelease-channel servers until Rule D's generous
-- time_cap + count_cap thresholds are both exceeded.
--
-- This is a CREATE OR REPLACE on the existing function — adds the new CTE
-- and includes it in the final UNION ALL.
...
COMMENT ON FUNCTION public.upgrade_retention_plan(text, integer) IS
    ...
    'rows that upgrade_retention_apply will delete. Rules A-D from v2, Rule E added for prerelease-to-prerelease purge.';
