...
-- =============================================================================
-- UP MIGRATION: Bubble up children info to parent + INOUT handler protocol
--
-- Two related changes in one migration:
-- 1. complete_parent_if_ready: Aggregate children's info into parent's info
--    when a parent task completes (all children done). Numeric keys are summed
--    across children; non-numeric keys take last child's value.
-- 2. INOUT p_info: All 20 handler procedures get INOUT p_info jsonb.
--    process_tasks captures it and writes to worker.tasks.info in its existing
--    post-handler UPDATE, eliminating 3 redundant per-handler UPDATEs.
-- =============================================================================
...
-- =====================================================================
-- 1. Function changes (must come before procedures that call them)
-- =====================================================================

-- Must drop procedure first (depends on function), then function (changing return type)
DROP PROCEDURE IF EXISTS worker.derive_statistical_unit(jsonb);
...
BEGIN
    ...
    -- Return info instead of doing UPDATE worker.tasks
...
-- =====================================================================
-- 2. Drop old single-arg overloads before creating two-arg replacements
-- =====================================================================
-- Without this, CREATE OR REPLACE adds a second overload instead of replacing.
-- derive_statistical_unit already dropped above (line 122) due to return type change.
DROP PROCEDURE IF EXISTS worker.command_collect_changes(jsonb);
DROP PROCEDURE IF EXISTS admin.import_job_process(jsonb);
...
DROP PROCEDURE IF EXISTS worker.command_import_job_cleanup(jsonb);
DROP PROCEDURE IF EXISTS worker.command_task_cleanup(jsonb);

-- =====================================================================
-- 3. Handlers that write info (4) — replace UPDATE with p_info assignment
-- =====================================================================

-- worker.command_collect_changes: assign p_info instead of UPDATE
...
-- worker.derive_statistical_unit (procedure): capture function return into p_info
CREATE OR REPLACE PROCEDURE worker.derive_statistical_unit(IN payload jsonb, INOUT p_info jsonb DEFAULT NULL)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'worker', 'pg_temp'
...
-- admin.import_job_process (jsonb wrapper): forward INOUT to integer overload
CREATE OR REPLACE PROCEDURE admin.import_job_process(IN payload jsonb, INOUT p_info jsonb DEFAULT NULL)
 LANGUAGE plpgsql
...
-- admin.import_job_process (integer): assign p_info instead of UPDATE
CREATE OR REPLACE PROCEDURE admin.import_job_process(IN job_id integer, INOUT p_info jsonb DEFAULT NULL)
 LANGUAGE plpgsql
...
BEGIN
    ...
    -- Return info via INOUT instead of UPDATE worker.tasks
    SELECT * INTO job FROM public.import_job WHERE id = job_id;
    p_info := jsonb_build_object(
...
-- worker.statistical_unit_refresh_batch: add batch counts to p_info (NEW info)
CREATE OR REPLACE PROCEDURE worker.statistical_unit_refresh_batch(IN payload jsonb, INOUT p_info jsonb DEFAULT NULL)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'worker', 'pg_temp'
...
-- =====================================================================
-- 3. Remaining 16 handlers: mechanical INOUT addition (body unchanged)
-- =====================================================================

CREATE OR REPLACE PROCEDURE worker.derive_units_phase(IN payload jsonb, INOUT p_info jsonb DEFAULT NULL)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'worker', 'pg_temp'
...
CREATE OR REPLACE PROCEDURE worker.derive_reports_phase(IN payload jsonb, INOUT p_info jsonb DEFAULT NULL)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'worker', 'pg_temp'
...
CREATE OR REPLACE PROCEDURE worker.derive_reports(IN payload jsonb, INOUT p_info jsonb DEFAULT NULL)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'worker', 'pg_temp'
AS $derive_reports$
...
CREATE OR REPLACE PROCEDURE worker.statistical_unit_flush_staging(IN payload jsonb, INOUT p_info jsonb DEFAULT NULL)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'worker', 'pg_temp'
...
CREATE OR REPLACE PROCEDURE worker.derive_statistical_history(IN payload jsonb, INOUT p_info jsonb DEFAULT NULL)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'worker', 'pg_temp'
...
CREATE OR REPLACE PROCEDURE worker.derive_statistical_history_period(IN payload jsonb, INOUT p_info jsonb DEFAULT NULL)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'worker', 'pg_temp'
...
CREATE OR REPLACE PROCEDURE worker.statistical_history_reduce(IN payload jsonb, INOUT p_info jsonb DEFAULT NULL)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'worker', 'pg_temp'
...
CREATE OR REPLACE PROCEDURE worker.derive_statistical_unit_facet(IN payload jsonb, INOUT p_info jsonb DEFAULT NULL)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'worker', 'pg_temp'
...
CREATE OR REPLACE PROCEDURE worker.derive_statistical_unit_facet_partition(IN payload jsonb, INOUT p_info jsonb DEFAULT NULL)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'worker', 'pg_temp'
...
CREATE OR REPLACE PROCEDURE worker.statistical_unit_facet_reduce(IN payload jsonb, INOUT p_info jsonb DEFAULT NULL)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'worker', 'pg_temp'
...
CREATE OR REPLACE PROCEDURE worker.derive_statistical_history_facet(IN payload jsonb, INOUT p_info jsonb DEFAULT NULL)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'worker', 'pg_temp'
...
CREATE OR REPLACE PROCEDURE worker.derive_statistical_history_facet_period(IN payload jsonb, INOUT p_info jsonb DEFAULT NULL)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'worker', 'pg_temp'
...
CREATE OR REPLACE PROCEDURE worker.statistical_history_facet_reduce(IN payload jsonb, INOUT p_info jsonb DEFAULT NULL)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'worker', 'pg_temp'
...
CREATE OR REPLACE PROCEDURE worker.command_import_job(IN p_payload jsonb, INOUT p_info jsonb DEFAULT NULL)
 LANGUAGE plpgsql
...
CREATE OR REPLACE PROCEDURE worker.command_import_job_cleanup(IN payload jsonb, INOUT p_info jsonb DEFAULT NULL)
 LANGUAGE plpgsql
 SET search_path TO 'public', 'worker', 'pg_temp'
...
CREATE OR REPLACE PROCEDURE worker.command_task_cleanup(IN payload jsonb, INOUT p_info jsonb DEFAULT NULL)
 LANGUAGE plpgsql
 SET search_path TO 'public', 'worker', 'pg_temp'
...
-- =====================================================================
-- 4. worker.process_tasks: INOUT capture + info in post-handler UPDATE
-- =====================================================================
