-- Migration 20260203221612: move_analyze_after_batch_seq_assignment
BEGIN;

-- PERFORMANCE FIX: Move ANALYZE to AFTER batch_seq assignment.
--
-- Previously, ANALYZE ran BEFORE batch_seq was assigned, so the planner saw
-- batch_seq = NULL for all rows. This caused the planner to estimate rows=1
-- for WHERE batch_seq = $1, leading to Nested Loop instead of Hash Join.
--
-- Result: 53 million rows removed by join filter (21 seconds per batch!)
--
-- By moving ANALYZE to after batch_seq assignment, the planner has correct
-- statistics and chooses Hash Join instead (175ms per batch).
