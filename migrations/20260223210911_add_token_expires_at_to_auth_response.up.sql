-- Migration 20260223210911: add_token_expires_at_to_auth_response
BEGIN;

-- Add token_expires_at to auth.auth_response composite type
ALTER TYPE auth.auth_response ADD ATTRIBUTE token_expires_at timestamptz;

-- Drop the old 3-parameter overload before creating the new 4-parameter version.
-- CREATE OR REPLACE with a different signature creates a new overload, causing
-- "function is not unique" errors when called with named parameters.
DROP FUNCTION auth.build_auth_response(auth."user", boolean, auth.login_error_code);

-- Update build_auth_response to accept and set token_expires_at
CREATE OR REPLACE FUNCTION auth.build_auth_response(p_user_record auth."user" DEFAULT NULL::auth."user", p_expired_access_token_call_refresh boolean DEFAULT false, p_error_code auth.login_error_code DEFAULT NULL::auth.login_error_code, p_token_expires_at timestamptz DEFAULT NULL)
...
-- Update auth_status to extract exp from JWT claims and pass token_expires_at
...
-- Update login to include token_expires_at
...
-- Update refresh to include token_expires_at
