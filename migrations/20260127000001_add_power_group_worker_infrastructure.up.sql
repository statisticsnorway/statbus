...
WITH RECURSIVE hierarchy AS (
    ...
    WHERE EXISTS (
        -- Has at least one CONTROLLING child (>= 50% ownership)
        SELECT 1 
        FROM public.legal_relationship AS lr
        WHERE lr.influencing_id = lu.id
          AND lr.percentage >= 50
          AND lr.valid_range && lu.valid_range
    )
    AND NOT EXISTS (
        -- Does NOT have a controlling parent (>= 50% ownership)
        SELECT 1 
        FROM public.legal_relationship AS lr
...
COMMENT ON VIEW public.legal_unit_power_hierarchy IS 
    'Recursive view showing power hierarchy traversal from root legal units down through controlling interests (>= 50% ownership)';
...
WITH hierarchy_relationships AS (
    ...
    WHERE lr.percentage >= 50  -- Only controlling relationships form power groups
)
SELECT 
...
BEGIN
    ...
    -- Step 4: Clear power_group_id from relationships that are no longer controlling (< 50%)
    UPDATE public.legal_relationship AS lr
    ...
    GET DIAGNOSTICS _row_count = ROW_COUNT;
    IF _row_count > 0 THEN
        RAISE DEBUG '[derive_power_groups] Cleared power_group from % non-controlling relationships', _row_count;
    END IF;
...
-- Notification procedures for UI status updates
CREATE PROCEDURE worker.notify_is_deriving_power_groups_start()
LANGUAGE plpgsql AS $procedure$
BEGIN
    PERFORM pg_notify('worker_status', json_build_object('type', 'is_deriving_power_groups', 'status', true)::text);
END;
$procedure$;

CREATE PROCEDURE worker.notify_is_deriving_power_groups_stop()
LANGUAGE plpgsql AS $procedure$
BEGIN
    PERFORM pg_notify('worker_status', json_build_object('type', 'is_deriving_power_groups', 'status', false)::text);
END;
$procedure$;
...
VALUES (
    ...
    'worker.notify_is_deriving_power_groups_start', 
    'worker.notify_is_deriving_power_groups_stop',
