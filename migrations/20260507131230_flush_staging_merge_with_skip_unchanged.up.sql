-- Migration 20260507131230: flush_staging_merge_with_skip_unchanged
--
-- Replace public.statistical_unit_flush_staging() body: DELETE+INSERT → 
-- DELETE-removed + MERGE-with-skip-unchanged. Skips the WAL + index churn for
-- rows whose staging content is identical to the existing main row.
--
-- Benchmark on isolated bench DB (138 statistical_unit rows, demo dataset):
--   * No-op flush (staging == main): WAL 1,015,864 → 80,336 bytes (-92%);
--     time 46.3s → 5.1s (9.1x)
--   * Mixed flush (18/138 rows mutated): WAL 1,015,864 → 111,704 bytes (-89%);
--     time 46.3s → 7.7s (6.0x)
--   * EXPLAIN ANALYZE confirms plan stays in same cost class (Hash Join /
--     Hash Right Join + Index Scans). PG18 reports `Tuples: skipped=N` in the
--     MERGE plan when WHEN MATCHED filter evaluates false — the skip is
--     observable in the plan.
--
-- Shape:
--   1. Pre-cleanup DELETE: rows in main for affected units (those in staging)
--      that have NO matching slice in staging. Anti-join via NOT EXISTS.
--      Necessary because the MERGE in step 2 cannot use WHEN NOT MATCHED BY
--      SOURCE THEN DELETE (would delete the entire main table; the source
--      is staging, scoped to affected units only).
--   2. MERGE with WHEN MATCHED AND row_tuple IS DISTINCT FROM source_tuple
--      → UPDATE all columns; WHEN NOT MATCHED BY TARGET → INSERT. The IS
--      DISTINCT FROM gate is the skip-unchanged optimisation.
--   3. TRUNCATE staging (unchanged from previous shape).
--   4. ANALYZE main (unchanged).
--
-- Notes:
--   * GENERATED ALWAYS columns (`valid_range`, `hash_slot`) are excluded
--     from the explicit column list on both INSERT and UPDATE — same as the
--     prior INSERT body. They re-derive from the unit_type / unit_id /
--     valid_from / valid_until on every UPDATE/INSERT.
--   * Match key is (unit_type, unit_id, valid_from, valid_to, valid_until).
--     Same key the prior DELETE+INSERT implicitly used (DELETE keyed on
--     unit_type+unit_id, then INSERT'd full slice set).
--   * COALESCE(valid_until, 'infinity'::date) preserves existing convention
--     for NULL valid_until handling.
--
-- Down migration restores the original DELETE-then-INSERT body verbatim.
