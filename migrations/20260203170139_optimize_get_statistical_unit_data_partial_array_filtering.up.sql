-- Migration 20260203170139: optimize_get_statistical_unit_data_partial_array_filtering
--
-- PERF: Replace <@ multirange filtering with = ANY(array) in get_statistical_unit_data_partial
--
-- Uses the int4multirange_to_array() helper from the previous migration to convert
-- the multirange parameter to an array, then uses = ANY() for efficient index seeks
-- instead of slow <@ containment scans.
--
-- Benchmark: Function scan time 454ms → 79ms for 4445 establishments (5.7x faster)
...
BEGIN
    IF p_unit_type = 'establishment' THEN
        ...
        WHERE t.unit_id = ANY(v_ids);  -- PERF: = ANY(array) instead of <@ multirange
