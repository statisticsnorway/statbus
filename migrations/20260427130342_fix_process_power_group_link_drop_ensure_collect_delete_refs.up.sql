-- Migration 20260427130342: fix import.process_power_group_link — drop refs to dropped triggers
--
-- cffcf2965 dropped 14 b_*_ensure_collect* triggers (the unconditional
-- worker.ensure_collect_changes() variants) but missed that
-- import.process_power_group_link still ALTERed those triggers by name
-- in DISABLE/ENABLE blocks around its temporal_merge sequence, causing
-- 7 fast-test failures (118, 119, 120, 204, 306, 309, 344) with:
--
--   ERROR: trigger "b_power_root_ensure_collect_delete" for table "power_root" does not exist
--
-- Empirical lesson captured: a migration that drops named triggers must
-- grep all migration bodies AND `pg_get_functiondef` for those names
-- BEFORE landing. Pure pg_trigger probing isn't enough — names appear
-- as STRINGS inside ALTER statements in pl/pgsql bodies.
--
-- Audit (run on the live DB after cffcf2965): of the 14 dropped trigger
-- names, only ONE — b_power_root_ensure_collect_delete — appears in any
-- routine body (import.process_power_group_link). The two sister
-- triggers b_power_root_ensure_collect_insert and _update are kept
-- (they call the specialized worker.ensure_collect_changes_for_power_root
-- which I deliberately preserved in cffcf2965), so their DISABLE/ENABLE
-- lines stay in this fixed body.
--
-- Fix: CREATE OR REPLACE PROCEDURE with the body verbatim from HEAD's
-- live shape MINUS the two `b_power_root_ensure_collect_delete` lines.
-- The `a_power_root_log_*` triggers and the `_insert/_update` ensure
-- triggers all stay — those names still exist in pg_catalog.
--
-- Last-modifying source: migrations/20260310150200_fix_power_group_edit_by_user_id.up.sql
-- (defines the procedure with the same DISABLE/ENABLE block; no later
-- migration touched it before cffcf2965).
BEGIN;
