...
-- Add change-tracking triggers to person_for_unit and tag_for_unit.
-- Both tables feed into statistical unit derivation but changes were silently ignored:
--   person_for_unit: used in es_periods_base/lu_periods_base UNIONs
--   tag_for_unit: used in tag_paths_agg

-- Step 1: Update log_base_change with two new CASE branches
...
-- Step 2: Create triggers for person_for_unit
-- Each event needs its own trigger because transition tables can't span multiple events.

CREATE TRIGGER a_person_for_unit_log_insert
AFTER INSERT ON public.person_for_unit
REFERENCING NEW TABLE AS new_rows
FOR EACH STATEMENT EXECUTE FUNCTION worker.log_base_change();

CREATE TRIGGER a_person_for_unit_log_update
AFTER UPDATE ON public.person_for_unit
REFERENCING OLD TABLE AS old_rows NEW TABLE AS new_rows
FOR EACH STATEMENT EXECUTE FUNCTION worker.log_base_change();

CREATE TRIGGER a_person_for_unit_log_delete
AFTER DELETE ON public.person_for_unit
...
-- Step 3: Create triggers for tag_for_unit

CREATE TRIGGER a_tag_for_unit_log_insert
AFTER INSERT ON public.tag_for_unit
REFERENCING NEW TABLE AS new_rows
FOR EACH STATEMENT EXECUTE FUNCTION worker.log_base_change();

CREATE TRIGGER a_tag_for_unit_log_update
AFTER UPDATE ON public.tag_for_unit
REFERENCING OLD TABLE AS old_rows NEW TABLE AS new_rows
FOR EACH STATEMENT EXECUTE FUNCTION worker.log_base_change();

CREATE TRIGGER a_tag_for_unit_log_delete
AFTER DELETE ON public.tag_for_unit
REFERENCING OLD TABLE AS old_rows
FOR EACH STATEMENT EXECUTE FUNCTION worker.log_base_change();
