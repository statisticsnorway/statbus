-- Down migration 20260427130342: restore the dropped-trigger refs in import.process_power_group_link
--
-- This down migration is meaningful ONLY when cffcf2965 is also rolled
-- back (its down migration restores the b_power_root_ensure_collect_delete
-- trigger). On a system where cffcf2965 is still applied, running this
-- down alone leaves the procedure in a broken state — the ALTER TRIGGER
-- ... b_power_root_ensure_collect_delete statement will fail at the
-- first call. Documented; not defensively coded.
BEGIN;
