-- Migration 20260210121155: optimize_targeted_closed_groups
--
-- For partial refreshes (specific enterprise/LU/establishment IDs), use a targeted
-- recursive BFS from only the affected enterprise IDs instead of computing the full
-- transitive closure over ALL enterprises. On production (1.1M+ enterprises), this
-- reduces derive_statistical_unit from ~93s to ~10ms for single-enterprise changes.
--
-- Full refresh path is unchanged — still uses get_enterprise_closed_groups().
BEGIN;
