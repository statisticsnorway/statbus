-- Down migration 20260507123326: restore the broken-by-design original.
--
-- The original body INSERTs per-slot rows (from
-- public.statistical_history_facet_def) directly into the per-period-dim
-- target without aggregation. This re-introduces the documented
-- UNIQUE-constraint violation on rows that span more than one hash_slot.
-- Restored verbatim for round-trip; do not call the function with this
-- migration in place.
