...
-- Restore review to boolean NOT NULL DEFAULT false.
-- First set any NULLs to false so the NOT NULL constraint can be added.
UPDATE public.import_job SET review = false WHERE review IS NULL;
ALTER TABLE public.import_job ALTER COLUMN review SET DEFAULT false;
ALTER TABLE public.import_job ALTER COLUMN review SET NOT NULL;

-- Restore the original import_job_process procedure with simple boolean review check.
