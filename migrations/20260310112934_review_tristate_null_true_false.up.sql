...
-- Change review from boolean NOT NULL DEFAULT false to nullable with NULL default.
-- NULL = review only if errors found during analysis
-- true = always review
-- false = never review (auto-approve)
ALTER TABLE public.import_job ALTER COLUMN review DROP NOT NULL;
ALTER TABLE public.import_job ALTER COLUMN review SET DEFAULT NULL;

-- Update the import_job_process procedure to handle tri-state review logic.
-- Only the review check (originally: IF job.review THEN) is changed.
