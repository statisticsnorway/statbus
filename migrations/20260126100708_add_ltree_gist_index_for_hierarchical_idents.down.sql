-- Down Migration 20260126100708: add_ltree_gist_index_for_hierarchical_idents
BEGIN;

-- Restore the original procedure without GIST ltree index support
...
-- Restore original cleanup procedure
...
-- Drop any GIST ltree indexes that were created
DO $$
DECLARE
    r RECORD;
BEGIN
    FOR r IN
        ...
        WHERE schemaname = 'public'
          AND tablename = 'statistical_unit'
          AND indexname ILIKE 'su_ei_%_ltree_gist_idx'
    LOOP
        EXECUTE format('DROP INDEX IF EXISTS %I', r.indexname);
        RAISE NOTICE 'Dropped GIST ltree index %', r.indexname;
    END LOOP;
END;
$$;
