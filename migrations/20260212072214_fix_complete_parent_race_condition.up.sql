...
-- Fix race condition in complete_parent_if_ready when concurrent child fibers
-- complete simultaneously. Previously, the parent completion check happened
-- BEFORE COMMIT, so concurrent children couldn't see each other's committed
-- state changes. Now the check happens AFTER COMMIT in a new transaction
-- snapshot where all sibling completions are visible.
--
-- For test transactions (pg_regress), the check remains inline since all
