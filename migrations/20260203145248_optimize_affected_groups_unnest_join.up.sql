-- Migration 20260203145248: optimize_affected_groups_unnest_join
--
-- PERF: Replace "= ANY(array)" pattern with UNNEST + JOIN for 84x speedup
--
-- Problem: The original affected_groups CTE uses:
--   WHERE ae.enterprise_id = ANY(g.enterprise_ids)
-- This causes O(n*m) comparisons where n = affected enterprises, m = total enterprises.
-- With 4924 groups × 4924 enterprises = 24M row comparisons (24,240,852 rows removed by filter).
--
-- Solution: Use UNNEST to create enterprise_id→group_id mapping, then hash/merge join.
-- This enables efficient join algorithms instead of nested loop with filter.
--
-- Result: 2,458ms → 29ms (84x speedup) for the affected_groups query
...
BEGIN
    ...
    FOR v_group IN 
        ...
        -- PERF: Create enterprise_id→group_id mapping via UNNEST instead of = ANY(array)
        -- This enables hash/merge join instead of O(n²) nested loop with filter
        enterprise_to_group AS (
            SELECT g.group_id, eid.enterprise_id
            FROM all_groups g
            CROSS JOIN LATERAL UNNEST(g.enterprise_ids) AS eid(enterprise_id)
        ),
        affected_groups AS (
            SELECT DISTINCT etg.group_id
            FROM enterprise_to_group etg
            JOIN affected_enterprise_ids ae ON ae.enterprise_id = etg.enterprise_id
        )
        SELECT 
            g.group_id,
            g.enterprise_ids,
