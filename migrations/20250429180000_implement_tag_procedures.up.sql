...
CREATE OR REPLACE PROCEDURE admin.analyse_tags(p_job_id INT, p_batch_row_ids BIGINT[], p_step_code TEXT)
...
BEGIN
    ...
    EXCEPTION WHEN others THEN
        ...
        v_sql := format('
            ...
            WHERE dt.row_id = ANY(%L);
        ', v_data_table_name, 'error', SQLERRM, v_step.priority - 1, p_batch_row_ids);
        EXECUTE v_sql;
    ...
    v_sql := format($$
        ...
        WHERE dt.row_id = err.data_row_id AND err.error_jsonb != %L; -- Ensure there's an error to apply
...
CREATE OR REPLACE PROCEDURE admin.process_tags(p_job_id INT, p_batch_row_ids BIGINT[], p_step_code TEXT)
