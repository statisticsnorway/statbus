...
WITH es_periods AS (
    ...
    SELECT id AS unit_id, valid_after, valid_to FROM public.establishment WHERE p_establishment_id_ranges IS NULL OR id <@ p_establishment_id_ranges
    UNION ALL SELECT establishment_id, valid_after, valid_to FROM public.activity WHERE p_establishment_id_ranges IS NULL OR establishment_id <@ p_establishment_id_ranges
    UNION ALL SELECT establishment_id, valid_after, valid_to FROM public.location WHERE p_establishment_id_ranges IS NULL OR establishment_id <@ p_establishment_id_ranges
    UNION ALL SELECT establishment_id, valid_after, valid_to FROM public.contact WHERE p_establishment_id_ranges IS NULL OR establishment_id <@ p_establishment_id_ranges
    UNION ALL SELECT establishment_id, valid_after, valid_to FROM public.stat_for_unit WHERE p_establishment_id_ranges IS NULL OR establishment_id <@ p_establishment_id_ranges
    UNION ALL SELECT establishment_id, valid_after, valid_to FROM public.person_for_unit WHERE p_establishment_id_ranges IS NULL OR establishment_id <@ p_establishment_id_ranges
),
lu_periods_base AS (
    -- 2. Gather periods directly related to the given legal units (NOT from their children yet).
    SELECT id AS unit_id, valid_after, valid_to FROM public.legal_unit WHERE p_legal_unit_id_ranges IS NULL OR id <@ p_legal_unit_id_ranges
    UNION ALL SELECT legal_unit_id, valid_after, valid_to FROM public.activity WHERE p_legal_unit_id_ranges IS NULL OR legal_unit_id <@ p_legal_unit_id_ranges
    UNION ALL SELECT legal_unit_id, valid_after, valid_to FROM public.location WHERE p_legal_unit_id_ranges IS NULL OR legal_unit_id <@ p_legal_unit_id_ranges
    UNION ALL SELECT legal_unit_id, valid_after, valid_to FROM public.contact WHERE p_legal_unit_id_ranges IS NULL OR legal_unit_id <@ p_legal_unit_id_ranges
    UNION ALL SELECT legal_unit_id, valid_after, valid_to FROM public.stat_for_unit WHERE p_legal_unit_id_ranges IS NULL OR legal_unit_id <@ p_legal_unit_id_ranges
    UNION ALL SELECT legal_unit_id, valid_after, valid_to FROM public.person_for_unit WHERE p_legal_unit_id_ranges IS NULL OR legal_unit_id <@ p_legal_unit_id_ranges
),
...
lu_periods_with_children AS (
    ...
    WHERE (p_legal_unit_id_ranges IS NULL OR es.legal_unit_id <@ p_legal_unit_id_ranges) AND after_to_overlaps(p.valid_after, p.valid_to, es.valid_after, es.valid_to)
),
all_periods (unit_type, unit_id, valid_after, valid_to) AS (
    ...
    WHERE (p_establishment_id_ranges IS NULL OR e.id <@ p_establishment_id_ranges) AND after_to_overlaps(p.valid_after, p.valid_to, e.valid_after, e.valid_to)
    UNION ALL
    ...
    WHERE (p_legal_unit_id_ranges IS NULL OR l.id <@ p_legal_unit_id_ranges) AND after_to_overlaps(p.valid_after, p.valid_to, l.valid_after, l.valid_to)
    UNION ALL
    ...
    WHERE (p_enterprise_id_ranges IS NULL OR lu.enterprise_id <@ p_enterprise_id_ranges) AND after_to_overlaps(p.valid_after, p.valid_to, lu.valid_after, lu.valid_to)
    UNION ALL
    ...
    WHERE (p_enterprise_id_ranges IS NULL OR es.enterprise_id <@ p_enterprise_id_ranges) AND after_to_overlaps(p.valid_after, p.valid_to, es.valid_after, es.valid_to)
),
unpivoted AS (
...
BEGIN
    ...
    IF p_establishment_id_ranges IS NULL AND p_legal_unit_id_ranges IS NULL AND p_enterprise_id_ranges IS NULL THEN
        ...
        INSERT INTO public.timepoints SELECT * FROM public.timepoints_calculate(NULL, NULL, NULL);
    ELSE
        -- Partial refresh
