...
all_periods (unit_type, unit_id, valid_after, valid_to) AS (
    ...
    WHERE es.enterprise_id IS NOT NULL AND (p_enterprise_id_ranges IS NULL OR es.enterprise_id <@ p_enterprise_id_ranges) AND after_to_overlaps(p.valid_after, p.valid_to, es.valid_after, es.valid_to)
),
unpivoted AS (
