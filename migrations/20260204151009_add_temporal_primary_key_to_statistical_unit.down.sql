-- Down Migration 20260203233456: add_temporal_primary_key_to_statistical_unit
BEGIN;

-- Restore old GIST exclusion constraint structure
-- NOTE: This will fail if there are overlapping periods in the data.
-- The temporal PK would have prevented that, but the GIST constraint needs a clean slate.

-- Step 1: Recreate old unique indexes
CREATE UNIQUE INDEX statistical_unit_upsert_pkey
    ON public.statistical_unit (unit_type, unit_id, valid_from);

CREATE UNIQUE INDEX statistical_unit_from_key
    ON public.statistical_unit (valid_from, valid_until, unit_type, unit_id);

-- Step 2: Drop temporal PRIMARY KEY
ALTER TABLE public.statistical_unit 
DROP CONSTRAINT statistical_unit_temporal_pk;

-- Step 3: Re-add old GIST exclusion constraint
ALTER TABLE public.statistical_unit 
ADD CONSTRAINT statistical_unit_type_id_daterange_excl
EXCLUDE USING gist (
    unit_type WITH =,
    unit_id WITH =,
...
-- Step 4: Drop valid_range column
ALTER TABLE public.statistical_unit 
DROP COLUMN valid_range;

-- Step 5: Restore original statistical_unit_refresh (uses SELECT * without explicit columns)
...
-- Step 6: Restore original get_statistical_unit_data_partial (without valid_range column)
...
-- Step 7: Restore original relevant_statistical_units (without valid_range column)
