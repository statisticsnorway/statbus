...
BEGIN
    ...
    v_sql := format($$
        ...
        SELECT row_id, %s, %s, tag_id, edit_comment, action 
        FROM public.%I dt WHERE row_id = ANY(%L) AND tag_id IS NOT NULL AND action != 'skip'; -- Added alias dt
    $$, v_select_lu_id_expr, v_select_est_id_expr, v_data_table_name, p_batch_row_ids);
