-- Migration 20260320102510: fix_notify_task_progress_idle_signal
--
-- Fix: send explicit "idle" signals when pipeline phases become inactive.
-- Without this, the frontend stays stuck showing "Recording changes - Running..."
-- because notify_task_progress() only sent pipeline_progress when phases were active,
-- and the frontend preserves previous phase state when absent from the payload.
BEGIN;
