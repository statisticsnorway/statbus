-- Down Migration 20260205022156: optimize_import_procedures_performance
-- Restore the previous versions of the optimized procedures

-- Restore analyse_link_establishment_to_legal_unit from 20260128225812_optimize_link_establishment_range_agg.up.sql
-- (uses CTE chain instead of temp tables)
...
-- Note: For analyse_external_idents, restore from 20250429100000_import_job_procedures_for_external_idents.up.sql
-- The key change being reverted: ExistingIdents join uses IS NOT DISTINCT FROM instead of =
-- This is a placeholder comment - the full function would need to be included from the original migration
-- For brevity, running "migrate down" followed by "migrate up" on the earlier migrations would restore it
