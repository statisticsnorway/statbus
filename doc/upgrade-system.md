...
| `migration-deadlocks-with-running-worker-holding-table-lock` | stall | **R1, architectural.** Worker session holds AccessShareLock on a table; migration's CREATE/DROP INDEX needs AccessExclusiveLock on the same table; lock manager parks the migration indefinitely. The scenario uses `start_continuous_worker_workload` to drive the contention and asserts the upgrade does NOT hang. Surfaces whether the install state machine has a quiesce-services-before-DDL pre-step. |
| `install-flag-released-without-clean-handoff-detected-as-stale` | external | **R3, architectural.** Install exits cleanly but does not release `tmp/upgrade-in-progress.json`; upgrade-service on its next tick observes a flag with a dead holder PID and interprets the state as a crashed install, clearing the flag. No in-code injection — the scenario observes the actual install/service handoff and asserts the journal records the stale-flag path (or that install releases cleanly, depending on the fix). |
| `service-watchdog-timeout-during-db-reconnect-after-container-restart` | stall | **Race D, Layer 1.** Upgrade-service's reconnect loop runs without `WATCHDOG=1` pings to systemd. A long reconnect (DB container restart drags past `WatchdogSec` ~2 min) gets SIGABRTed by systemd. The fix is to ping the watchdog from inside the reconnect loop; the scenario validates restart-counter-bounded recovery. |
| `advisory-lock-attempted-before-db-ready-after-container-restart` | external | **Race E, architectural.** DB container is still restarting when the upgrade-service tries its first advisory lock; the attempt fails and the process exits 42. systemd restarts after backoff; the second attempt succeeds. No in-code injection — the scenario externally orchestrates the container restart timing and asserts the systemd restart counter stays bounded. |
| `seed-restore-runs-on-populated-database-destroying-data` | stall | **R5, architectural — DATA LOSS GRADE.** Install state machine routes to the seed-restore step against a database that already has user data; `pg_restore` of the seed silently destroys existing rows. The scenario starts from a populated DB, drives the install to the seed step, and asserts data survives. `assert_demo_data_present` is the load-bearing check. Per forensics, the fix is to classify DB content before dispatching the seed step. |
