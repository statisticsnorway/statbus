...
- **`fast-tests.yaml`** — gates `./sb release prerelease` (runs in pre-flight; STATBUS-199 D1). Runs after Images on master pushes, plus `pull_request` and `workflow_dispatch`. Self-contained on the GHA runner: builds `sb`, brings up the full Docker stack, and runs `./dev.sh migrate-and-test fast` (the pg_regress fast suite, excluding the large 4xx/5xx import tests). Locally, `./dev.sh continous-integration-test` runs the equivalent fast SQL suite.
