...
### Docker image tag naming (rc.63)

Images are tagged by **commit_short** only — 8-char lowercase hex — never by release tag, never with a `sha-` prefix. This makes pulls content-addressed: the image a deploy fetches is determined by commit identity, not by a human-facing label that could be moved.

| Tag pattern   | Source                                    | Example    |
|---------------|-------------------------------------------|------------|
| `<8-hex>`     | CI workflow (master push) + Release tag   | `abc1234f` |
| `local`       | Local `docker compose build`              | Default when `COMMIT_SHORT` is unset |

The release workflow publishes a `release-manifest.json` asset that maps the release tag → `commit_short` + full `commit_sha`, so consumers that think in release tags can still resolve to an image reference.

Docker Compose files reference images as:

```yaml
image: ghcr.io/statisticsnorway/statbus-app:${COMMIT_SHORT:-local}
```

When `COMMIT_SHORT` is set (via `.env`, generated by `./sb config generate`), containers pull the pre-built image. When unset, they build locally.
