...
Status: **target model, ratified by the King 2026-06-27.** Enabled by the read-only window (read-only-upgrade-window design · STATBUS-110) + error classification (STATBUS-109).
...
- **failed** — a rollback was chosen but its *restore itself* broke (rsync/disk); box can't reach runnable. [HUMAN — "restore-broke"] *Operator UX (grounded 2026-06-28; tmp/mechanic-restore-broke.md): **verified gap** — the failed message already says "manual CLI recovery (./sb install)", but on the common path (marker removed, row=`failed`) `./sb install` only runs the idempotent step-table and **cannot re-attempt the restore** (the snapshot is retained + `restoreDatabase` is a replayable rsync from a `:ro` source — the capability exists, no ladder path reaches it). Fix (rec): teach the install ladder that a `failed` row with a retained `backup_path` is re-attemptable → replay the restore, **human-gated via `./sb install`** (NOT the auto systemd service — restore-broke stays a human stop, no auto-thrash). Then the operator output = a **legend** (`./sb install` headline + state-relevant commands) + a **forecast** (detected state + planned action). Tracked as its own work item (STATBUS-111).*
...
- **STATBUS-110**: build the read-only toggle (ALTER DATABASE default_transaction_read_only; ON before the DB stop, OFF at the 4 verified terminals 4211/4227/4828/5684; upgrade session self-exempts per (re)connect). Arc-tested (STATBUS-071). See the read-only-upgrade-window design.
- **STATBUS-109**: the in-process backoff + the two curated error lists; default unknown→stop.
- **STATBUS-107**: lock the recovery slugs (done — `doc/upgrade-vocabulary.md`) + de-jargon the 3 diagrams (draw corrupt-flag; promote git-unknown + unrecognized-phase from prose to branches; split the single failed/human blob into "unknown" vs "restore-broke").
- **STATBUS-111**: the install-re-attempts-restore + the operator legend/forecast.
