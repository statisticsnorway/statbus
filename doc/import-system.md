...
7.  **Processing Procedures (`analyse_procedure`, `process_procedure`)**:
    ...
    *   They contain the actual logic for:
        *   **Analysis (`analyse_procedure`):**
            ...
            *   **Error Handling (JSONB Columns `errors` and `warnings`)**:
                *   Each `analyse_procedure` is authoritative for specific keys within the `errors` and `warnings` JSONB columns. A key typically corresponds to an input data column name (e.g., `sector_code`, `physical_latitude`) or a specific validation type (e.g., `inconsistent_legal_unit`, `invalid_period_source`).
                *   **Hard Errors (`errors` column)**: Critical issues (e.g., malformed mandatory dates, missing essential identifiers for `external_idents` step) that prevent further processing. The `errors`, `warnings`, and `merge_status` columns are defined as `NOT NULL` with a `DEFAULT '{}'::jsonb` to simplify programming logic by removing the need to handle `NULL` values.
                ...
                *   **Soft Errors / Invalid Codes (`warnings` column)**: Non-critical issues where specific input codes (e.g., `sector_code`) are not found, or optional data is malformed but doesn't halt further analysis by other steps.
                    *   These populate the `warnings` JSONB column, typically storing the original problematic value. Keys in `warnings` should also correspond to `source_input` column names (e.g., `{"sector_code": "INVALID_XYZ"}`).
                    *   They do *not* directly set `state = 'error'` or `action = 'skip'`. The row continues analysis.
                    *   `warnings` should be treated as additive. A step appends its new soft errors by merging its `warnings` JSONB object with the existing `dt.warnings` (e.g., `dt.warnings || new_warnings_jsonb`). `COALESCE` is not needed.
                    *   If a step resolves a soft error it previously reported (e.g., an invalid code becomes valid upon re-evaluation or data correction), it should clear *only its specific key(s)* from the `warnings` column (e.g., `dt.warnings - 'my_invalid_code_key'`).
            ...
            *   The `external_idents` step specifically determines the potential `operation` (`insert`, `replace`, `update`) based on identifier lookups. This column serves two purposes: 1) to provide human-readable insight into the planned action for the primary entity, and 2) for use by "core entity" procedures (e.g., `analyse_legal_unit`) that have business logic which legitimately differs between a patch (`update`) and a full replacement (`replace`). The `analyse_external_idents` step then determines the final `action` based on this `operation`, the job's `strategy`, and any hard errors. A row that passes all analysis steps will have its `action` set to `'use'`, while a row with a hard error will have its `action` set to `'skip'`.
            *   Updates the row's `state` (to `analysing`, or `error` for hard errors), `error` (for hard errors), `warnings` (for soft errors), and `last_completed_priority`.
    ...
    *   **Sub-Entity Processing and the Role of `founding_row_id`**:
        ...
        Therefore, a key architectural principle is the logical decoupling of procedures from this column where possible.
        *   **Core Entity Procedures** (e.g., `analyse_legal_unit`): These procedures MAY use the `operation` column if their business logic genuinely depends on the distinction between a patch (`update`) and a full replacement (`replace`). For example, requiring a `name` for an `insert` or `replace` but not for an `update`.
        *   **Sub-Entity Procedures** (e.g., `process_contact`, `analyse_enterprise_link_for_legal_unit`): These procedures **MUST NOT** read the `operation` column. Instead of checking for `operation = 'insert'`, they must check for `legal_unit_id IS NULL`. Instead of checking for `operation IN ('replace', 'update')`, they must check for `legal_unit_id IS NOT NULL`. This makes their logic more robust and directly tied to the state of the entity ID, rather than an intermediate flag. They follow a more nuanced, two-step logic:
...
3.  **Worker Processing (`admin.import_job_process`)**: The worker picks up the job.
    ...
    *   **Analyse (`admin.import_job_analysis_phase`)**:
        ...
        *   After each transaction, the parent `import_job_process` procedure performs a **Progress Recount**: It recounts `completed_analysis_steps_weighted` to update the job's `analysis_completed_pct` value. The row-level state is **not** changed until the entire phase is complete.
            *   The procedure performs lookups/validations, updates `internal` columns, `error` (for hard errors), and `warnings` (for soft errors).
            *   **Error Handling Rule for Analysis Procedures**:
                ...
                *   **Soft Errors / Invalid Codes**: If an `analyse_procedure` detects a non-critical issue (e.g., an unresolvable `sector_code`), it should:
                    *   Populate the `warnings` JSONB column, merging with existing invalid codes (e.g., `dt.warnings || jsonb_build_object('my_code_key', 'original_value')`).
            ...
            *   When a step successfully processes data or resolves a previously reported issue, it should clear *only its specific keys* from the `errors` and `warnings` columns (e.g., `dt.errors - 'my_error_key'`, `dt.warnings - 'my_code_key'`). This ensures that errors or invalid codes reported by other steps are preserved.
...
3.  **Implement Procedures**: Write the PL/pgSQL functions named in the `import_step` records (`analyse_procedure`, `process_procedure`). These functions must:
    ...
    *   Correctly update `last_completed_priority`, `state`, `action` (for hard errors), `errors` (for hard errors), and `warnings` (for soft errors) columns in `_data`, adhering to the key management principles (each step manages its own keys, `warnings` is additive, `errors` is merged, and specific keys are cleared upon resolution).
    *   **Error Handling Rule for Analysis Procedures (reiteration)**:
        ...
        *   **Soft Errors**: Populate `warnings` JSONB (merging with existing invalid codes). Do *not* set `state = 'error'` or `action = 'skip'` solely for this. `last_completed_priority` advances.
        *   **Clearing Errors**: If a condition is resolved, clear only the step-specific keys from `errors` and/or `warnings`.
...
2.  **Define Data Columns (`import_data_column`)**: For this definition, we need columns in the `_data` table associated with the chosen steps (linked via `step_id`):
    ...
    *   **`external_idents` step:**
        ...
        *   `operation` (purpose: `internal`, type: `public.import_row_operation_type`) - *Determined operation (insert/replace/update) based on identifier lookups. Used for human insight and by core entity procedures.*
    ...
    *   **`metadata` columns (implicitly added):**
        ...
        *   `warnings` (purpose: `metadata`, type: `JSONB NOT NULL DEFAULT '{}'`)
...
3.  **Implement Procedures**: Write the PL/pgSQL functions for each step (e.g., `admin.analyse_edit_info`, `admin.analyse_external_idents`, `admin.analyse_enterprise_link_for_legal_unit`, `admin.process_legal_unit`, etc.). These functions must:
    ...
    *   Read from/write to the `_data` table using dynamic SQL and `p_batch_row_ids` (e.g., `WHERE row_id = ANY(p_batch_row_ids)`).
    *   Perform logic (lookups, validation, casting for analysis; final DML using audit info and `action` from `_data` for processing).
    *   Correctly update `last_completed_priority`, `state`, `action` (if an error occurs during analysis), and `error`/`warnings` columns in `_data`, following the key management and additive principles.
    *   **Error Handling Rule for Analysis Procedures**: If an `analyse_procedure` detects an error making the row unprocessable, it *must* set `state = 'error'`, `action = 'skip'`, and store details in the `error` JSONB column (merging with existing errors). `last_completed_priority` should be advanced to the current step's priority. Soft errors are added to `warnings` without setting `state='error'` or `action='skip'`.
    *   Handle batch-wide errors (e.g., constraint violation during a batch DML in a `process_procedure`) by marking all rows identified by `p_batch_row_ids` as error (and action='skip') in the `_data` table and potentially failing the job.
