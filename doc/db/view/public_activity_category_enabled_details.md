```sql
                             View "public.activity_category_enabled"
...
View definition:
 SELECT acs.code AS standard_code,
  ...
  ORDER BY ac.path;
Triggers:
    activity_category_enabled_upsert_custom INSTEAD OF INSERT ON activity_category_enabled FOR EACH ROW EXECUTE FUNCTION admin.activity_category_enabled_upsert_custom()
