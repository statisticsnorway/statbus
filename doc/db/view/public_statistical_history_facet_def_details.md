...
View definition:
 WITH year_with_unit_basis AS (
         SELECT range.resolution,
            ...
            su_curr.birth_date,
            su_curr.death_date,
            COALESCE(su_curr.birth_date >= range.curr_start AND su_curr.birth_date <= range.curr_stop, false) AS born,
            COALESCE(su_curr.death_date >= range.curr_start AND su_curr.death_date <= range.curr_stop, false) AS died,
           ...
           FROM get_statistical_history_periods('year'::history_resolution) range(resolution, year, month, prev_stop, curr_start, curr_stop)
             JOIN LATERAL ( SELECT range_units.unit_type,
                    ...
                    range_units.unit_size_id,
                    range_units.unit_size_code,
                    range_units.status_id,
                    range_units.status_code,
                    range_units.include_unit_in_reports,
                    range_units.last_edit_comment,
                    range_units.last_edit_by_user_id,
                    range_units.last_edit_at,
                    range_units.invalid_codes,
                    range_units.has_legal_unit,
                    range_units.related_establishment_ids,
                    range_units.excluded_establishment_ids,
                    range_units.included_establishment_ids,
                    range_units.related_legal_unit_ids,
                    range_units.excluded_legal_unit_ids,
                    range_units.included_legal_unit_ids,
                    range_units.related_enterprise_ids,
                    range_units.excluded_enterprise_ids,
                    range_units.included_enterprise_ids,
                    range_units.stats,
                    range_units.stats_summary,
                    range_units.included_establishment_count,
                    range_units.included_legal_unit_count,
                    range_units.included_enterprise_count,
                   ...
                   FROM ( SELECT su_range.unit_type,
                            ...
                            su_range.fax_number,
                            su_range.unit_size_id,
                            su_range.unit_size_code,
                            su_range.status_id,
                            su_range.status_code,
                            su_range.include_unit_in_reports,
                            su_range.last_edit_comment,
                            su_range.last_edit_by_user_id,
                            su_range.last_edit_at,
                            su_range.invalid_codes,
                            su_range.has_legal_unit,
                            su_range.related_establishment_ids,
                            su_range.excluded_establishment_ids,
                            su_range.included_establishment_ids,
                            su_range.related_legal_unit_ids,
                            su_range.excluded_legal_unit_ids,
                            su_range.included_legal_unit_ids,
                            su_range.related_enterprise_ids,
                            su_range.excluded_enterprise_ids,
                            su_range.included_enterprise_ids,
                            su_range.stats,
                            su_range.stats_summary,
                            su_range.included_establishment_count,
                            su_range.included_legal_unit_count,
                            su_range.included_enterprise_count,
                            su_range.tag_paths,
                          ...
                          WHERE from_to_overlaps(su_range.valid_from, su_range.valid_to, range.curr_start, range.curr_stop) AND (su_range.death_date IS NULL OR range.curr_start <= su_range.death_date) AND (su_range.birth_date IS NULL OR su_range.birth_date <= range.curr_stop) AND su_range.include_unit_in_reports) range_units
                  WHERE range_units.last_in_range) su_curr ON true
             LEFT JOIN LATERAL ( SELECT su_range.unit_type,
                    su_range.unit_id,
                    su_range.valid_from,
                    su_range.valid_to,
                    su_range.valid_until,
                    ...
                    su_range.tag_paths
                   FROM statistical_unit su_range
                  WHERE su_range.unit_type = su_curr.unit_type AND su_range.unit_id = su_curr.unit_id AND from_to_overlaps(su_range.valid_from, su_range.valid_to, range.curr_stop, range.curr_stop) AND (su_range.death_date IS NULL OR range.curr_start <= su_range.death_date) AND (su_range.birth_date IS NULL OR su_range.birth_date <= range.curr_stop) AND su_range.include_unit_in_reports
                  ORDER BY su_range.valid_from
                 LIMIT 1) su_next ON true
             LEFT JOIN statistical_unit su_prev ON su_prev.unit_type = su_curr.unit_type AND su_prev.unit_id = su_curr.unit_id AND from_to_overlaps(su_prev.valid_from, su_prev.valid_to, range.prev_stop, range.prev_stop) AND (su_prev.death_date IS NULL OR range.curr_start <= su_prev.death_date) AND (su_prev.birth_date IS NULL OR su_prev.birth_date <= range.curr_stop) AND su_prev.include_unit_in_reports
         ...
         SELECT basis.resolution,
            ...
            basis.prev_unit_size_id,
            basis.prev_unit_size_code,
            basis.curr_unit_size_id,
            basis.curr_unit_size_code,
            ...
            basis.track_changes AND NOT basis.born AND NOT basis.died AND basis.prev_unit_size_code IS DISTINCT FROM basis.curr_unit_size_code AS unit_size_code_changed,
         ...
         SELECT source.resolution,
          ...
          GROUP BY source.resolution, source.year, source.unit_type, source.curr_primary_activity_category_path, source.curr_secondary_activity_category_path, source.curr_sector_path, source.curr_legal_form_id, source.curr_physical_region_path, source.curr_physical_country_id, source.curr_unit_size_id, source.curr_status_id
         ...
         SELECT source.resolution,
          ...
          GROUP BY source.resolution, source.year, source.month, source.unit_type, source.curr_primary_activity_category_path, source.curr_secondary_activity_category_path, source.curr_sector_path, source.curr_legal_form_id, source.curr_physical_region_path, source.curr_physical_country_id, source.curr_unit_size_id, source.curr_status_id
        )
 SELECT year_with_unit_per_facet.resolution,
    year_with_unit_per_facet.year,
    year_with_unit_per_facet.month,
    year_with_unit_per_facet.unit_type,
    year_with_unit_per_facet.primary_activity_category_path,
    year_with_unit_per_facet.secondary_activity_category_path,
    year_with_unit_per_facet.sector_path,
    ...
    year_with_unit_per_facet.unit_size_id,
    ...
    year_with_unit_per_facet.name_change_count,
    year_with_unit_per_facet.primary_activity_category_change_count,
    year_with_unit_per_facet.secondary_activity_category_change_count,
    year_with_unit_per_facet.sector_change_count,
    year_with_unit_per_facet.legal_form_change_count,
    ...
    year_with_unit_per_facet.unit_size_change_count,
    year_with_unit_per_facet.status_change_count,
    year_with_unit_per_facet.stats_summary
   FROM year_with_unit_per_facet
UNION ALL
 SELECT year_and_month_with_unit_per_facet.resolution,
    year_and_month_with_unit_per_facet.year,
    year_and_month_with_unit_per_facet.month,
    year_and_month_with_unit_per_facet.unit_type,
    year_and_month_with_unit_per_facet.primary_activity_category_path,
    year_and_month_with_unit_per_facet.secondary_activity_category_path,
    year_and_month_with_unit_per_facet.sector_path,
    ...
    year_and_month_with_unit_per_facet.unit_size_id,
    ...
    year_and_month_with_unit_per_facet.name_change_count,
    year_and_month_with_unit_per_facet.primary_activity_category_change_count,
    year_and_month_with_unit_per_facet.secondary_activity_category_change_count,
    year_and_month_with_unit_per_facet.sector_change_count,
    year_and_month_with_unit_per_facet.legal_form_change_count,
    ...
    year_and_month_with_unit_per_facet.unit_size_change_count,
