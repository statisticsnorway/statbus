```sql
CREATE OR REPLACE FUNCTION admin.upsert_generic_valid_time_table(schema_name text, table_name text, unique_columns jsonb, temporal_columns text[], ephemeral_columns text[], new record)
 RETURNS integer
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
  ...
  BEGIN
    -- Validate that all temporal columns have non-null values
    WITH temporal_validation AS (
      SELECT column_name
      FROM unnest(temporal_columns) AS column_name
      WHERE (new_data->>column_name) IS NULL
    )
    SELECT string_agg(column_name, ', ' ORDER BY column_name) AS missing_columns
    INTO missing_required_columns
    FROM temporal_validation;

    IF missing_required_columns IS NOT NULL THEN
      RAISE EXCEPTION 'Temporal columns cannot be null: %', missing_required_columns;
    END IF;
  END;
