```sql
CREATE OR REPLACE PROCEDURE admin.process_enterprise_link_for_establishment(IN p_job_id integer, IN p_batch_ctids tid[])
 LANGUAGE plpgsql
AS $procedure$
DECLARE
...
BEGIN
    ...
    v_sql := format($$
        INSERT INTO temp_new_est (ctid, name, edit_by_user_id, edit_at)
    ...
    BEGIN
        v_sql := format($$
            WITH inserted_enterprises AS (
                ...
                SELECT tne.name, tne.edit_by_user_id, tne.edit_at
            ...
            SELECT tne.ctid, ie.id
            FROM temp_new_est tne
            JOIN inserted_enterprises ie ON tne.name = ie.name; -- Linking back via name is weak
        $$);
