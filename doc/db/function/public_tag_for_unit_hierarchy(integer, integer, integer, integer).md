```sql
CREATE OR REPLACE FUNCTION public.tag_for_unit_hierarchy(parent_establishment_id integer DEFAULT NULL::integer, parent_legal_unit_id integer DEFAULT NULL::integer, parent_enterprise_id integer DEFAULT NULL::integer, parent_power_group_id integer DEFAULT NULL::integer)
 RETURNS jsonb
 LANGUAGE sql
 STABLE
AS $function$
  WITH ordered_data AS (
    SELECT to_jsonb(t.*)
     ...
     WHERE (  parent_establishment_id    IS NOT NULL AND tfu.establishment_id    = parent_establishment_id
           ...
           OR parent_power_group_id IS NOT NULL AND tfu.power_group_id = parent_power_group_id
           )
       ORDER BY t.path
  ), data_list AS (
