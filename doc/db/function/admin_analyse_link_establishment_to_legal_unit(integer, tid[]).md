```sql
CREATE OR REPLACE PROCEDURE admin.analyse_link_establishment_to_legal_unit(IN p_job_id integer, IN p_batch_ctids tid[])
 LANGUAGE plpgsql
AS $procedure$
DECLARE
...
BEGIN
    ...
    v_sql := format('
        ...
        SELECT
            up.data_ctid, up.ident_code, up.ident_value, xit.id, xifu.legal_unit_id
        FROM ( %s ) up
        ...
        LEFT JOIN public.external_ident_for_unit xifu ON xifu.external_ident_id = xi.id AND xifu.legal_unit_id IS NOT NULL; -- Only match LU links
    ', v_unpivot_sql);
    ...
    v_sql := format('
        WITH RowChecks AS (
            ...
            LEFT JOIN temp_unpivoted_lu_idents tui ON orig.data_ctid = tui.ctid
            GROUP BY orig.data_ctid
        )
        INSERT INTO temp_batch_errors (ctid, error_jsonb)
        SELECT
            data_ctid,
