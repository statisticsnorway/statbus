```sql
CREATE OR REPLACE PROCEDURE admin.process_establishment(IN p_job_id integer, IN p_batch_ctids tid[])
 LANGUAGE plpgsql
AS $procedure$
DECLARE
...
BEGIN
    ...
    BEGIN
        ...
        v_sql := format('
            WITH est_lookup AS (
                 SELECT DISTINCT ON (xi.value) est.id as establishment_id, xi.value as tax_ident
                 FROM public.establishment est
                 JOIN public.external_ident_for_unit xifu ON xifu.establishment_id = est.id
                 ...
                 ORDER BY xi.value, est.id DESC -- Get latest ID in case of duplicates
            )
            UPDATE public.%I dt SET
