```sql
CREATE OR REPLACE PROCEDURE admin.process_stats_for_unit(IN new_jsonb jsonb, IN unit_type text, IN unit_id integer, IN valid_from date, IN valid_to date, IN data_source_id integer)
 LANGUAGE plpgsql
AS $procedure$
DECLARE
    stat_code TEXT;
