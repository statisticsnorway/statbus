```sql
CREATE OR REPLACE FUNCTION public.connect_legal_unit_to_enterprise(legal_unit_id integer, enterprise_id integer, valid_from date DEFAULT CURRENT_DATE, valid_to date DEFAULT 'infinity'::date)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
