...
-----------------------+--------------------------+-----------+----------+---------------------------------------------------------------------------+----------+-------------+--------------+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 error                 | text                     |           |          |                                                                           | extended |             |              | 
 rollback_completed_at | timestamp with time zone |           |          |                                                                           | plain    |             |              | 
 skipped_at            | timestamp with time zone |           |          |                                                                           | plain    |             |              | 
 from_version          | text                     |           |          |                                                                           | extended |             |              | 
 images_downloaded     | boolean                  |           | not null | false                                                                     | plain    |             |              | 
 backup_path           | text                     |           |          |                                                                           | extended |             |              | 
 superseded_at         | timestamp with time zone |           |          |                                                                           | plain    |             |              | 
 progress_log          | text                     |           |          |                                                                           | extended |             |              | Tail of the upgrade progress log (last ~50 lines). Populated on success, rollback, or direct failure by the upgrade service so operators can inspect what happened from the admin UI.
 ...
 artifacts_ready       | boolean                  |           |          | generated always as (docker_images_ready AND release_builds_ready) stored | plain    |             |              | Composite ready flag, GENERATED STORED from docker_images_ready AND release_builds_ready. True when both levels of CI output are published and the upgrade is safe to start. Indexable; appears in select=* automatically.
 ...
 discover_version_tag  | text                     |           |          |                                                                           | extended |             |              | Output of `git describe --tags --always <commit_sha>` captured at discovery time. Used by the upgrade service to look up Docker images in GHCR without drift caused by later tags being pushed past the commit.
Indexes:
...
CASE state
    WHEN 'available'::upgrade_state THEN scheduled_at IS NULL AND started_at IS NULL AND completed_at IS NULL AND error IS NULL AND rollback_completed_at IS NULL AND skipped_at IS NULL AND dismissed_at IS NULL AND superseded_at IS NULL
    WHEN 'scheduled'::upgrade_state THEN scheduled_at IS NOT NULL AND started_at IS NULL AND completed_at IS NULL AND error IS NULL AND rollback_completed_at IS NULL
    WHEN 'in_progress'::upgrade_state THEN scheduled_at IS NOT NULL AND started_at IS NOT NULL AND completed_at IS NULL AND error IS NULL AND rollback_completed_at IS NULL
    WHEN 'completed'::upgrade_state THEN completed_at IS NOT NULL AND error IS NULL AND rollback_completed_at IS NULL
    WHEN 'failed'::upgrade_state THEN error IS NOT NULL AND started_at IS NOT NULL AND completed_at IS NULL AND rollback_completed_at IS NULL
    WHEN 'rolled_back'::upgrade_state THEN rollback_completed_at IS NOT NULL AND error IS NOT NULL AND completed_at IS NULL
    WHEN 'dismissed'::upgrade_state THEN dismissed_at IS NOT NULL AND (error IS NOT NULL OR rollback_completed_at IS NOT NULL)
...
Not-null constraints:
    ...
    "upgrade_images_downloaded_not_null" NOT NULL "images_downloaded"
