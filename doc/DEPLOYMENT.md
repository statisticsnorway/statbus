...
|----------|---------|-------------|
| `UPGRADE_ROLE` | `production` | What the box is: `production`, `canary`, or `development`. The release channel (`stable` / `prerelease` / `local`) is DERIVED from it into the generated `.env` — `UPGRADE_CHANNEL` is not a setting and is refused in `.env.config` (STATBUS-254) |
