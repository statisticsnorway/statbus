...
- `doc/upgrades.md` — daemon operation, lifecycle, rollback, troubleshooting
- `doc/releases.md` — CalVer, release pipeline, CI images, image cleanup
- `doc/DEPLOYMENT.md` — standalone deployment with upgrade daemon section
...
2. **Rollback `git checkout -f` unchecked** — DONE
   - `daemon.go`: rollback() now checks error from git checkout and config generate, logs CRITICAL warning
...
7. **syncConfigToSystemInfo silent errors** — DONE
   - `daemon.go`: now logs warnings to stderr for .env load failures and Exec errors

### Low / Accept As-Is

8. **skipOlderReleases `discovered_at` ordering** — accepted (GitHub API returns chronological order)
...
Implemented in `daemon.go`. After health check passes, checks for `tmp/simulate-upgrade-failure`:
...
Tag and push after all fixes committed. Daemon on dev.statbus.org applies it.
