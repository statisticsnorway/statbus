...
| `GITHUB_TOKEN` | *(unset)* | Optional fine-grained, read-only, repository-scoped GitHub token used by release REST requests and invocation-scoped HTTPS `git fetch`. Anonymous HTTPS remains the default. Fresh `.env.config` files document a commented entry, but `./sb config generate` never inserts the key into an existing file. A process environment value wins over `.env.config`; the value is never propagated to another installation. |
