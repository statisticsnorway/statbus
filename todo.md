...
- [x] **Refactor: Generate database types via SQL**
    - **Problem**: The `database.types.ts` file is currently generated by an external tool (`supabase gen types`). This creates an external dependency and offers less control over the output.
    - **Solution**: Create a PL/pgSQL function that introspects the database schema and generates the TypeScript types directly.
    - **Plan**:
        1. [x] Create a prototype SQL script (`tmp/generate_database_types.sql`) that defines and calls a generator function.
        2. [x] Implement the function to generate Enums.
        3. [x] Extend the function to generate Table `Row` types with correct column-to-TS type mapping.
        4. [x] Extend the function to generate Table `Insert` and `Update` types.
        5. [x] Extend the function to generate `Relationships` based on foreign keys.
        5a. [x] Extend the function to generate View `Insert` and `Update` types.
        6. [x] The SQL-based generator is now feature-complete.
        7. [ ] **Next:** Replace the `generate-types` command in `devops/manage-statbus.sh` with the new SQL script.
