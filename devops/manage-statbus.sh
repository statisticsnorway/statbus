     ...
     'psql' )
        ...
        if $(which psql > /dev/null); then
          ...
          if test -t 0 && test -t 1 && test ! -p /dev/stdin && test ! -f /dev/stdin; then
            ...
            PGOAUTHDEBUG=UNSAFE docker compose exec -w /statbus -e PGOAUTHDEBUG -e PGPASSWORD -u postgres db psql -U $PGUSER $PGDATABASE "$@"
          else
            # Non-interactive mode - explicitly disable TTY allocation
            PGOAUTHDEBUG=UNSAFE docker compose exec -T -w /statbus -e PGOAUTHDEBUG -e PGPASSWORD -u postgres db psql -U $PGUSER $PGDATABASE "$@"
          fi
        fi
      ;;
