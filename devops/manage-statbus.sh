...
EOS
        ...
        # Calculate port offsets based on deployment slot
        BASE_PORT=3000
        SLOT_MULTIPLIER=10
        PORT_OFFSET=$((BASE_PORT + DEPLOYMENT_SLOT_PORT_OFFSET * SLOT_MULTIPLIER))

        # Set addresses using consistent port calculation
        ...
        DB_PUBLIC_LOCALHOST_PORT="$((PORT_OFFSET + 2))"
...
################################################################
EOS
        # Generate deployment.caddyfile based on current configuration
        DEPLOYMENT_SLOT_CODE=$(./devops/dotenv --file .env.config get DEPLOYMENT_SLOT_CODE | tr -d '"')
        DEPLOYMENT_USER="statbus_${DEPLOYMENT_SLOT_CODE}"
        DOMAIN="${DEPLOYMENT_SLOT_CODE}.statbus.org"
        APP_PORT=$(echo "$APP_BIND_ADDRESS" | cut -d':' -f2)
        API_PORT=$(echo "$SUPABASE_BIND_ADDRESS" | cut -d':' -f2)

        cat > deployment.caddyfile <<EOF
# Generated by manage-statbus.sh
...
EOF
        echo "Generated deployment.caddyfile for $DOMAIN"
        ;;
     'postgres-variables' )
        PGHOST=127.0.0.1
